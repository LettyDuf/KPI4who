...
- **Prochaine action recommandée** : **volet gestion du changement du constat n°10 LIVRÉ** (cadre `change` + 6 fiches `cg-*` + câblage de `resistance-transformation`, tag `mvp-cadre-changement-livre`). **Prochaine étape : le second volet du mini-chantier, constat `26.c4.coll.1`** — créer le cadre Management par objectifs et les 9 indicateurs de discipline de cascade MBO, puis adosser `cascade-objectifs` au référentiel (même patron que le volet gestion du changement). Puis **jalon D** (retrait des 5 mockups autonomes orphelins) et **jalon E** (tests E2E + smoke test global). **Point transverse ouvert** : brancher les liens sortants des fiches-questions vers une vue d'ensemble de leur cadre. **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord, **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**, **26.c1.coll.1 (dette NBSP module fiche-question)**, **26.c2.coll.1 (fiche-question candidate post-MVP)**, **26.c4.coll.1 (cadre MBO + 9 indicateurs de cascade)**, **26.c4.coll.2 (crossLink cascade ↔ pasteque muet)**. **Demande différée de Lætitia** : rendre cliquables les noms d'indicateurs dans les pieds de colonne § 4.3. Aucun blocage actif. Tag de point de retour courant : `mvp-cadre-changement-livre`.
