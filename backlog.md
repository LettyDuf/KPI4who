...
- **État de l'outil après reprise 09/05/2026 (`fec3ca2`)** : sept changements depuis `ed52a23` — (a) retrait CSS mort `.porte-statut` (`35bbc61`), (b) mission « manifeste deux étages » (`378634d`, +41 lignes), (c) doctrine § 6.1 strate 2 (`58a5dca`, +15 lignes doc-cadre-visuel.md), (d) tutoiement 4 postures par rôle (`26a27b6`, 6 remplacements), (e) tutoiement conseil pédagogique (`230db2a`, 5 remplacements), (f) tutoiement 2 messages d'action (`aa8d9d5`), (g) tutoiement 8 mockups doctrinaux (`ff83d5c`, 29 remplacements). Smoke test interactif à reprendre côté Lætitia : (i) rendu de la mission, (ii) postures par rôle dans la porte niveau, (iii) conseil pédagogique au-dessus d'une liste filtrée, (iv) confirmation de réinitialisation du tableau de bord. Les invariants I1-I6 du `journal-invariants-23g.md` restent valides (aucune modification du référentiel ni du moteur de filtrage).
...
- **Prochaine action recommandée** : ordre validé par Lætitia en reprise 09/05/2026 — (a) **commit cosmétique CSS mort `.porte-statut`** ✅ livré (`35bbc61`), (b) **mission de l'accueil unifié** ✅ livré (`378634d` + `58a5dca`, composition Beta), (c) **chantier 22 harmonisation tutoiement** ✅ livré (`26a27b6` + `230db2a` + `aa8d9d5` + `ff83d5c`, 42 corrections sur 9 fichiers), (d) **chantier 25 audit zones inactives** ouvert (`fec3ca2`) — candidat naturel pour la prochaine séance car il prolonge directement la cartographie du chantier 22 (cas connu : `_rendreQuestionnaire()` VUE 3). Un seul **point reporté** restant : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher). Autres chantiers ouverts cumulés : **21 (vue Lexique)**, **13.bis (chips cumulables maturité — en attente du retour de la maturité au périmètre actif)**, **24 (URLs partageables des filtres — en attente trigger)**. Aucun blocage actif sur l'outil. Mockup d'arbitrage `mockup-mission-accueil.html` v6 conservé à la racine (référence pour la doctrine de la mission).
