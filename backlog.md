...
- **Prochaine action recommandée** : séance 09/05/2026 close à `89ffeab` après 18 commits cumulés. **Deux chantiers ouverts en attente d'arbitrage utilisateur, à reprendre en priorité** : **(I) Audit accents + em dash dans les fiches du référentiel** — diagnostic posé : 204 em dash dans les chaînes des champs `definition / objectif / frequence / exemple_* / risque / alt`, plus une dette d'accents systémique observée sur les fiches anciennes (*marche, capture, cles, gere, versionne, opportunites, equivalence, recurrents, percu, resoudre, entrainent, degradation, consomme, planifie, controler, depassements, lancee, depenses, supplementaires, correctrice* et probablement bien d'autres). Trois options sur la table à arbitrer en reprise : (A) agent autonome qui audite tout et applique en bloc — risque sur les arbitrages éditoriaux ; (B) audit séquentiel par Claude avec table de remplacements validée par Lætitia mot par mot — plus lent mais contrôlé ; (C) lots ciblés boy scout sur les ~20 fiches visibles dans les screenshots actuels, le reste en chantier 26. **(II) Option A pour le tableau de bord** — basculer `_htmlCartePanier()` vers le format unifié `htmlCarte()` (avec X retirer à la place des boutons panier ✓ / 💡), engagement pris mais non livré. Effort ~30-45 min. **Point reporté restant** : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher). **Autres chantiers ouverts cumulés** : **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables des filtres)**, **25 (audit zones inactives, ouvert cette séance)**. Aucun blocage actif sur l'outil.
