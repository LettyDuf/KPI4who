...
Généralisation à l'ensemble du référentiel en cours, par lots éditoriaux. **Lot A livré** (`422b0a3`, 21/04/2026), **Lot B livré** (`9846600`, 21/04/2026). Lots C (Support client / Qualité, ~4 liens) et D (Sécurité / Incident, ~2 liens + arbitrage MTTR DORA vs MTTR Sécurité) encore à faire — voir détail plus bas.
...
1. **Lot C — Support client / Qualité** (🟢). Quatre liens, dont `ti-o2.risque ↔ af-sc4` qui croise les deux cultures (TI / Affaires). Session très courte.
2. **Lot D — Sécurité / Incident** (🟡). Deux liens mais un arbitrage sur `s9.alt → MTTR` : pointe-t-on vers o4 (DORA générique) ou ti-s2 (MTTR Sécurité) ? À décider par lecture contextuelle du champ.
...
5. **7.2 — Stepper générique extensible** (🟡). Chantier structurant, à coupler avec la symétrisation des portes *problème* et *niveau*.
