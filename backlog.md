...
Dernière mise à jour : **2 mai 2026 — reprise du chantier 10, D1 tranchée**. Ordre de cadrage validé et consigné dans `doc-contrats-navigation.md` § 8.1bis (commit `7c29031`). **D1 (sort de `vue-questionnaire`) tranchée** : (c) suppression en 10.3 + (a) matière sauvegardée dans `matiere-questionnaire-pour-catalogue.md` (commit `ddd98d9`). 3 décisions structurantes restent (D2 lentilles, D3 mode fin, D4 responsive) + 2 tactiques (D5-D6) + 2 chantiers éditoriaux interdépendants. Étape 2 du cadrage (D2+D4 ensemble) à ouvrir à la prochaine reprise.
