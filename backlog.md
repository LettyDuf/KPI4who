...
- **Points reportés cumulés (à reprendre en commits dédiés)** :
  ...
  3. **CSS mort `.porte-statut` / `.porte-statut.bientot`** (~ligne 1101-1115 de cadre-indicateurs.html, identifié au 23.g-1 lors de la cartographie) — décrivait le badge « Bientôt » de la porte maturité, supprimée en 23.f. ~15 lignes mortes à retirer dans un commit cosmétique léger.
- **État de l'outil après 23.g-3** : aucun changement de comportement (chantier documentaire pur). L'outil tourne nominalement, smoke test interactif soir 8/05 validé (cf. journal-invariants-23g.md).
- **Mockups produits cette séance prolongée** : aucun. **Harnais de tests produit en 23.g-4.1 + étoffé en 23.g-4.2** : `tests-accueil-unifie.html` (1457 l. dont 774 injectées + 6 sous-sections de tests A.0-A.6) + `outils/construire-tests-accueil-unifie.js` (170 l.). Smoke-test 21/21 verts (4 pilotes + 17 unitaires en section A).
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` toujours actif (à étendre à `.git/HEAD.lock` si besoin — observé sur le commit du journal 23.g-3). ~14 paires `.git/*.lock.tmp` accumulées sur cette séance prolongée, s'ajoutent aux ~44 paires des séances précédentes. Plus ~10 fichiers `.todelete-23*` (backups + fichiers retirés via mv côté sandbox).
- **Prochaine action recommandée** : enchaîner sur **23.g-4.3 — section B (sentinelles I1-I6 sur référentiel réel)**. Mécanique : substituer les stubs `CM.Referentiel` (fiches synthétiques) et `CM.IndicateursMeta` (zone de données stub) par un inlining du bloc réel issu de `cadre-indicateurs.html`. Pour ce faire, étendre le générateur `outils/construire-tests-accueil-unifie.js` pour qu'il extraie aussi le bloc `CM.Referentiel — BEGIN/END` et la zone `CM._indicateursMetaData:BEGIN/END`, et les inline dans une seconde zone d'injection du harnais. Côté tests, ajouter 6 sentinelles (I1-I6) calées sur les valeurs exactes du `journal-invariants-23g.md` à `d03ccd0` : 89 fiches au total, Niveau · Programme = 9, Cadre · DORA = 4, Niveau · Programme + Cadre · DORA = 0 (combinaison vide → libellé canonique), etc. Effort estimé ~45-60 min. **Question d'ouverture pour 23.g-4.3** : le harnais devra alors basculer entre stub et référentiel réel selon la section ; deux options — (a) deux instances du harnais (un fichier section A, un fichier section B), (b) un seul fichier avec les deux blocs montés simultanément + une fonction `monterStub()` / `monterReel()` qui swappe les ports avant chaque test. Option (b) plus compact mais plus fragile (risque de fuite d'état entre tests). À trancher en ouverture.
