...
- **Événements majeurs de la séance (07/05/2026)** : (1) **fiche pastèque** livrée (chantier 20.7, doctrine § 4.6 *Trio à 3 axes invariants* inaugurée). (2) **Cross-link réciproque** entre les 2 fiches-questions livrées (décisions 20.7 et 20.8 closes). (3) **Fiche pilotage hebdomadaire** livrée en v1 puis **reformulée en v2** *« Comment mesurer l'occupation hebdomadaire de mon équipe ? »* après revue Lætitia (la v1 *« Quoi mettre dans mon pilotage hebdomadaire ? »* était ouverte, incompatible avec § 4.3 — reformulée pour exposer le terme-piège *occupation*). (4) **Doctrine fond fiche § 6.5 retrouvée** (commit fondateur `17d62f3` du 06/05) — `.fiche` en blanc franc, pas en crème ; bug propagé sur pastèque + pilotage hebdo, corrigé sur les deux mockups en rétrofit.
- **10 commits sur la séance** : `f5fb2df` `9baa152` `b2798cf` `cec66e0` `97eb530` `ffba1ae` `b8de41b` `cb1248c` (bloc 1+2) ; `528d08f` (pilotage hebdo v2 — reformulation + suppression boutons + fond blanc) ; `8367a9c` (rétrofit pastèque fond blanc).
- **Mockups produits** : `mockup-fiche-question-pasteque.html` (rétrofit fond blanc) · `mockup-fiche-question-pilotage-hebdo.html` (v2 reformulé). Tous deux en palette D.
...
- **Question candidate ajoutée** : *« Quoi mettre dans mon pilotage hebdomadaire ? »* (déversée lors de la reformulation 07/05) — à reverser comme fiche standard § 4.4 avec trio par niveau si le besoin se confirme.
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges (4 identifiés à étoffer + *occupation* nouvellement nommée) · 20.3 volumétrie panels · 20.4 responsive double réponse (couplée à D4) · 20.6 trio alternatif v2.
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→J` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **rédiger une 4<sup>e</sup> fiche-question** sur un panel encore à 0 (Coach exécutif, Gestion du changement, Terrain des autres cadres) ; (b) **mettre en mockup la 1<sup>re</sup> fiche** d'amélioration continue (rédigée 05/05 sans mockup) ; (c) **traiter une fiche déclarative manquante** (équipe ou programme) ; (d) **rédiger la fiche cadre patrimoniale triangulation** ; (e) **traiter la dette filets niveau** (§ 2). NB : **doctrine § 6.5 fond fiche désormais en mémoire** (`project_doctrine_fond_fiche.md`) — ne plus recopier aveuglément le CSS d'un mockup antérieur.
