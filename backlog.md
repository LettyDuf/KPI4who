...
- **Événements majeurs de la séance (07/05/2026 soir)** : (1) **4<sup>e</sup> fiche-question MVP** *« Par où démarrer une démarche d'amélioration continue ? »* (panel Coach Lean) autonomisée depuis le mockup format — doctrine *autonome = source unique, mockup-format pointe* actée. (2) **Mockup-format scène 1 → miroir didactique** + **cross-link pasteque ↔ amélio continue** réciproque effectif. (3) **5<sup>e</sup> fiche-question MVP** *« Pourquoi ma transformation résiste ? »* (panel Gestion du changement — Kotter / ADKAR / Bridges / Lewin) livrée puis itérée 2 fois (v2 « équipe pilote » → « en transformation », v3 sigles ADKAR + anglicismes nettoyés). **Premier test § 4.4 hors couloir Lean-Agile-Drucker concluant.** ADKAR utilisé comme grille pédagogique de Q1, mots français dans les cards.
- **6 commits sur la séance soir** : `9eaf704` (4e fiche autonome) · `613f46f` (mockup-format → miroir) · `f5cf068` (pasteque réciproque actée) · `6b39cad` (5e fiche v1) · `07aa693` (v2 équipe en transformation) · `6ad97bf` (v3 français orthodoxe).
- **Mockups produits cette séance** : `mockup-fiche-question-amelioration-continue.html` (457 lignes, palette D, § 4.4 trio par niveau, panel Coach Lean) · `mockup-fiche-question-resistance-transformation.html` (489 lignes v3, palette D, § 4.4 trio par niveau, panel Gestion du changement, anatomie enrichie d'un sur-libellé `.indicateur-phase`).
- **Avancement chantier 20.5** : 5/8-12 fiches MVP livrées. Panels couverts : Coach Lean (2 — amélio continue + scène 1 originelle), Drucker méta (1 — pasteque), Coach Agile reformulation (1 — pilotage hebdo), Gestion du changement (1 — résistance). Panels encore à 0 : Coach exécutif (Drucker MBO orthodoxe), Terrain des autres cadres (BSC, OKR Doerr, ITIL).
...
- **Dette de référentiel cumulée** : (issue pastèque) 2 fiches déclaratives + 1 fiche cadre triangulation ; (issue résistance) 3 fiches métriques absentes (Score conscience-désir, Taux d'adoption, Fréquence régressions) + fiche cadre *Gestion du changement* (Kotter / ADKAR / Prosci / Bridges / Lewin) — directement motivée par cette livraison, à inscrire formellement comme constat collatéral n°10 du chantier 1.ter aboutissant.
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→V` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **continuer le MVP** sur un panel encore à 0 — Coach exécutif Drucker MBO orthodoxe (test naturel après ADKAR pour fermer la famille Drucker) ou Terrain des autres cadres (BSC / OKR Doerr / ITIL — test plus large) ; (b) **résorber la dette de référentiel résistance** (4 fiches à créer dont la cadre *Gestion du changement*) ; (c) **traiter la dette filets niveau** (§ 2) ; (d) **engager la décision 20.4 responsive** maintenant que 5 fiches MVP existent comme matière de test ; (e) **ouvrir la décision 20.6 trio alternatif v2**.
