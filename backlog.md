...
- **Prochaine action recommandée** : **jalon C.4 CLOS — jalon § 4.4 entièrement terminé**, les 5 fiches-questions sont intégrées au catalogue. **Décision Lætitia (20/05) maintenue : intercaler maintenant le mini-chantier éditorial du constat collatéral n°10** — créer dans le référentiel les fiches d'indicateurs de gestion du changement nommées par les cellules `CADRES_A_VENIR` de `resistance-transformation` + la fiche-cadre Gestion du changement. **Nouveau, à séquencer dans le même mini-chantier** : constat `26.c4.coll.1` — créer les 9 indicateurs de discipline de cascade MBO + la fiche-cadre Management par objectifs, pour adosser `cascade-objectifs` au référentiel (même logique que n°10). Puis **jalon D** (retrait des 5 mockups autonomes orphelins — le câblage des 5 tuiles d'accueil est déjà fait) et **jalon E** (tests E2E + smoke test global). **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord, **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**, **26.c1.coll.1 (dette NBSP module fiche-question)**, **26.c2.coll.1 (fiche-question candidate post-MVP)**, **26.c4.coll.1 (9 indicateurs de cascade MBO absents du référentiel)**, **constat collatéral n°10 (gestion du changement)**. **Demande différée de Lætitia** : rendre cliquables les noms d'indicateurs dans les pieds de colonne § 4.3 — à traiter après le mini-chantier n°10. **Friction beforeunload** : réglée doctrinalement. Aucun blocage actif. Tag de point de retour courant : `mvp-chantier-c4-livre`.
