...
Dernière mise à jour : **26 avril 2026 — chantier 9.D livré** (voie lente d'ajout au tableau de bord dans le tiroir de fiche : duo-cartes basculables, zone note pliée par défaut, re-render complet via #t-fiche). Tag `mvp-9d-voie-lente` sur `9959872`. La séance a aussi livré une refonte terminologique préalable (`6221002` : 12 libellés UI « panier » → « tableau de bord »).
...
- **Événement majeur de la séance** : **chantier 9.D livré** (voie lente d'ajout au tableau de bord dans le tiroir de fiche). Mockup-preview 3 options arbitré (`c7a86a3`), option B retenue (deux duo-cartes-statut basculables en pied du tiroir). Refonte terminologique préalable « panier » → « tableau de bord » sur 12 libellés UI (`6221002`, hors classes CSS et noms techniques). Implémentation initiale (`7c51646`) puis 4 correctifs en boucle de test interactif : (1) `0c26f46` clic carte = ajout immédiat (suppression de la phase de préparation visuelle, modèle mental aligné avec voie rapide 9.C) ; (2) `004581a` lecture de la fiche courante via DOM au lieu de CM.Hash ; (3) `aae08c3` note de contexte pliée par défaut + lien « ✎ Ajouter une note » + replaceChild ; (4) `56ac689` re-render complet de #t-fiche au lieu de mutation ciblée (les tentatives outerHTML / replaceChild ciblées ne propageaient pas les classes actif-place/actif-idee dans certains contextes d'ouverture) ; (5) `9959872` aide de la carte active reformulée en « Choix actuel. » (suppression de « cliquer ne change rien » qui portait à confusion).
- **Architecture finale 9.D** : `CM.Composants.rendreVoieLente(ind)` rend `<div id="voie-lente-tiroir" data-fiche-id="...">` avec deux états — pas-au-tableau-de-bord (2 cartes neutres) ou au-tableau-de-bord (cartes colorées + zone note pliée/dépliée). Zone note dépliée d'office si `ligne.note` non vide, sinon pliée avec lien « ✎ Ajouter une note » qui déplie + focus textarea. Handlers exposés : `voieLente_choisirStatut` (ajoute/bascule), `voieLente_deplierNote` (toggle classe `.deplie`), `voieLente_mettreAJourNote` (CM.Panier.modifierNote + feedback éphémère), `voieLente_retirer`, `voieLente_compteur` (live 0/500). Re-render via abonnement à CM.Panier : `_rafraichirVoieLenteSiOuverte(resultat)` skip 'modifie-note' (textarea = source de vérité), sinon `_renderVoieLenteDOM(ficheId)` qui réécrit `#t-fiche.innerHTML = rendreFicheHtml(ind)` — même mécanisme qu'`ouvrirTiroir`, fiable par construction.
- **Refonte terminologique** : 12 chaînes littérales touchées (tooltips voie rapide 9.C, recherche, en-tête vue TDB, invitation vide, carte TDB retrait, tooltip dynamique, confirm reset). Hors champ : noms d'identifiants (CM.Panier, _htmlPanierHeader, etc.), classes CSS (.panier-btn, .panier-tdb…), commentaires de code, terme métier « panier moyen » de af-vt1. La vue *Mon tableau de bord* qui était titrée « Mon panier d'indicateurs » s'aligne désormais sur le nom de l'onglet.
- **Workflow git sandbox** : pattern `mv .git/index.lock .git/index.lock.tmp$N` + `mv .git/HEAD.lock .git/HEAD.lock.tmp$N` toujours appliqué avant chaque commit ; les warnings `unable to unlink` persistent côté sandbox mais ne bloquent aucun commit (à nettoyer côté ordi en fin de séance si gênant).
- **Prochaines actions ouvertes (chantier 9 — tranches restantes)** :
  - **9.D voie lente** : ✅ **clos** — tag `mvp-9d-voie-lente` sur `9959872`.
  - **9.G garde-fou `beforeunload`** : confirmation si le tableau de bord n'est pas vide. Non démarré, court (≈ 30 lignes).
  - **9.E/F vue imprimable + export PDF** : one-pager A4 minimaliste arbitré sur `preview-panier-impression.html` (`494a4b9`). Non démarré, plus gros chantier.
...
- **Tag posé** : **`mvp-9d-voie-lente`** sur `9959872`. Chaîne complète : `mvp-chantier-14-livre` (matin 26/04) → `mvp-9c-voie-rapide` (45bdc62) → `mvp-9b4-recherche-globale` → **`mvp-9d-voie-lente`** (`9959872`, fin de séance).
- **Blocages / questions ouvertes** : aucun. Test interactif Lætitia validé sur le flow complet (clic carte → couleur + ajout, bascule, retrait, ouverture/fermeture du textarea, propagation TDB et icônes 9.C). Note pédagogique consignée comme nouvelle fiche feedback : pas de message d'inaction sur les contrôles UI.
