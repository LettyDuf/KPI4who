...
Dernière mise à jour : **26 avril 2026 — étape (c.2) livrée** (porte *Par mon cadre* migrée vers `CM.RequeteMetriques.executer` : `recommander` devient un traducteur orthodoxe. I2 du journal vert en interactif Safari : 13 fiches DORA × Opérationnel, *Fréquence de déploiement* en tête. Tag `mvp-etape-c2-porte-cadre-migree` posé sur `894ab8a`.)
...
- **Événement majeur de la séance** : **étape (c.2) du chantier 14 livrée — porte *Par mon cadre* migrée vers `CM.RequeteMetriques.executer`**. Commit unique atomique (`894ab8a` `recommander`). I2 du journal vert en interactif Safari (13 fiches DORA × Opérationnel, *Fréquence de déploiement* en tête). Tag `mvp-etape-c2-porte-cadre-migree` = `894ab8a`. `CM.DiagnosticCadre` n'accède plus à `CM.Referentiel` ni `CM.IndicateursMeta.cadresDe` directement — seuls les passe-plats structurels (`infoCadre`, `famillesOrdonnees`, etc.) restent. (c.2) tient en 1 commit au lieu de 2 (pas d'analogue à `aDeleguerPour` côté porte cadre ; le bloc *Cadres voisins* est hors périmètre car c'est de la proximité entre cadres, pas du filtrage de fiches).
- **Chantier actif** : **chantier 14 — refactor hexagonal**. Étapes (a), (b), (c.1), (c.2) closes. **Prochaine étape : (c.3) — porte *Par mon niveau*** (`CM.VuePorteNiveau` qui appelle aujourd'hui `CM.DiagnosticProbleme.recommander` ligne ~7859 — bénéficie déjà indirectement de la bascule, mais à reformuler proprement en cohérence avec 7.2a-code.3 qui est gelé derrière 14). Discipline inchangée : sentinelle = à choisir dans le journal (I6 actuellement en skip — peut nécessiter un I7+ documenté). Tag de clôture cible : `mvp-etape-c3-porte-niveau-migree`.
- **Doctrine traducteur héritée par (c.2)** : un traducteur **formule** le filtre, il ne post-filtre pas. Confirmée à (c.2) — la migration tient en une formulation `executer({niveau:[ctx.niveau], cadres:[ctx.cadre]})` sans aucun post-filtre côté traducteur. Tri par fiabilité conservé localement. Hérite à (c.3) sans débat.
- **Document central pour la reprise** : **`journal-invariants-pre-c.md`** (commit `9525f8e`). Quatre invariants figés sur HEAD `2a61336`. I1 ✓ post-(c.1). I2 ✓ post-(c.2). Reste à valider I3 exhaustif et I5 après chaque bascule de porte. Toute divergence ±1 → rollback à `5655b03`.
- **Test interactif côté Lætitia — état du journal** : I1 ✓ (post-(c.1)). I2 ✓ (post-(c.2), confirmé sur 13 fiches DORA × Opérationnel + *Fréquence de déploiement* en tête). I3 partiel (à exhausifier sur la liste complète des 13 fiches I2 — non bloquant). **I5 toujours incomplet** (titre de la première fiche manquant — à compléter au prochain passage). I4 et I6 en skip documenté ; I6 redeviendra pertinent au moment de (c.3) — peut nécessiter un I7+.
...
- **Dernier SHA de contenu** : `894ab8a` (c.2 commit unique). Précédent : `83d3bd2` (commit 2 de c.1). **Tags** : `baseline-avant-hexagonal` = `5655b03`, `mvp-etape-a-schema-inventorie` = `1ba5112`, `mvp-etape-b-coeur-extrait` = `2a61336`, `mvp-etape-c1-porte-probleme-migree` = `83d3bd2`, `mvp-etape-c2-porte-cadre-migree` = `894ab8a`. Prochain tag cible : `mvp-etape-c3-porte-niveau-migree`.
- **Prochaine action recommandée** : trois pistes selon énergie. (1) **démarrer (c.3) — porte *Par mon niveau*** ; cadrage préalable nécessaire (sentinelle à choisir, articulation avec 7.2a-code.3 à trancher : faut-il dégeler 7.2a-code.3 *avant* ou *pendant* (c.3) ?). (2) **compléter le journal** : titre première fiche I5 + liste complète des 13 fiches I2 pour I3 exhaustif (clos désormais I2 partiellement validé). (3) **rouvrir 6.9** seulement après livraison 14.
- **Blocages / questions ouvertes** : aucun côté contenu. Une question ouverte côté méthode : sentinelle pour (c.3) — I6 en skip car porte gelée ; à concevoir à l'ouverture du chantier (c.3). Côté workflow git sandbox : `.lock` orphelins continuent de s'accumuler à chaque commit (HEAD.lock, index.lock, tmp_obj_*) — nettoyage côté ordi à faire régulièrement (`rm -f .git/*.lock .git/objects/*/tmp_obj_* .git/objects/maintenance.lock` depuis le dossier projet). À noter pour (c.2) : un cycle de retry a été nécessaire (`.git/index.lock` recréé entre rm et commit) — résolu en évitant tout `git status` intermédiaire et en commitant via `-F /tmp/msg.txt`.
