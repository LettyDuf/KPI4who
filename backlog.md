...
Dernière mise à jour : **6 mai 2026 (séance journée — clôture, puis ré-ouverture pour audit visuel)** — décision 20.1 *Palette définitive* close en deux temps : (i) palette **B vert profond `#1f5945`** initialement retenue ; (ii) **bascule sur palette D bleu de Prusse `#1d3a5f`** le même jour après audit visuel cross-niveaux. Motif : B confondait avec 2 niveaux (équipe + programme), D ne confond qu'avec 1 (stratégique). Doctrine actualisée `doc-cadre-visuel.md` § 6.5 et `doc-catalogue-questions-fines.md` § 4.5. Mockup format en v4.2. Nouvelle dette détectée : filets niveau équipe/tactique sous-AA comme composants graphiques. 14 commits au total.
...
- **Événement majeur de la séance (06/05/2026 journée — clôture puis ré-ouverture)** : **décision 20.1 close en deux temps**. (i) Arbitrage initial sur 4 candidates côte à côte → **palette B vert profond éprouvé `#1f5945`** retenue ; D bleu de Prusse `#1d3a5f` en réserve documentée ; cuivre écarté. Doctrine § 6.5 et § 4.5 posée. Deux réglages post-arbitrage : matière en blanc franc, surlignage anti-pattern atténué (0.20 → 0.10). (ii) **Audit visuel cross-niveaux** mené à la reprise de session — production d'un mockup dédié `mockup-palette-fiche-question-niveaux.html` (4 niveaux × 2 palettes côte à côte). Constat de Lætitia à l'œil : B confond perceptivement avec **deux** niveaux (équipe + programme — deux teintes vertes), D ne confond qu'avec **un** seul (stratégique). **Bascule actée** : palette **D `#1d3a5f`** adoptée comme signature, palette **B `#1f5945`** rétrogradée en réserve documentée comme alternative chaude écartée. La doctrine du filet de sécurité posée le matin (« si bascule, conserver les autres tokens — seul l'accent change ») a joué à la lettre.
- **14 commits sur la journée** : matin (B retenue) — `72a7451` `f4f7dac` `26f5ccc` `63b62f1` `6abeaa3` `17d62f3` `74a6a13` `cb72570` `6375e33` `f53d4b5` ; après-midi (audit + bascule) — `b3d99fb` (mockup cross-niveaux) · `cb5a353` (bascule tokens B → D) · `575a64a` (cadre-visuel § 6.5 révisé) · `c64187c` (catalogue § 4.5 révisé) · `e89db68` (annotation mockup comparatif) · `3c6632a` (pied verdict mockup niveaux).
- **Mémoires touchées cette séance** : `project_palette_fiche_question_actee.md` créée le matin (B retenue) → à corriger pour refléter la bascule (D retenue, B en réserve).
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges · 20.3 volumétrie panels · 20.4 responsive double réponse · 20.5 rédaction MVP (1/8-12 fiches livrées) · 20.6 trio alternatif v2 · 20.7 fiche-question méta véracité/pastèque · 20.8 cross-link.
- **Dette de référentiel ouverte** : 4 fiches métriques à créer (First-pass yield × équipe ; Lead time × programme ; COPQ × stratégique ; Écart de vérité × stratégique).
- **Nouvelle dette détectée par l'audit AA cross-niveaux** : filets de niveau **équipe `#6fa89e` (2.70:1)** et **tactique `#c79373` (2.68:1)** sous-passent le seuil 3:1 WCAG SC 1.4.11 sur fond blanc/crème comme composants graphiques porteurs d'information. Programme `#7d9670` limite (3.25 sur blanc, 2.87 sur crème). Indépendant du chantier 20. Ouvert dans § 2 *UX à vérifier (harmonisation visuelle)*.
- **Décisions du chantier 10** : ✅ D1, D2 (V3), D5 fermées ; ⏳ D3 (mode fin), D4 (responsive), D6 (deep-linking).
- **Sentinelles** : non relancées cette séance (rédaction doctrinale + mockups, pas de code applicatif touché). À relancer avant tout commit code.
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→Q` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **rédiger une seconde fiche-question** d'un autre panel (20.5 — alimente le MVP) ; (b) **créer les 4 fiches métriques manquantes** ; (c) **rédiger la fiche-question méta véracité/pastèque (20.7)** ; (d) **traiter la dette filets niveau** détectée par l'audit AA (item ouvert en § 2). La palette est désormais verrouillée sur fondations testées.
