...
- **Événement majeur de la séance prolongée (8-9/05/2026, 13 commits `8e5a9b6` → `6a3ba46`)** : **chantier 23.f livré bout en bout (11 commits) + chantier 23.g ouvert avec 2 sous-commits éditoriaux**. **23.f** a retiré le legacy d'architecture rendu orphelin par l'accueil unifié : 4 portes (vue-porte-probleme/cadre/niveau, vue-par-question), pyramide, CM.Stepper, CM.DiagnosticCadre, toggle pyramide ↔ portes. `cadre-indicateurs.html` passé de 12 110 à 9 176 lignes (-24 %), plus 3 fichiers compagnons retirés. Voir mémoire `project_chantier_23f_livre.md` pour le détail. **23.g-1** (`8f746e7`) réécrit `scenario-non-regression.md` pour l'architecture post-23.f : §1-§10 actualisés (~38 pas exécutables couvrant accueil unifié, chips, cartouches, bandeau, panier, tiroir, hash URLs, console), §11 invariants I1-I6 posés en structure (valeurs à figer en 23.g-3). **23.g-2** (`6a3ba46`) supprime `journal-invariants-pre-c.md` (60 l. caduques — décrivait les invariants des 4 portes 14 retirées). Cohérent avec l'option C.
- **Chantier 23 — état d'avancement** : 23.a ✅ · 23.b ✅ · 23.c ✅ · 23.d ✅ · 23.e ✅ · 23.f ✅ · **23.g 🟡 en cours** (23.g-1 ✅ scénario réécrit · 23.g-2 ✅ journal supprimé · **23.g-3 🔴 sentinelles I1-I6 — smoke test interactif Lætitia requis** · 23.g-4 🔴 tests automatisés `CM.AccueilUnifie`).
- **Chantiers ouverts cumulés** : 21 (vue Lexique) · 13.bis (chips cumulables maturité) · 22 (harmonisation tutoiement) · **23 (refonte radicale accueil — sous-chantier 23.g en cours)** · 24 (URLs partageables des filtres, en attente trigger) · *Constat collatéral n°1 latent — sort de l'attribut `axes:[…]` Mintzberg* · *Constat collatéral n°2 latent — audit vocabulaire de l'accueil*.
...
- **État de l'outil après 23.g-1/-2** : aucun changement de comportement (modifications éditoriales pures). L'outil tourne nominalement (smoke test 23.f validé en fin de séance soir 8/05).
- **Mockups produits cette séance prolongée** : aucun (chantier éditorial).
- **Mémoires actualisées en clôture séance soir 8/05** : `project_accueil_unifie_v7.md` + nouvelle mémoire `project_chantier_23f_livre.md` + MEMORY.md indexé. Pas de mise à jour mémoire post 23.g-1/-2 (rien de doctrinal nouveau).
- **Workflow git sandbox** : pattern `mv .git/index.lock` toujours actif. ~13 paires `.git/*.lock.tmp` accumulées sur cette séance prolongée, s'ajoutent aux ~44 paires des séances précédentes. Plus ~10 fichiers `.todelete-23*` (backups + fichiers retirés via mv côté sandbox).
- **Prochaine action recommandée** : reprendre le sous-chantier **23.g-3 — sentinelles d'invariants I1-I6**. Smoke test interactif requis côté Lætitia : pour chaque invariant du §11 du scénario réécrit, capturer compteur N et premier id de la grille. Effort estimé ~1 h. Méthode : Lætitia ouvre `cadre-indicateurs.html` en session privée, exécute les 6 scénarios, me reporte les valeurs ; je crée `journal-invariants-23g.md` (ou autre nom) figeant les références. Ensuite, **23.g-4** : tests automatisés `CM.AccueilUnifie` sur le pattern `tests-requete-metriques.html` (effort ~2-3 h, code + test).
- **Décision préalable à prendre** : déposer un tag git `mvp-chantier-23-livre` sur `d03ccd0` (HEAD du chantier 23.f hors clôture docs) ? À arbitrer en ouverture de la prochaine séance.
