...
Dernière mise à jour : **4 mai 2026 — étape B du chantier 19 livrée, D2 tranchée**. Trois mockups-preview produits (accueil v2 `958042d`, fiche `e821662`, lentille `31f7c1b`) incarnant la posture des 8 axes. **D2 du chantier 10 tranchée à la lumière du mockup-lentille** (commit `64c8249`) : V3 retenue (filtre chips cumulables avec résultat dynamique) — argument structurant : extensibilité aux lentilles futures. Couplage D2+D4 relâché. Restent D3 (mode fin), D4 (responsive), D5 (pyramide), D6 (deep-linking) + 2 chantiers éditoriaux interdépendants.
...
- **Événement majeur de la séance (04/05/2026)** : **étape B du chantier 19 livrée + D2 du chantier 10 tranchée**. Trois mockups-preview produits, incarnant la posture des 8 axes : accueil v2 (`958042d` — 2 variantes avec/sans pyramide, icônes SVG line, filet de niveau saturé en haut), fiche d'indicateur (`e821662` — fiche t4 *Efficacité de flux du portefeuille*, posture confirme la densité existante, déplacements ciblés sur couronnement / chip type / signature voix), lentille *Par mes 4 axes* (`31f7c1b` — page nouvelle, 3 variantes V1/V2/V3 toutes incarnant la posture). **D2 tranchée par Lætitia** : V3 retenue (filtre chips cumulables) — argument structurant : extensibilité aux lentilles futures du § 4.4 (commit `64c8249`).
- **Posture retenue** *(rappel étape A, doctrine permanente)* : *« un outil artisan-sérieux-chaud, sobre par discipline, interactif par mission, situé par voix »*. 8 axes positionnés tels que recommandés par les panels (aucun déplacement de Lætitia).
- **Étapes du chantier 19** : ✅ A (posture globale) — close 04/05/2026 ; ✅ B (incarnation sur 3 pages-types) — livraison technique close 04/05/2026, **consolidation dans `doc-cadre-visuel.md` § 6 à faire** ; ⏳ C (retour au chantier 10 — D3, D4 restants) après consolidation.
- **Décisions du chantier 10** : ✅ D1 tranchée 02/05 ; ✅ D2 tranchée 04/05 (V3) ; ouvertes — D3 (mode fin), D4 (responsive, découplé de D2), D5 (pyramide), D6 (deep-linking additif). Plus 2 chantiers éditoriaux interdépendants (refonte accueil, catalogue de questions fines).
...
- **Sentinelles** : non relancées cette séance (mockups-preview HTML autonomes + docs, pas de code touché). À relancer avant tout commit code.
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Warnings `unable to unlink` persistent sans bloquer. 4 `.git/*.lock.tmp*` créés cette séance.
- **Prochaines actions ouvertes** : (1) **Consolidation étape B** — rédiger § 6 de `doc-cadre-visuel.md` avec les décisions d'incarnation des 3 pages-types (accueil, fiche, lentille). (2) **Étape C du chantier 19 / reprise chantier 10** — D3 (mode fin, après catalogue de questions), D4 (responsive seul), D5 (pyramide), D6 (deep-linking). (3) Chantier éditorial catalogue de questions fines (panels d'experts nommés). (4) Constats restants 1.ter (n°8, n°10) et 1.bis axes restants en sommeil. (5) Items 6.9, 12-17, 18 en sommeil.
