...
Dernière mise à jour : **25 avril 2026 — étape (c.1) livrée** (porte *Par mon problème* migrée vers `CM.RequeteMetriques.executer` : `recommander` + `aDeleguerPour` deviennent des traducteurs orthodoxes. I1 du journal vert en interactif Safari : 1 reco + 6 délégation. Tag `mvp-etape-c1-porte-probleme-migree` posé sur `83d3bd2`.)
...
- **Événement majeur de la séance** : **étape (c.1) du chantier 14 livrée — porte *Par mon problème* migrée vers `CM.RequeteMetriques.executer`**. Deux commits atomiques (`ee5361b` `recommander`, `83d3bd2` `aDeleguerPour`). I1 du journal vert en interactif Safari (1 reco *Taux d'avancement des jalons* + 6 délégation). Tag `mvp-etape-c1-porte-probleme-migree` = `83d3bd2`. `CM.DiagnosticProbleme` n'accède plus à `CM.Referentiel` ni `CM.IndicateursMeta` directement.
- **Chantier actif** : **chantier 14 — refactor hexagonal**. Étapes (a), (b), (c.1) closes. **Prochaine étape : (c.2) — porte *Par mon cadre*** (`CM.DiagnosticCadre.recommander` en façade orthodoxe vers `executer({niveau:[...], cadres:[...]})`). Discipline inchangée : sentinelle = I2 du journal, journal d'invariants vert avant tag, jamais en parallèle. Tag de clôture cible : `mvp-etape-c2-porte-cadre-migree`.
- **Doctrine traducteur posée à (c.1)** : un traducteur **formule** le filtre, il ne post-filtre pas. Variante orthodoxe (calcul du complément de niveau dans `aDeleguerPour`) retenue contre la façade simple à post-filtre — pour que (c.2) et (c.3) héritent de la même règle sans débat.
- **Document central pour la reprise** : **`journal-invariants-pre-c.md`** (commit `9525f8e`). Quatre invariants figés sur HEAD `2a61336`. I1 ✓ post-(c.1). Reste à valider I2, I3, I5 après chaque bascule de porte. Toute divergence ±1 → rollback à `5655b03`.
- **Test interactif côté Lætitia — état du journal** : I1 ✓ (post-(c.1)). I2 (DORA × Opérationnel = 13, première = `Fréquence de déploiement`) à reconfirmer après (c.2). I3 partiel. **I5 toujours incomplet** (titre de la première fiche manquant — à compléter au prochain passage). I4 et I6 en skip documenté.
...
- **Dernier SHA de contenu** : `83d3bd2` (commit 2 de c.1). Précédent : `ee5361b` (commit 1 de c.1). **Tags** : `baseline-avant-hexagonal` = `5655b03`, `mvp-etape-a-schema-inventorie` = `1ba5112`, `mvp-etape-b-coeur-extrait` = `2a61336`, `mvp-etape-c1-porte-probleme-migree` = `83d3bd2`. Prochain tag cible : `mvp-etape-c2-porte-cadre-migree`.
- **Prochaine action recommandée** : trois pistes selon énergie. (1) **démarrer (c.2) — porte *Par mon cadre*** ; sentinelle = I2 du journal ; même découpage en 2 commits (recommander en façade puis tag). (2) **compléter le journal** : titre première fiche I5 + liste complète des 13 fiches I2 pour I3 exhaustif (utile avant (c.2) car I2 sera la sentinelle). (3) **rouvrir 6.9** seulement après livraison 14.
- **Blocages / questions ouvertes** : aucun côté contenu. Côté workflow git sandbox : `.lock` orphelins continuent de s'accumuler à chaque commit (HEAD.lock, index.lock, tmp_obj_*) — nettoyage côté ordi à faire régulièrement (`rm -f .git/*.lock .git/objects/*/tmp_obj_* .git/objects/maintenance.lock` depuis le dossier projet). Un orphelin sandbox également : `cadre-indicateurs.html.test-write` (vide, créé pour tester l'écriture, ne peut pas être rm depuis la sandbox).
