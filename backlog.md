...
- **Test interactif côté Lætitia en attente** : (1) ouvrir `tests-requete-metriques.html` dans Safari — le rapport doit afficher *✓ 63 tests — tous verts*. (2) ouvrir `cadre-indicateurs.html` et jouer `scenario-non-regression.md` (~30 clics, ~10 min) pour confirmer l'iso-comportement avant de démarrer l'étape (c). Si régression détectée : mot-clé `rollback à l'étape b` (point de retour `5655b03` = `baseline-avant-hexagonal`).
