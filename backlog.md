...
| ~~6.7 Porte « Par mon cadre »~~ | ~~Nouveau module `CM.VuePorteCadre` parallèle à `CM.VuePorteProbleme`~~ | ✅ Livré 20/04/2026 (cinq commits : `ccbd1e7` planification + `4842231` feat porte complète + `ce39360` description des cadres + `680f492`+`2988e07` fixes accents). Deux modules créés : `CM.DiagnosticCadre` (logique métier pure, délègue les niveaux à `CM.DiagnosticProbleme`) et `CM.VuePorteCadre` (contrôleur de vue). Référentiel enrichi : champ `famille` sur `CADRES`, nouvel objet `FAMILLES` (7 écoles de pensée) + `ORDRE_FAMILLES` + `CADRES_A_VENIR` (10 cadres projetés). API étendue : `infoFamille`, `familleDe`, `famillesOrdonnees`, `cadresParFamille`, `cadresAVenirParFamille`. Stepper 3 étapes (cadre → niveau → indicateurs) avec accordéon `<details>`/`<summary>` natif en étape 1. Palette UX 7 familles validée avec l'expert UX (bordure gauche 4px via `::before` + fond ≥ 94% luminance, chevron SVG 1.75rem renforcé). |
| ~~6.7a Cadres voisins dans le référentiel~~ | ~~Fonction `CM.IndicateursMeta.cadresProchesDe(cadreId, options)` + bloc en pied de résultats dans `CM.VuePorteCadre._etapeResultats` + fonction `pivoterVersCadre` distincte de `setCadre`~~ | ✅ Livré 20/04/2026 (commit `6cdb0c4`, 207 insertions / 6 suppressions). Signature par objet options (`{ n, seuilMin, exclure }`) — extensible sans casser l'existant. Algorithme : co-occurrence pure sur `META.cadres`, tri décroissant, filtrage par seuil, exclusion `generique`. Rendu via `_htmlBlocVoisins(cadreId)` — tuiles cliquables (icône + label + description + compteur de fiches partagées) sur un fond gradient niv-1-clair→fond. Pivot préserve `etat.niveau` et `etat.etape=3` (contraste avec `setCadre` qui relance à l'étape 2). Styles CSS scopés `#vue-porte-cadre` pour éviter les fuites. |
