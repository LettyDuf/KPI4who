...
- **Événement majeur de la séance nuit (08/05/2026, 7 commits `03eae05` → `127f592`)** : **chantier 23.e livré bout en bout** — la liste filtrée 2 colonnes est en place, l'accueil unifié est désormais **visible et utilisable**. Six fixes UX issus du test interactif côté Lætitia, livrés en commits atomiques : `03eae05` (feature), `e1dd34b` (lien orphelin), `a6dda4f` (740→920px), `8b67d07` (ligne bleue), `a4fffc9` (color carte-nom), `0173f20` (compteur tableau de bord), `127f592` (en-tête épuré).
- **Méthodologie validée** : tous les fixes UX sauf `e1dd34b` ont la même racine — *quand on fait passer un composant existant dans un nouveau contexte (l'accueil unifié), des hypothèses tacites sur l'environnement parent (couleur héritée, marges en cascade, largeur disponible) sont brisées*. Le test interactif est essentiel ; aucun de ces 4 bugs n'aurait été détecté sans Lætitia regardant la page rendue. Pattern à inscrire en doctrine craftsmanship.
- **Chantier 23 — état d'avancement** : 23.a ✅ · 23.b ✅ · 23.c ✅ · 23.d ✅ · **23.e ✅** · **23.f 🔴 prochain (suppression code orphelin : `vue-porte-probleme`/`vue-porte-cadre`/`vue-porte-niveau`, `vue-par-question`, pyramide, orchestrateur stepper du chantier 14 — le traducteur orthodoxe reste)** · 23.g (tests E2E + sentinelles + scénario de non-régression).
- **Chantiers ouverts cumulés** : 21 (vue Lexique) · 13.bis (chips cumulables maturité) · 22 (harmonisation tutoiement) · **23 (refonte radicale accueil — sous-chantiers 23.f et 23.g restants)** · 24 (URLs partageables des filtres, en attente trigger) · *Constat collatéral n°1 latent — sort de l'attribut `axes:[…]` Mintzberg* · *Constat collatéral n°2 latent — audit vocabulaire de l'accueil*.
- **Points reportés cette séance, à reprendre en commits dédiés** :
  1. **Friction garde-fou beforeunload** sur clic fiche-question — Safari demande « Voulez-vous vraiment quitter cette page ? » dès que le panier n'est pas vide. Quatre options identifiées (target="_blank" ~5min sûr / modale interne ~chantier / garde-fou conditionnel ~15min mais dangereux / LocalStorageDepot ~2-3h chantier qui résout la cause racine). Lætitia tranchera.
  2. **Mission de l'accueil unifié** — l'en-tête est épuré (h2 seul) ; un élément de contexte/mission viendra au-dessus du h2 dans un commit dédié. Note dans le commit `127f592`.
- **État de l'outil aujourd'hui** : la page d'accueil unifiée est **complètement opérationnelle**. Chips (sélecteurs accordéon, sélection persistée, fermeture en mode actif) ✅. Cartouche question dépliée (index inline des 6 fiches-questions cliquables) ✅. Cartouche compteur (libellé live `N sur 89 indicateurs`, libellé descriptif des lentilles, panneau dépliant avec liste 2 colonnes peuplée par `htmlCarte`, état vide pédagogique, ordre canonique du référentiel) ✅. Bandeau du haut avec compteur d'indices live sur *Mon tableau de bord* ✅. Largeur stable à 920px. La pyramide n'a plus de lien dans l'UI mais reste accessible via `#pyramide` jusqu'au 23.f. Les vues détail des 3 portes (`vue-porte-probleme`/`cadre`/`niveau`) et `vue-par-question` sont également orphelines, à supprimer en 23.f.
- **Mockups produits cette séance** : `mockup-23e-largeur.html` (preview d'arbitrage 740/880/920 avec 2 cartes DORA réelles, conservé pour traçabilité de la décision visuelle).
- **Mémoires à actualiser** : `project_accueil_unifie_v7.md` (largeur 740 → 920 ; en-tête réduit au h2 seul, mission à réintroduire plus tard ; compteur live sur Mon tableau de bord). Pas fait dans cette séance, à inscrire en début de la prochaine.
- **Workflow git sandbox** : pattern `mv .git/index.lock` (et `.git/HEAD.lock`) toujours actif. ~7 paires `.git/*.lock.tmpA → tmpM` accumulées sur cette séance, s'ajoutent aux ~37 paires des séances précédentes — à nettoyer côté ordi quand l'occasion se présentera.
- **Prochaine action recommandée** : ouvrir le sous-chantier **23.f — suppression du code orphelin**. Effort estimé 1-2h. Avant de toucher quoi que ce soit, croiser les recherches `vue-porte-probleme`, `vue-porte-cadre`, `vue-porte-niveau`, `vue-par-question`, `pyramide`, `CM.Stepper` pour identifier toutes les zones à retirer. Le **traducteur orthodoxe `CM.RequeteMetriques` reste** — c'est lui qui formule le filtre depuis l'accueil. Doctrine `project_doctrine_traducteur_orthodoxe.md` préservée.
