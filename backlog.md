...
Dernière mise à jour : **6 mai 2026 (séance journée — clôture)** — décision 20.1 *Palette définitive* close + 2 réglages post-arbitrage (matière en blanc franc, surlignage anti-pattern atténué). Doctrine étendue dans `doc-cadre-visuel.md` § 6.5 et `doc-catalogue-questions-fines.md` § 4.5. Mockup format passé en v4.1. 9 commits.
...
- **Événement majeur de la séance (06/05/2026 journée)** : **décision 20.1 close** — palette de la fiche-question arbitrée sur 4 candidates côte à côte. Lætitia retient **B (vert profond éprouvé `#1f5945`)** ; **D (bleu de Prusse `#1d3a5f`)** en réserve documentée. Cuivre `#a14e1f` écarté. **Doctrine étendue** : `doc-cadre-visuel.md` § 6.5 *Fiche-question (mode Par ma question)* — 4e page-type incarnée, division du travail chromatique (indigo = chrome universel, vert profond = signature spécifique fiche-question). Deux réglages post-arbitrage : matière en blanc franc (correction beige-sur-beige), surlignage anti-pattern atténué (opacité 0.20 → 0.10).
- **9 commits de la séance** : `72a7451` (cadre-visuel § 6.5) · `f4f7dac` (catalogue § 4.5 + décision #1 close) · `26f5ccc` (mockup format-fiche-question v4 — palette B propagée) · `63b62f1` (mockup palette comparatif — trace d'arbitrage 4 candidates) · `6abeaa3` (backlog 20.1 close + État courant) · `17d62f3` (fix v4.1 — matière en blanc franc) · `74a6a13` (fix — surlignage atténué) · `cb72570` (doc catalogue § 4.5 — précision opacité) · `6375e33` (mini-preview surlignage — trace d'arbitrage 3 versions).
- **Mémoire créée cette séance** : `project_palette_fiche_question_actee.md` — palette B retenue + D en réserve + division du travail chromatique posée comme règle permanente.
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires de `.git/*.lock.tmp.A→J` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **vérification finale palette B** *(tâche #6 ouverte du plan initial)* — audit accessibilité AA des contrastes vert profond `#1f5945` sur blanc/crème + cohérence cross-niveaux (4 niveaux à tester, surtout programme `#7d9670` désaturé qui pourrait télescoper le vert profond et déclencher la bascule vers D bleu de Prusse) ; (b) **rédiger une seconde fiche-question** d'un autre panel ; (c) **créer les 4 fiches métriques manquantes** ; (d) **rédiger la fiche-question méta véracité/pastèque (20.7)**.
