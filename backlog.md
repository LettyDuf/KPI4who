...
- **Points reportés cumulés (à reprendre en commits dédiés)** :
  ...
  4. **Double libellé pour combinaison vide** (constat C3 du journal 23.g-3) — compteur dit « Aucun indicateur ne combine ces lentilles. » et panneau dit « Aucune fiche ne correspond à cette combinaison d'axes. ». Deux registres lexicaux (indicateur/fiche, combine/correspond, lentilles/axes). **Arbitrage Lætitia 09/05 : oubli d'harmonisation, à corriger** — un seul libellé à retenir et à appliquer aux deux endroits, choix éditorial à faire en ouverture du commit.
...
- **Prochaine action recommandée** : ouvrir le sous-chantier **23.g-4 — tests automatisés `CM.AccueilUnifie`** (différé en clôture séance 09/05, jalon B). Pattern de référence : `tests-requete-metriques.html` (suite de tests JS pure, ouverture navigateur, assertions visibles). Effort estimé ~2-3 h. Couverture cible : les 6 invariants du journal 23.g-3 (I1-I6) reproduits en assertions automatiques + cas adjacents (changement de chips successifs, état restauré au montage, libellé d'axes actifs). À discuter en ouverture : faut-il fusionner avec un fichier de tests existant ou créer `tests-accueil-unifie.html` dédié ? **En préalable** : régler le point reporté n°4 (harmonisation des deux libellés vide), parce que le test des invariants vides (I4, I6) doit pointer sur le libellé canonique à retenir.
