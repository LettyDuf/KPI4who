...
- **Événement majeur de la séance soir (08/05/2026, 11 commits `8e5a9b6` → `d03ccd0`)** : **chantier 23.f livré bout en bout** — suppression du legacy d'architecture rendu orphelin par l'accueil unifié. Onze sous-commits atomiques : `8e5a9b6` (vue-porte-probleme −497 l.), `f2ccfcc` (vue-porte-cadre −653 l.), `8a70e81` (vue-porte-niveau + DiagnosticCadre −892 l.), `abfa3e1` (vue-par-question −251 l.), `b6fe19a` (pyramide −252 l.), `1b6adcb` (CM.Stepper + 3 fichiers compagnons −7914 l.), `e6eced1` (sélecteur CSS −8 l.), `37a2a20` (demanderRetourAccueil −19 l.), `7963402` (toggle accueil démantelé), `475c067` (commentaires reformulés), `d03ccd0` (doc-contrats-chantier-14 allégé option C). **`cadre-indicateurs.html` passé de 12 110 à 9 176 lignes (-24 %)**. Plus 3 fichiers compagnons retirés (`tests-porte-niveau.html`, `outils/construire-tests-porte-niveau.js`, `doc-contrats-stepper-roles.md`).
- **Doctrine d'ordre appliquée** : un module pivot tombe **après** tous ses consommateurs. Posée pour CM.DiagnosticCadre (consommé par 2 portes) et CM.Stepper (consommé par 3 façades). Cartographie systématique avant chaque sous-commit, ajustement du plan en cours de chantier (CM.DiagnosticCadre déplacé de 23.f-2 à 23.f-3). Smoke test interactif côté Lætitia validé fin de séance — aucune régression.
- **Chantier 23 — état d'avancement** : 23.a ✅ · 23.b ✅ · 23.c ✅ · 23.d ✅ · 23.e ✅ · **23.f ✅** · **23.g 🔴 prochain (tests E2E + sentinelles + scénario de non-régression)**.
- **Chantiers ouverts cumulés** : 21 (vue Lexique) · 13.bis (chips cumulables maturité) · 22 (harmonisation tutoiement) · **23 (refonte radicale accueil — sous-chantier 23.g restant)** · 24 (URLs partageables des filtres, en attente trigger) · *Constat collatéral n°1 latent — sort de l'attribut `axes:[…]` Mintzberg* · *Constat collatéral n°2 latent — audit vocabulaire de l'accueil*.
- **Points reportés cumulés (à reprendre en commits dédiés)** :
  1. **Friction garde-fou beforeunload** sur clic fiche-question (depuis 23.e) — Safari demande « Voulez-vous vraiment quitter cette page ? » dès que le panier n'est pas vide. Quatre options identifiées (target="_blank" ~5min sûr / modale interne ~chantier / garde-fou conditionnel ~15min mais dangereux / LocalStorageDepot ~2-3h chantier qui résout la cause racine). Lætitia tranchera.
  2. **Mission de l'accueil unifié** (depuis 23.e) — l'en-tête est épuré (h2 seul) ; un élément de contexte/mission viendra au-dessus du h2 dans un commit dédié. Note dans le commit `127f592`.
- **État de l'outil après 23.f** : la page d'accueil unifiée est **complètement opérationnelle**, le legacy d'architecture est nettoyé. Architecture vivante : `CM.AccueilUnifie` (consommateur unique de `CM.RequeteMetriques.executer`), `CM.Panier`, `CM.DiagnosticProbleme` (réduit à un fournisseur de métadonnées), `CM.Composants.htmlCarte`, `CM.FicheViewModel`, `CM.IndicateursMeta`, `CM.Referentiel`, `CM.Config`, `CM.Roles`. Modules retirés : 4 façades CM.VuePorte*, CM.Stepper, CM.DiagnosticCadre, CM.App.basculerAccueil, CM.App.entrer. Toggle pyramide ↔ portes démantelé : un seul accueil possible désormais (l'id DOM `accueil-portes` est conservé pour rétro-compat). Hash `#portes` reconnu, `#pyramide` ignoré.
- **Mockups produits cette séance** : aucun (chantier de suppression pure).
- **Mémoires actualisées en clôture** : `project_accueil_unifie_v7.md` (passage de « ce qui meurt » à « ce qui est mort » avec liens vers 23.f) ; nouvelle mémoire `project_chantier_23f_livre.md` posée. MEMORY.md indexé.
- **Workflow git sandbox** : pattern `mv .git/index.lock` toujours actif. ~11 paires `.git/*.lock.tmpA → tmpG5` accumulées sur cette séance, s'ajoutent aux ~44 paires des séances précédentes — à nettoyer côté ordi quand l'occasion se présentera. Plus ~9 fichiers `.todelete-23f1` et `.todelete-23f6` (les backups + les 3 fichiers retirés via mv côté sandbox).
- **Prochaine action recommandée** : ouvrir le sous-chantier **23.g — tests E2E + sentinelles + scénario de non-régression**. Effort à estimer en ouverture de séance. Couvre : (i) sentinelles d'invariants pour l'accueil unifié (type `journal-invariants-pre-c.md` posé pour les portes 14), (ii) scénario de non-régression actualisé qui ne référence plus les portes retirées, (iii) tests automatisés via le patron Node.js (cf. `tests-requete-metriques.html` et `tests-panier.html`). Décision préalable : déposer un tag git `mvp-chantier-23-livre` sur `d03ccd0` ? À arbitrer en début de séance.
