...
Dernière mise à jour : **30 avril 2026 — chantier 1.ter, 4 cadres clos** (`safe`/`lean`/`itil`/`dora`) — 45 tags abusifs corrigés, taxonomie étendue (3 cadres ajoutés : `securite`, `cd`, `space` + famille `risque-securite`). Plus tôt même séance : axe Stratégique de 1.bis clos (`ba690cb`). Veille (27/04/2026) : dette `tests-porte-niveau.html` éteinte, tag `mvp-dette-tests-porte-niveau-eteinte` sur `bc3042d` ; chantier 9.F clos, tag `mvp-9f-libelle-pdf` sur `e8649e9`.
...
- **Événements majeurs de la séance (3)** : **(1) Axe Stratégique de 1.bis clos** — 1 phrase corrigée sur `s9` (term-def `COSO ERM` + `ISO 31000`, commit `ba690cb`). Méthode d'audit corrigée actée (vérifier l'inclusion dans un span term-def, pas seulement compter les occurrences brutes — diagnostic initial surévaluait la dette d'un facteur ~4). **(2) Chantier 1.ter ouvert** (`848e307`) — audit légitimité des tags `cadres` après remontée Lætitia que des fiches taggées `safe` ne relevaient pas du corpus. Périmètre : 10 cadres, 85 fiches mappées, 144 occurrences. Méthode d'audit standardisée écrite. **(3) 4 cadres clos sur 1.ter** : `safe` (5✅/6❌, t4 enrichi avec Flow Metrics SAFe v6), `lean` (14✅/15❌), `itil` (10✅/6❌), `dora` (4✅/9❌). **45 occurrences abusives corrigées au total.** **Extension de taxonomie** (commit `662682a`) : 3 cadres ajoutés (`securite` NIST CSF/ISO 27035, `cd` Continuous Delivery Humble-Farley, `space` Forsgren et al. — `space` déplacé de `CADRES_A_VENIR`) + nouvelle famille `risque-securite` (8 familles). 13 fiches retaggées avec rigueur stricte (corpus signature documenté). Constats collatéraux n°2 (cadre sécurité) et n°3 (cadre CD/SPACE) ADRESSÉS dans la même session.
- **Apprentissages du jour (2)** : **(1)** un diagnostic « par parsing brut » sans isoler les attributs HTML (`data-def="..."`) ni distinguer « occurrence couverte par term-def » de « occurrence libre » donne une dette gonflée. Toujours croiser les chiffres bruts avec un test d'inclusion. **(2)** Tagger un cadre par contiguïté (« niveau Tactique → SAFe par habitude ») sans appui doctrinal produit du faux signal massif (50–69 % de dette selon les cadres audités). Toute pose de tag exige un marqueur lexical du corpus OU une métrique signature documentée — sinon : pas de tag.
- **Validations par Lætitia** : reprise mot-à-mot des définitions s10 sur s9 ; blocs A/B/C des cadres `safe`/`lean`/`itil`/`dora` validés en bloc ou avec mes avis nuancés ; nouvelle famille `risque-securite` créée plutôt que rangement dans `observabilite` ; 3 cadres et leurs scopes validés ; 13 re-attributions strictes validées.
- **Workflow git sandbox** : pattern `mv .git/index.lock` + `mv .git/HEAD.lock` toujours appliqué. Warnings `unable to unlink` persistent sans bloquer. ~14 fichiers `.git/*.lock.tmp*` accumulés cette session.
- **Prochaines actions ouvertes** : (1) **chantier 1.ter cadres restants** : `scrum` (6), `kanban` (5), `mbo` (4), `bsc` (3), `okr` (3), `generique` (54, audit-éclair). (2) **1.bis axes restants** — Tactique (LPM, DORA, ROI), Programme (CRM, probable faux positif), Op TI, Op Affaires. (3) Constat collatéral n°1 (52 fiches hors taxonomie `CADRES_META`, dont s9 cybersécurité non mappée) à statuer. (4) Chantier 10 (gelé à `5655b03`) peut reprendre. (5) Items 6.9, 18, 12-17 en sommeil.
- **Tag posé** : aucun (modifs éditoriales ponctuelles, pas de jalon MVP). Dernière référence taggée : `mvp-dette-tests-porte-niveau-eteinte` sur `bc3042d`.
- **Blocages / questions ouvertes** : aucun. Fichiers `.todelete` toujours en attente côté ordi.
...
**Constat collatéral n°6** *(remonté lors de l'audit cadre `bsc` le 30/04/2026, à statuer hors 1.ter)*. Divergence existante entre `CADRES{}` (taxonomie active) et `VOCAB.cadres` (validateur de filtres) : depuis le commit `662682a` (extension taxonomie `securite` / `cd` / `space` lors de l'audit `dora`), les 3 cadres ajoutés à `CADRES{}` n'ont **pas été ajoutés à `VOCAB.cadres`**. Conséquence potentielle : un appel à filtrer sur `cd`, `space` ou `securite` lèverait probablement une erreur de validation. Audit à mener pour vérifier l'impact réel (le validateur `validerClauseSet` est-il appelé ? sur quels chemins ?). Le retrait `bsc` de ce commit a été appliqué proprement aux deux endroits (cohérence du modèle restaurée pour bsc).
