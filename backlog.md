...
- **Prochaine action recommandée** : **mini-chantier éditorial du constat n°10 ENTIÈREMENT BOUCLÉ** — volet gestion du changement (cadre `change` + 6 fiches `cg-*`, tag `mvp-cadre-changement-livre`) et volet MBO (cadre `mbo` + 9 fiches `mbo-*`, tag `mvp-cadre-mbo-livre`) livrés ; `resistance-transformation` et `cascade-objectifs` sont désormais entièrement adossées au référentiel (104 fiches). **Prochaine étape : jalon D** — retrait des 5 mockups autonomes de fiches-questions orphelins (conservés exprès lors du ménage du 23/05) et nettoyage des références. Puis **jalon E** (tests E2E + smoke test global). **Point transverse ouvert** : brancher les liens sortants des fiches-questions vers une vue d'ensemble de leur cadre (`resistance-transformation`, `cascade-objectifs`). **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord, **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**, **26.c1.coll.1 (dette NBSP module fiche-question)**, **26.c2.coll.1 (fiche-question candidate post-MVP)**, **26.c4.coll.2 (crossLink cascade ↔ pasteque muet)**. **Demande différée de Lætitia** : rendre cliquables les noms d'indicateurs dans les pieds de colonne § 4.3. Aucun blocage actif. Tag de point de retour courant : `mvp-cadre-mbo-livre`.
