...
- **Événement majeur de la séance soir (08/05/2026, 7 commits `5ebb0d7` → `696e589`)** : **chantier 23.d livré bout en bout** — cartouches cliquables, compteur live branché au traducteur, index inline des 6 fiches-questions, largeur stable. L'accueil est désormais **filtrant** (le N change avec les chips actives) ; il manque encore la liste 2 colonnes (23.e) pour rendre les fiches sélectionnées visibles.
  - **23.d-mockup α vs β (2 commits)** : préview côte à côte avec les 6 entrées réelles (`5ebb0d7`) ; finalisation β après arbitrage Lætitia (emojis + sans sous-titres, `bd29219`).
  - **23.d-0 coquille** (`655376a`) : `5` → `6` questions terrain dans le squelette HTML (un commit dédié pour atomicité).
  - **23.d-1 cliquabilité** (`f5cd431`) : délégation de clic, abonnement `surChangementCartouche`, synchro DOM (aria-expanded, classe `.deplie`, libellé Voir/Masquer, chevron ▾/▴), panneaux dépliables repérés par `data-cartouche-panneau`.
  - **23.d-2 compteur live** (`a6ad178`) : `appliquerFiltre()` invoque désormais `CM.RequeteMetriques.executer` ; mapping état chips → filtre canonique (niveau/maturiteMin/tags/cadres) ; libellé `.axes-actifs` en voix française naturelle pour 0/1/2/3+ lentilles actives ; total absolu mis en cache au montage.
  - **23.d-3 index inline** (`14162ee`) : panneau de la cartouche question peuplé statiquement avec les 6 fiches livrées (set d'emojis validé : 🍉 📅 🔄 🚧 🪜 🪞) ; liens vers les mockups HTML autonomes existants ; CSS scopé `.accueil-unifie .panneau-cartouche .liste-questions-inline`.
  - **23.d-fix1 largeur stable** (`696e589`) : bug remonté par Lætitia au test interactif — `.accueil-unifie` prenait la largeur intrinsèque du contenu le plus large parce que `#accueil-portes` est `flex column align-items:center` ; fix `width:100%` borné par `max-width:740px`.
- **Arbitrages tranchés cette séance (3 décisions clés)** :
  1. **Variante β retenue** — index inline des 6 fiches-questions dans le panneau dépliant, plutôt qu'un lien sortant vers `vue-par-question`. Cohérent avec la doctrine v7 « l'accueil est l'outil » et permet de supprimer `vue-par-question` au 23.f.
  2. **Set d'emojis validé** sans collision avec les axes/cadres de l'accueil (🍉 pasteque, 📅 hebdomadaire, 🔄 amélioration continue, 🚧 résistance, 🪜 cascade, 🪞 miroir). Cohérent avec les sélecteurs des chips qui exposent eux-mêmes les emojis du référentiel — la doctrine § 6.0 « SVG line monochromes partout » reste la cible globale, sa migration est un chantier séparé.
  3. **Q1 89 vs 75 indicateurs — option (i)** : on assume les 89 du référentiel réel, le `75` du mockup et de la doctrine v7 était un placeholder. `doc-cadre-visuel.md § 6.1` et la mémoire `project_accueil_unifie_v7.md` mises à jour en conséquence.
- **Chantier 23 — état d'avancement** : 23.a ✅ · 23.b ✅ · 23.c ✅ · **23.d ✅** · **23.e 🔴 prochain (liste filtrée 2 colonnes — peuplement de `<div id="panneau-cartouche-compteur" class="liste-fiches">` via `htmlCarte()` à partir des `fiches: ID[]` du payload `filtre-change`)** · 23.f (suppression code orphelin : `vue-porte-probleme`/`vue-porte-cadre`/`vue-porte-niveau`, `vue-par-question`, pyramide, orchestrateur stepper du chantier 14 — le traducteur orthodoxe reste) · 23.g (tests E2E + sentinelles).
- **Chantiers ouverts cumulés** : 21 (vue Lexique) · 13.bis (chips cumulables maturité) · 22 (harmonisation tutoiement) · **23 (refonte radicale accueil — sous-chantiers 23.e à 23.g restants)** · 24 (URLs partageables des filtres, en attente trigger) · *Constat collatéral n°1 latent — sort de l'attribut `axes:[…]` Mintzberg* · *Constat collatéral n°2 latent — audit vocabulaire de l'accueil*.
- **État de l'outil aujourd'hui** : la page d'accueil unifiée est **visible, interactive et filtrante**. Les chips ouvrent leur sélecteur, la sélection persiste en localStorage, la chip se referme en mode actif. La cartouche question dépliée révèle l'index des 6 fiches-questions cliquables. La cartouche compteur affiche `89 sur 89 indicateurs` et descend en temps réel selon les chips actives, avec libellé descriptif des lentilles. Mais la cartouche compteur dépliée ouvre encore un panneau **vide** (la liste 2 colonnes arrive en 23.e). L'accueil 5 portes a disparu, la pyramide n'a plus de lien dans l'UI mais reste accessible via `#pyramide` jusqu'au 23.f.
- **Mémoires actualisées cette séance** : `project_accueil_unifie_v7.md` (5 → 6 questions terrain, 75 → 89 indicateurs avec note sur le placeholder).
- **Workflow git sandbox** : pattern `mv .git/index.lock` (et `.git/HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.aa → ag` accumulées sur cette séance (~7 paires), s'ajoutent aux ~30 paires des séances précédentes — à nettoyer côté ordi.
- **Prochaine action recommandée** : ouvrir le sous-chantier **23.d — cartouches cliquables avec compteur live**. Brancher l'écouteur de clic sur les deux cartouches (*Par ma question* et le compteur) pour ouvrir/fermer ; le compteur live se met à jour via `surChangementFiltre` (déjà en place). Effort estimé 1-2h.
- **Blocages / questions ouvertes** : aucun. Le vocabulaire de l'accueil (*Flux*, *Alignement*, *Maturité*) est en attente mais ne bloque pas le MVP — décision actée.
