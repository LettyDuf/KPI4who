...
| 7.8 | **Navigation transverse — pontage entre portes, retour au tableau de bord, unification des accueils** | 🟡 | *Ouvert le 22/04/2026 après remontée terrain de Lætitia.* Trois manques identifiés à l'usage : (a) **pas de persistance visible du tableau de bord** — une fois sortie de l'app à onglets (via « ← Accueil »), plus aucun indice ne signale qu'un tableau de bord existe et avec quel contexte ; il faut se rappeler quel niveau/domaine on avait sélectionné dans la pyramide pour y retourner. (b) **Pas de pontage horizontal entre portes** ni entre portes ↔ app-onglets — le bouton « ← Accueil » est le seul pivot, obligeant à repasser par l'accueil pour changer de porte. (c) **Deux accueils qui se masquent mutuellement** (pyramide `#accueil` vs 4 portes `#accueil-portes`, bascule via `CM.App.basculerAccueil(...)`) — selon l'accueil préféré mémorisé, le retour n'aboutit pas toujours là où l'utilisateur s'attend, et l'autre accueil n'est accessible que via un petit lien en pied de page. **Pistes** : barre de navigation persistante en en-tête donnant accès direct aux 3 portes + accueil unifié ; ou bien fusion des deux accueils en une seule page montrant pyramide ET portes ; ou encore indicateur visuel « contexte actif » (niveau/domaine sélectionné) persistant dans l'en-tête. Décision UX à cadrer. Voisin de 7.7 (persistance des parcours) — peut être mené conjointement. | ⏸ |
...
| 7.8 Navigation transverse | En-tête des portes (lignes ~2134, ~2164, ~2199 dans `cadre-indicateurs.html`) + en-tête de `#app` (ligne ~2227) + `CM.App.basculerAccueil` / `afficherAccueil` (lignes ~6848 à ~6860) + pieds des deux accueils (lignes ~1977 et ~2121) | Revoir la topologie : trois portes + app-onglets + deux accueils = 5 vues où l'utilisateur peut être piégé. Deux questions à trancher avant le code : (i) **garder ou fusionner les deux accueils** (pyramide et 4 portes) ; (ii) **introduire une barre de navigation persistante** en en-tête (accès direct aux 3 portes + tableau de bord) ou se limiter à enrichir les pieds de page actuels. Respecter le contrat de cohérence entre portes — toute barre de nav doit rester identique sur toutes les vues, pas de divergence voulue ici. Pas de nouveau module nécessaire si la solution tient dans l'en-tête existant. |
