...
Dernière mise à jour : **25 avril 2026 — étape (b) du chantier 14 livrée** (tâche 3 close avec commit 3 : harnais `tests-requete-metriques.html` + générateur `outils/construire-tests-requete-metriques.js`, 9 suites / 59 tests verts en headless. Tag `mvp-etape-b-coeur-extrait` posé sur `31e59ac`.)
...
- **Événement majeur de la session** : **étape (b) du chantier 14 LIVRÉE**. Commit 3 (le 5e commit de la tâche 3) clos avec un harnais de tests automatisés autonome. Marqueurs `BEGIN/END` posés autour de `CM.RequeteMetriques` dans `cadre-indicateurs.html` (`29b7de2`, no-op runtime, suit le patron `CM.Panier`). Générateur Node `outils/construire-tests-requete-metriques.js` + harnais `tests-requete-metriques.html` livrés (`31e59ac`, calque strict de `construire-tests-panier.js`). Le harnais monte `CM.Referentiel` et `CM.IndicateursMeta` en doubles de test (stubs in-memory) — la sémantique du module est exercée en isolation, indépendamment du contenu éditorial réel. Jeu de test : 12 fiches synthétiques couvrant tous les axes du filtre. **9 suites / 59 tests verts** en exécution headless via runner Node VM minimal. Couverture : Sanity, Comportements limites §10.3 (22 tests), Union OR, Intersection AND avec sémantique d'absence, Seuils ordinaux fiabiliteMin/maturiteMin, Capping `limite`, Sucre `probleme`, Filtres croisés 2-4 clauses, Ordre de retour stable. Tag `mvp-etape-b-coeur-extrait` posé sur le HEAD `31e59ac`. Iso-comportement utilisateur strict — aucun consommateur n'appelle encore `executer`.
...
- **Dernier SHA de contenu** : `31e59ac` (commit 3 de la tâche 3, étape (b) close). Précédents de la séance : `29b7de2` (marqueurs BEGIN/END sur CM.RequeteMetriques), `2c6aa61` (commit 2b — union/intersection), `4c7e1b8` (commit 2a — tagsThematiques + générateur), `63e76f2` (commit 1 — squelette + validation), `abd9315` (alignement §10.3). **Tags posés** : `baseline-avant-hexagonal` = `5655b03` (retour absolu), `mvp-etape-a-schema-inventorie` = `1ba5112`, **`mvp-etape-b-coeur-extrait` = `31e59ac` (nouveau)**. Prochain tag cible : `mvp-etape-c-portes-migrees` après l'étape (c).
...
- **Prochaine action recommandée** : **étape (c) du chantier 14 — migration porte par porte**. Démarrer par la porte la plus simple (probablement *Par mon problème*) en remplaçant la logique de filtrage interne par un appel à `CM.RequeteMetriques.executer(filtre)`. Préalable : ouvrir `scenario-non-regression.md` et l'enrichir d'une variante par porte si besoin. Discipline : un seul commit par porte, scénario vert avant de passer à la suivante. Cf. §4 du doc compagnon (étape (c)).
