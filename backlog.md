...
Dernière mise à jour : **7 mai 2026 (séance — fiche-question pastèque + doctrine § 4.6 trio à 3 axes invariants)** — (i) **deuxième fiche-question livrée** : *« Mes indicateurs reflètent-ils la réalité du terrain ? »* (chantier 20.7), variante méta. **Doctrine § 4.6 *Trio à 3 axes invariants* inaugurée** dans `doc-catalogue-questions-fines.md` comme mécanique-sœur de § 4.4, réservée aux fiches méta : la triangulation est invariante (3 axes : Déclaratif / Objectif / Humain anonyme), seul l'outillage varie selon l'échelle. (ii) **Décisions 20.7 et 20.8 closes** — cross-link réciproque effectif entre les deux fiches-questions livrées. (iii) **Nouvelle dette de référentiel détectée** : 2 fiches déclaratives (équipe + programme) + 1 fiche cadre patrimoniale *triangulation*. Mockup : `mockup-fiche-question-pasteque.html` (569 lignes, palette D, panel Drucker / Toyota Way / Edmondson / DORA-SPACE). **4 commits au total** sur la séance — `f5fb2df` `9baa152` `b2798cf` `cec66e0`.
...
- **Événement majeur de la séance (07/05/2026)** : **deuxième fiche-question livrée** — *« Mes indicateurs reflètent-ils la réalité du terrain ? »* (chantier 20.7), variante méta. **Doctrine § 4.6 *Trio à 3 axes invariants* inaugurée** comme mécanique-sœur de § 4.4 (réservée aux fiches méta). Pour une fiche méta, la triangulation est invariante (Déclaratif / Objectif / Humain anonyme), seul l'outillage varie selon l'échelle. Cross-link réciproque activé entre les deux fiches-questions livrées (pastèque ↔ amélioration continue) — décision 20.8 close.
- **4 commits sur la séance** : `f5fb2df` (mockup pastèque) · `9baa152` (doctrine § 4.6 + clôture 20.7) · `b2798cf` (réciproque cross-link dans mockup format) · `cec66e0` (clôture 20.8 dans § 9).
- **Mockups produits** : `mockup-fiche-question-pasteque.html` (569 lignes, palette D bleu de Prusse, panel Drucker / Toyota Way / Edmondson / DORA-SPACE).
- **Cibles fiches** : 9 cards possibles (3 axes × 3 niveaux), 7 cibles existantes (`s4` / `lss-1` / `p9` / `s11` / `x4` / `p4` / `s12`), 2 fiches en cours de constitution (déclaratifs équipe + programme — option mixte appliquée).
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges · 20.3 volumétrie panels · 20.4 responsive double réponse · 20.5 rédaction MVP (**2/8-12 fiches livrées** — pastèque s'ajoute à amélioration continue) · 20.6 trio alternatif v2.
- **Nouvelle dette de référentiel détectée** : 2 fiches déclaratives (équipe + programme) + 1 fiche cadre patrimoniale *triangulation déclaratif/objectif/humain*. Inscrites dans la table dette du chantier 20.
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→D` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **rédiger une troisième fiche-question** pour 20.5 (un autre panel — Coach Agile, Coach exécutif, ou Gestion du changement) ; (b) **traiter une des 2 fiches déclaratives manquantes** (équipe ou programme) pour solidifier le référentiel pastèque ; (c) **rédiger la fiche cadre patrimoniale** *triangulation* pour activer le lien sortant pédagogique de la fiche pastèque ; (d) **traiter la dette filets niveau** détectée par l'audit AA (item § 2).
