...
Dernière mise à jour : **21 avril 2026** (chantier 7.2a-inventaire — niveau Portefeuille livré avec `fcd8a84`, 13 fiches rôles couvrant SAFe Portfolio, PMI, Cagan, TOGAF, ASQ, Business Agility Institute, fonction publique QC/CA classes 1-2, et écosystème coopératif québécois. Micro-fix rétroactif sur Tactique dans `ae3233c` : la fiche *Directeur de service* est renommée *Directeur* et recalée sur la classification officielle avant l'ajout du niveau Portefeuille. Un niveau restant à produire : Exécutif).
...
| 7.2a-inventaire | **Inventaire éditorial des rôles par niveau (prérequis de la phase code)** | 🟡 | *Ouvert le 21/04/2026 après l'arbitrage D2 du mockup-preview. Trois niveaux livrés sur quatre : Opérationnel (`cfa5b68`, 13 rôles), Tactique (`6dd1d30`, 14 rôles — recalé par `ae3233c` sur la classification fonction publique), Portefeuille (`fcd8a84`, 13 rôles). Un niveau restant à produire : Exécutif.* Construction d'un référentiel éditorial de ~12-15 rôles par niveau, couvrant Agile/Scaled, Lean Six Sigma, produit, PMBOK, fonction publique, management classique. Décisions actées : **D2** (quatre axes humaine / projet / méthodologique / stratégique, cumulables), **N1** (double-libellé), **R2** (rôle répété avec qualificatif si à cheval), **C2** (clic sur rôle), **α** (un seul champ *descriptif orthodoxe* par rôle — le champ *dérive du marché* est abandonné ; renverse la décision ρ1 prise plus tôt le 21/04/2026, voir `project_porte_niveau_doctrine_editoriale.md`). Règles éditoriales : pas de mot *senior* (jugement de valeur), PO reformulé par périmètre de produit et non par séniorité. Livrable : fichier `inventaire-roles-porte-niveau.md` (3 niveaux sur 4 en place), source éditoriale pour `CM.Roles` en phase code. Panel consolidé : SAFe®, Scrum Guide 2020, Lean Six Sigma (Yellow/Green/Black/Master Black Belt + Champion + Sponsor), Marty Cagan (rôles produit), PMBOK / PMI, PRINCE2, TOGAF / Zachman, Business Agility Institute (Harbott, Sahota, Burrows), fonction publique québécoise et fédérale canadienne (classes 1 à 4 / EX-01 à EX-05), écosystème coopératif et parapublic QC (Desjardins prototype), Mintzberg (cartographie axes), Jaques (horizon temporel pour trancher les cas limites). Voir `project_porte_niveau_roles.md` et `project_porte_niveau_arbitrages_roles.md`. | ⏸ |
| 7.2a-code | **Stepper générique + porte niveau en accordéon à 4 cartouches (architecture)** | 🟡 | *Scindé de l'ex-7.2 le 21/04/2026 après-midi.* Volet **architecture**. Extraire un `CM.Stepper` générique qui accepte un tableau ordonné d'étapes (nombre variable, 3/4/5+ — exigence d'extensibilité validée le 20/04/2026 pour accueillir 7.3 et 7.4 sans réécriture). Porte *niveau* rejoint le parcours guidé avec ordre **niveau → problème → cadre → résultats** ; étape 1 sous forme d'accordéon à 4 cartouches (Opérationnel / Tactique / Portefeuille / Exécutif — libellés de surface, `META.niveau` interne inchangé). Chaque cartouche consomme `CM.Roles` (construit en 7.2a-inventaire). Clic sur le rôle = validation étape 1, état `{ niveau, role }` capturé. Prérequis : 7.2a-inventaire livré. | ⏸ |
