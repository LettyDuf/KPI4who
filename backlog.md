...
Dernière mise à jour : **26 avril 2026 — étape (c.3) livrée** (porte *Par mon niveau* migrée vers `CM.RequeteMetriques.executer` : `_etapeResultats` formule un filtre à trois axes au lieu de croiser deux listes diagnostics. Sentinelle I7 verte en interactif Safari (Développeur × Flux & goulots × DORA, 7 fiches, top 3 = *Fréquence de déploiement* → *Délai de livraison des changements* → *Durée de cycle*). Tag `mvp-etape-c3-porte-niveau-migree` posé sur `34366f3`. Reste (c.4) emplacement *Par ma maturité* à instruire pour clore (c).)
...
- **Événement majeur de la séance** : **étape (c.3) du chantier 14 livrée — porte *Par mon niveau* migrée vers `CM.RequeteMetriques.executer`**. Commit unique atomique (`34366f3`). `_etapeResultats` ne fait plus deux appels diagnostics + intersection manuelle ; il formule un filtre à trois axes (`{niveau, tags, cadres}`) et délègue. Sentinelle I7 figée pré-bascule sur `c2240b3` puis verte post-bascule (Développeur × Flux & goulots × DORA = 7 fiches, top 3 strictement identique). Tag `mvp-etape-c3-porte-niveau-migree` = `34366f3`. `CM.VuePorteNiveau` n'accède plus à `CM.Referentiel` ni `CM.IndicateursMeta` directement — seuls les passe-plats métadonnées (`problemeInfo`, `cadreInfo`, `niveauInfo`) restent. Helper privé `_rangFiabilite` ajouté dans le module pour le tri local.
- **Découverte de séance — chantier 18 ouvert** (zone aveugle catalogue rôles). Audit factuel pyramide vs accordéon Opérationnel : 4 TI + 6 Affaires de la pyramide → seuls 2/4 domaines TI couverts par les rôles, 0/6 Affaires. Section §18 du backlog avec question doctrinale α/β + axe de réflexion *case à cocher / étape supplémentaire dans le parcours des portes* (4 pistes inventoriées). Préalable à l'étape D de 7.2a-code.3 (« remplace pyramide » consigné ligne 7457). Hors scope chantier 14, post-livraison.
- **Chantier actif** : **chantier 14 — refactor hexagonal**. Étapes (a), (b), (c.1), (c.2), (c.3) closes. **Reste (c.4) — emplacement *Par ma maturité*** : porte stub, pas de migration code, juste s'assurer que l'emplacement architectural est propre (à instruire en début de prochaine séance, ~30 min). Puis (d) — nettoyage et documentation finale.
- **Doctrine traducteur confirmée à (c.3)** : la porte formule, elle ne post-filtre pas. Premier triplet à trois axes du chantier (c.1 et c.2 jouaient sur deux). Confirme que la composition AND inter-clauses du moteur tient sur trois dimensions sans surprise. Tri par fiabilité conservé localement, comme à (c.1) et (c.2).
- **Document central pour la reprise** : **`journal-invariants-pre-c.md`** (commit `9525f8e`, enrichi `c2240b3` avec I7). Quatre invariants figés sur HEAD `2a61336` + I7 figé sur `c2240b3` (post-§18 docs, code identique). I1 ✓ post-(c.1). I2 ✓ post-(c.2). I7 ✓ post-(c.3). Reste à valider I3 exhaustif et I5. Toute divergence ±1 → rollback à `5655b03`.
- **Test interactif côté Lætitia — état du journal** : I1 ✓, I2 ✓, **I7 ✓** (Développeur × Flux & goulots × DORA = 7 fiches strictement identiques pré/post). I3 partiel (à exhausifier sur la liste complète des 13 fiches I2 — non bloquant). **I5 toujours incomplet** (titre de la première fiche manquant — à compléter au prochain passage). I4 et I6 en skip documenté (I6 remplacé en pratique par I7).
...
- **Dernier SHA de contenu** : `34366f3` (c.3 commit unique). Précédents : `894ab8a` (c.2), `83d3bd2` (commit 2 de c.1). **Tags** : `baseline-avant-hexagonal` = `5655b03`, `mvp-etape-a-schema-inventorie` = `1ba5112`, `mvp-etape-b-coeur-extrait` = `2a61336`, `mvp-etape-c1-porte-probleme-migree` = `83d3bd2`, `mvp-etape-c2-porte-cadre-migree` = `894ab8a`, `mvp-etape-c3-porte-niveau-migree` = `34366f3`. Prochain tag cible : `mvp-etape-c-portes-migrees` (clôture globale (c) après (c.4) instruite).
- **Prochaine action recommandée** : trois pistes selon énergie. (1) **instruire (c.4) — emplacement *Par ma maturité*** ; courte vérification architecturale, puis tag `mvp-etape-c-portes-migrees`. (2) **compléter le journal** : titre première fiche I5 + liste complète des 13 fiches I2 pour I3 exhaustif. (3) **basculer sur (d) — nettoyage et documentation finale** dès que (c) est globalement clos.
- **Blocages / questions ouvertes** : aucun côté contenu. Côté workflow git sandbox : `.lock` orphelins continuent de s'accumuler à chaque commit (HEAD.lock, index.lock, tmp_obj_*) — nettoyage côté ordi à faire régulièrement (`rm -f .git/*.lock .git/objects/*/tmp_obj_* .git/objects/maintenance.lock` depuis le dossier projet). À noter pour la séance : 3 nettoyages nécessaires sur 3 commits successifs côté Lætitia.
