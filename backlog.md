...
Dernière mise à jour : **7 mai 2026 (séance — 2 fiches-questions livrées, 3 itérations chacune, 4 doctrines bougées)** — (revue Lætitia déclenche 3 corrections sur pilotage hebdo : reformulation question pour exposer le piège *occupation*, suppression boutons internes redondants avec le bandeau, bascule fond fiche en blanc franc — propagation tardive de la doctrine § 6.5 actée le 06/05 dans `17d62f3`. Rétrofit symétrique sur pastèque). Détail : (i) **deuxième fiche-question MVP livrée** : *« Mes indicateurs reflètent-ils la réalité du terrain ? »* (chantier 20.7, fiche pastèque), variante méta. **Doctrine § 4.6 *Trio à 3 axes invariants* inaugurée** comme mécanique-sœur de § 4.4 (la triangulation est invariante en axes, seul l'outillage varie selon l'échelle). (ii) **Décisions 20.7 et 20.8 closes** — cross-link réciproque effectif entre les fiches-questions livrées. (iii) **Troisième fiche-question MVP livrée** : *« Quoi mettre dans mon pilotage hebdomadaire ? »* (chantier 20.5, panel Coach Agile), **première matérialisation autonome de la variante § 4.3 (à reformulation)** hors mockup format. Couple acté : *Reporting d'occupation* ↔ *Pilotage par le flux*. Décision 20.5 passe à 🟡 (3/8-12 fiches MVP). (iv) **Nouvelle dette de référentiel détectée** sur la fiche pastèque : 2 fiches déclaratives (équipe + programme) + 1 fiche cadre patrimoniale *triangulation*. Mockups : `mockup-fiche-question-pasteque.html` (569 lignes) · `mockup-fiche-question-pilotage-hebdo.html` (391 lignes), tous deux en palette D. **7 commits au total** sur la séance — `f5fb2df` `9baa152` `b2798cf` `cec66e0` `97eb530` `ffba1ae` `b8de41b`.
...
- **Événements majeurs de la séance (07/05/2026)** : (1) **fiche pastèque** livrée puis itérée 3 fois (v1 livrée, v2 fond blanc + Q1 réduit à 3 options, v3 reformulation Q1 Déclaratif et Q2 *« Une équipe »*). Doctrine § 4.6 *Trio à 3 axes invariants* inaugurée et enrichie d'une règle (autant d'options Q1 que d'axes invariants). (2) **Cross-link réciproque** entre les 2 fiches-questions livrées (décisions 20.7 et 20.8 closes). (3) **Fiche pilotage hebdomadaire** livrée puis itérée 3 fois (v1 livrée, v2 reformulée pour exposer le piège *occupation* + suppression boutons + fond blanc, v3 suppression bandeau d'orientation + colonnes cliquables). (4) **Doctrine § 4.3 actualisée** : la variante à reformulation passe à v4.2 (colonnes cliquables, libellé de destination en pied, plus de boutons internes ni de bandeau séparé). Mockup format scène 2 aligné sur le nouveau pattern. (5) **Doctrine § 6.5 fond fiche retrouvée** (commit fondateur `17d62f3`) — `.fiche` en blanc franc, mise en mémoire pour ne plus reperdre.
- **15 commits sur la séance** : `f5fb2df` `9baa152` `b2798cf` `cec66e0` `97eb530` `ffba1ae` `b8de41b` `cb1248c` (bloc 1+2 : 2 fiches livrées + doctrines § 4.6 et § 4.3) ; `528d08f` `8367a9c` `6e278bd` (revue 1 : pilotage hebdo v2 + rétrofit pasteque) ; `798c4be` `85c9f5f` (revue 2 : pasteque v2 puis v3) ; `010987a` `80160fb` `985e3c0` (revue 3 : pilotage hebdo v3 + mockup format v4.2 + doctrine § 4.3 actualisée).
- **Mockups produits / itérés** : `mockup-fiche-question-pasteque.html` (v3, palette D, 3 axes invariants stricts) · `mockup-fiche-question-pilotage-hebdo.html` (v3, palette D, colonnes cliquables) · `mockup-format-fiche-question.html` (v4.2 — référence canonique alignée).
- **Avancement chantier 20.5** : 3/8-12 fiches MVP livrées. Panels couverts : Coach Lean (1), Drucker méta (1), Coach Agile reformulation (1). Panels encore à 0 : Coach exécutif (Drucker MBO orthodoxe), Gestion du changement (Kotter, ADKAR), Terrain des autres cadres (BSC, OKR Doerr, ITIL).
- **Question candidate** : *« Quoi mettre dans mon pilotage hebdomadaire ? »* (déversée au backlog 07/05) — à reverser comme fiche standard § 4.4 si le besoin se confirme.
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges (5 identifiés à étoffer : vélocité, ROI, NPS, story points, *occupation*) · 20.3 volumétrie panels · 20.4 responsive (couplée à D4 — désormais purement layout, plus comportement) · 20.6 trio alternatif v2.
- **Dette de référentiel à résorber** (issue de la fiche pastèque) : 2 fiches déclaratives (équipe + programme — *Done auto-déclaré*, *RAG status PM*) + 1 fiche cadre patrimoniale *triangulation déclaratif/objectif/humain*.
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→P` accumulées en attente côté ordi.
- **Mémoires actualisées cette séance** : `project_doctrine_trio_axes_invariants.md` (règle des options Q1) · `project_doctrine_fond_fiche.md` (matière en blanc franc, ne pas recopier aveuglément un CSS de mockup antérieur).
- **Prochaine action recommandée** : (a) **rédiger une 4<sup>e</sup> fiche-question** sur un panel encore à 0 (Coach exécutif, Gestion du changement, Terrain des autres cadres) ; (b) **mettre en mockup la 1<sup>re</sup> fiche** d'amélioration continue (rédigée 05/05 sans mockup) ; (c) **traiter une fiche déclarative manquante** ; (d) **rédiger la fiche cadre patrimoniale triangulation** ; (e) **traiter la dette filets niveau** (§ 2).
