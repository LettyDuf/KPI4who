...
- **Événements majeurs de la séance (07-08/05/2026 nuit, suite séance soir)** : (1) **Étape 1 du plan MVP global v1 livrée** — défaut accueil basculé sur les portes (`d51c0ae`), 5e porte *Par ma question* + vue d'index des 6 fiches-questions (`1566f63`), layout 2x2 + 5e en bandeau plein largeur après preview B (`366f647`). (2) **6 mockups fiches-questions nettoyés** option B retenue : `.titre-mockup` et `.note-pedagogique` retirés, aphorisme `.pied-mockup` conservé (`5afe198`). (3) **Étape 1.bis livrée en avance — bandeau du haut 8 entrées** sur les 6 vues de premier rang (`5d2a1c1` accueil + question, `5c72263` 3 vues détail, `c658a53` app). Wiring : 4 entrées fonctionnelles (Accueil, Mon TdB, Par ma question, À propos), 4 stubs assumés (Par mes 4 axes, Cascade, Maturité, Lexique) tant que le chantier 10 reste verrouillé par MISSION.md. Nouveau dispatcher `CM.App.demanderRetourAccueil()` qui délègue au module actif. (4) **Bug porte « Par mon niveau » corrigé** (`8430a0b`) — câblage historique vers la pyramide remplacé par `CM.VuePorteNiveau.ouvrir()`. La pyramide reste en sursis, accessible par lien de pied. (5) **Friction consignée § 10** (`8ddc85f`) — redondance pyramide ↔ choix du niveau dans la porte.
- **Note de rectification 08/05/2026** : le diagnostic rude proposé en fin de séance pointait à tort la dette T1 comme bloquante. Vérification du code (`rendreIconesPanier` ligne 7157, `basculerStatutPanier` ligne 9902, voie lente `.vl-duo-carte`, tag `mvp-9d-voie-lente`) et de la doctrine (§ 9 ligne 447, `doc-contrats-panier.md` ligne 324) : 9.C et 9.D livrés fin avril, persistance session éphémère assumée comme décision produit. La dette T1 telle qu'annoncée n'existe plus. Memory `feedback_verifier_avant_affirmer.md` à appliquer plus strictement. **9 commits sur la séance** (fonctionnels) + `e5ec9fb` clôture initiale (contenait la contre-vérité sur l'étape 2) + commit de rectification de cette note.
- **9 commits sur la séance** : `d51c0ae` (défaut portes) · `1566f63` (5e porte) · `366f647` (layout 2x2) · `5afe198` (nettoyage mockups) · `5d2a1c1` (bandeau accueil + question) · `5c72263` (bandeau 3 vues détail) · `8ddc85f` (friction consignée) · `c658a53` (bandeau app) · `8430a0b` (porte niveau câblée).
- **Avancement vision MVP global v1** : ✅ étape 1 (accueil 3 portes par défaut) · ✅ étape 1.bis (bandeau du haut) · ✅ **étape 2** — la dette T1 du chantier 9 a été en fait livrée **fin avril 2026** par les chantiers 9.C (voie rapide icônes coche/ampoule sur les cartes) et 9.D (voie lente zone d'ajout dans le tiroir, tag `mvp-9d-voie-lente`). Le bouton « Mon tableau de bord » du bandeau est donc fonctionnel : empilage des fiches, persistance session éphémère assumée (décision produit § 9 ligne 447, l'impression A4 sert d'export). · ✅ étape 3 (porte *Par ma question* livrée ce soir, version index minimaliste — l'enrichissement filtre/panel viendra au besoin terrain). · ⏳ **étape 4** — tests bout en bout par Lætitia + finitions impression, seul reste.
- **Bandeau du haut — état du wiring** : Accueil → `CM.App.demanderRetourAccueil()` (dispatcher par vue active) ; Mon tableau de bord → `CM.App.ouvrirTableauBord()` (panier rempli via voies rapide/lente déjà câblées depuis fin avril, persistance session éphémère assumée) ; Par ma question → `CM.VuePorteQuestion.ouvrir()` ; À propos → `CM.Apropos.ouvrir()`. Stubs (`Par mes 4 axes`, `Cascade stratégique`, `La maturité ?`, `Lexique`) : opacité 0.5, cursor default, click sans effet. Marqueur actif : fond pill `--niv-1-clair` + texte `--niv-1` + font-weight 700. État actif par vue : Accueil (accueil portes), Par mes 4 axes (3 vues détail problème/cadre/niveau), Par ma question (vue index), Mon tableau de bord (#app actif).
- **Dette de référentiel cumulée (inchangée)** : (issue pastèque) 2 fiches déclaratives + 1 fiche cadre triangulation ; (issue résistance) 3 fiches métriques absentes (Score conscience-désir, Taux d'adoption, Fréquence régressions) + fiche cadre *Gestion du changement* en `CADRES_A_VENIR` ; (issue cascade objectifs) 3 fiches métriques absentes + fiche cadre *Management By Objectives* en `CADRES_A_VENIR`. Total : 8 fiches métriques + 3 fiches cadres à créer.
...
- **Frictions § 10 (chantier nav verrouillé)** : (i) chemin laborieux vers Mon tableau de bord (26/04/2026) — résolu par le bandeau livré ce soir ; (ii) redondance pyramide ↔ choix du niveau (07/05/2026) — à traiter avec disparition pyramide au chantier 10.
- **Mémoires actualisées cette séance** : aucune (les frictions § 10 et l'avancement MVP sont consignés dans le backlog, source canonique).
...
- **Prochaine action recommandée** : **Étape 4 du plan MVP global v1 — validation bout en bout par Lætitia** (~30-60min de test, durée de remédiation variable selon les frictions). Le MVP est à 90% : 3 étapes sur 4 livrées (1, 1.bis, 2 livrée fin avril, 3 livrée ce soir). Reste à mettre l'outil à l'épreuve d'un parcours réel — entrer par une porte, choisir 3 à 5 indicateurs via voies rapide (coche/ampoule sur les cartes) ou lente (zone d'ajout dans le tiroir), ouvrir « Mon tableau de bord » depuis le bandeau, imprimer le scorecard A4. Signaler les frictions : libellés, comportements inattendus, finitions de l'impression. Vague de petites remédiations attendue, pas un nouveau chantier de fond. Travaux *en parallèle* possibles : nettoyer les `.todelete` et `.git/*.lock.tmp*` cumulés côté ordi ; ouvrir le câblage propre du Lexique (création d'un tiroir) pour transformer le stub du bandeau en lien fonctionnel ; ouvrir un test E2E avec un manager pilote externe (point fort du diagnostic du 08/05 — l'outil n'a jamais été touché par un utilisateur autre que Lætitia).
- **Blocages / questions ouvertes** : aucun. Étape 1 + 1.bis livrées, prêtes pour Lætitia à utiliser ; étape 2 disponible à l'attaque.
