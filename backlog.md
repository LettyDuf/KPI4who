...
Dernière mise à jour : **23 avril 2026 — après-midi** (chantier **10 — refonte architecture de navigation** : preview fondateur 10.0 livré, premier arbitrage tranché — **variante B (fond pill indigo clair)** retenue pour le marquage visuel de l'onglet actif).
...
- **Événement majeur de la session** : **chantier 10.0 — preview fondateur livré et premier arbitrage tranché**. Quatre previews produits et commités atomiquement : `988efb6` variante A (soulignement ambre), `b1f6bb9` variante B (fond pill indigo), `dcbda16` variante C (bold + barre accent orange), `d410cf0` page de comparaison côte à côte. **Variante B retenue** pour le marquage visuel de l'onglet actif — fond pill `--niv-1-clair` + texte `--niv-1` + `font-weight: 700`. Choix continuiste, en prolongement du geste déjà en place sur les onglets actuels.
- **Décisions UX cumulées (chantier 10)** : (a) **7 entrées de premier rang** actées (Accueil, Mon tableau de bord, Choisir mes indicateurs, Cascade stratégique, La maturité ?, Lexique, À propos) ; (b) **porte *Par ma maturité* conservée** distincte de l'onglet *La maturité ?* ; (c) libellé **La maturité ?** avec point d'interrogation volontaire (posture socratique) ; (d) *Cascade stratégique* réémerge en vue stub (*Bientôt*) dès 10.1 ; (e) **onglet actif = fond pill indigo clair** (arbitrage preview 10.0 — 23/04/2026) ; (f) chantier **12** (fiabilité éditoriale) et **13** (outil de diagnostic de maturité) ouverts.
- **Chantier actif** : **chantier 10.0 en cours — trois questions § 4.2 restent à trancher**. (1) exposition des 4 lentilles dans *Choisir mes indicateurs* (tabs internes / cards / stepper ouvert) ; (2) sort de `vue-questionnaire` (conservé / fondu / supprimé / fusionné avec *Cascade*) ; (3) comportement responsive (bandeau plein / hamburger / icônes-only). Prochaine étape = produire les previews pour ces axes, question par question. Point 4 (marquage actif) ✅ tranché.
- **Dernier SHA sur `main`** : à compléter après ce commit d'arbitrage (vient après `d410cf0`). Commits récents de la session : `ed51cf0` (fix `vue-maturite`), `10e35e5` (cotation *Risquées*), `2fac04f` / `10cea97` / `54d80fe` (doc compagnon A/B/C), `af1557e` (clôture doc + ouverture chantiers 12/13), `988efb6` / `b1f6bb9` / `dcbda16` / `d410cf0` (previews 10.0 A/B/C + comparaison).
...
- **Fiches mémoire pertinentes pour la suite du chantier 10** : `project_contrat_coherence_portes`, `project_porte_niveau_statut`, `project_regles_refactoring_progressif`, `project_document_compagnon_contrats`, `feedback_mockup_preview_ux` ; et [`MISSION.md`](./MISSION.md) comme boussole (à rafraîchir en clôture de 10 : renommer section *Maturité & Recommandations* → *La maturité ?*, retirer la mention *en pause* de *Cascade stratégique*).
- **Blocages / questions ouvertes** : aucun. À la reprise : arbitrer le prochain point § 4.2 (lentilles / `vue-questionnaire` / responsive), produire les previews associés.
