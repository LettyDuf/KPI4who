...
1. **7.2a-code — Stepper générique + porte niveau en accordéon à 4 cartouches** (🟡). **Prérequis 7.2a-inventaire satisfait (clos le 21/04/2026 soir avec `5ab22b8`).** Chantier architectural structurant : extraction de `CM.Stepper` générique qui accepte un tableau ordonné d'étapes (nombre variable, condition d'extensibilité pour 7.3 et 7.4), construction de `CM.VuePorteNiveau` qui consomme l'inventaire des 53 rôles. Mockup-preview obligatoire avant toute trancheuse UX. Armé du contrat de cohérence `project_contrat_coherence_portes.md`.
2. **AUDIT-F-1 — Conseil pédagogique dans la porte cadre** (🟡). Micro-fix opportuniste ressorti de l'audit du 21/04/2026. Session courte, effet éditorial direct. Intermède possible entre 7.2a-inventaire et 7.2a-code, ou avant 7.2a-code si besoin d'un sas plus léger.
