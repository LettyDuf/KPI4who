...
Dernière mise à jour : **1er mai 2026 — trio post-1.ter clos** (constats n°6, n°11, n°12). 8 commits atomiques, 2 nouveaux outils versionnés (`outils/lancer-tests-requete-metriques.js` + `outils/verifier-coherence-vocabulaire.js`). Bug runtime fermé sur la porte « Par mon cadre ». Doctrine §10.3 = source unique réaffirmée. 5 constats collatéraux du chantier 1.ter restent ouverts (n°1, n°4, n°8, n°9, n°10).
...
- **Événement majeur de la séance (01/05/2026)** : trio de constats collatéraux issus du chantier 1.ter clos en chaîne — n°6 (bug runtime VOCAB.cadres ↔ CADRES{} ↔ §10.3), n°11 (harnais de tests désaligné), n°12 (test d'invariant manquant). 8 commits atomiques sur la séance. Bug runtime fermé : porte *Par mon cadre* à nouveau complète sur ses 11 tuiles actives (auparavant 3 tuiles cassées : Sécurité, Continuous Delivery, SPACE). Aucun tag MVP posé (sentinelles ≠ release).
- **Outils livrés (variante équilibrée du n°12, validée par Lætitia)** : `outils/lancer-tests-requete-metriques.js` (`de27adc`) — runner Node headless qui exécute les 63 tests du harnais en CLI, mock DOM minimal, zéro dépendance, exit 0/1/2. `outils/verifier-coherence-vocabulaire.js` (`a8e960c`) — sentinelle qui vérifie 9 vocabulaires × 3 sources (invariant fort sur `cadres`, invariant simple sur les 8 autres), test négatif validé manuellement. Les deux à lancer **avant tout commit qui touche à VOCAB, CADRES, §10.3 ou §10.2**.
- **Doctrine restaurée** : §10.3 du doc compagnon réaffirmée comme source unique de vérité (commit `c14d979`). Encart pédagogique posé dans le journal §9 du chantier 14 documentant l'incident de gouvernance (le code a précédé le doc deux fois pendant l'audit 1.ter — extension `securite/cd/space` puis déplacement `bsc/mbo`).
...
- **Prochaines actions ouvertes** : (1) **4 constats collatéraux restants du chantier 1.ter** — n°4 (densification t3), n°8 (pauvreté corpus Scrum), n°9 (cadre `cx` manquant), n°10 (5 pistes : change-mgmt, sales, pmi-evm, grc-compliance, hr-analytics). [n°1 clos le 01/05/2026, voir le constat dans le corps du backlog] (2) **1.bis axes restants** — Tactique (LPM, DORA, ROI), Programme (CRM probable faux positif), Op TI, Op Affaires. (3) Chantier 10 (gelé à `5655b03`) peut reprendre. (4) Items 6.9, 18, 12-17 en sommeil.
...
**Constats collatéraux ouverts à statuer** : n°4 (densification t3), n°8 (pauvreté du corpus Scrum dans le référentiel), n°9 (cadre `cx`), n°10 (5 pistes cadres : change-mgmt, sales, pmi-evm, grc-compliance, hr-analytics). *[n°1 clos le 01/05/2026 ; n°6 clos le 01/05/2026 ; n°2, n°3, n°11, n°12 clos antérieurement]*
