...
- **Prochaine action recommandée** : (a) **résorber la dette de référentiel cumulée** — 11 fiches manquantes : 8 fiches métriques (2 déclaratives pasteque + 3 résistance + 3 cascade objectifs) + 3 fiches cadre (*triangulation déclaratif/objectif/humain*, *Gestion du changement*, *Management By Objectives*). Ordre d'attaque à arbitrer entre *fiches cadre d'abord* (débloque les liens patrimoniaux des 6 fiches-questions livrées) ou *fiches métriques d'abord* (densifie les trios) ; (b) **traiter la dette filets niveau** (§ 2) ; (c) **engager la décision 20.4 responsive** maintenant que 6 fiches MVP existent comme matière de test ; (d) **ouvrir la décision 20.6 trio alternatif v2**.
