...
- **Événements majeurs de la séance (07/05/2026)** : (1) **deuxième fiche-question MVP livrée** — *« Mes indicateurs reflètent-ils la réalité du terrain ? »* (chantier 20.7, fiche pastèque), variante méta. **Doctrine § 4.6 *Trio à 3 axes invariants* inaugurée** comme mécanique-sœur de § 4.4. Cross-link réciproque activé (décisions 20.7 et 20.8 closes). (2) **Troisième fiche-question MVP livrée** — *« Quoi mettre dans mon pilotage hebdomadaire ? »* (chantier 20.5), **première matérialisation autonome de la variante § 4.3 (à reformulation)**. Couple acté : *Reporting d'occupation* ↔ *Pilotage par le flux*.
- **7 commits sur la séance** : `f5fb2df` (mockup pastèque) · `9baa152` (doctrine § 4.6 + clôture 20.7) · `b2798cf` (réciproque cross-link) · `cec66e0` (clôture 20.8) · `97eb530` (backlog clôture 1<sup>er</sup> bloc) · `ffba1ae` (mockup pilotage hebdo) · `b8de41b` (catalogue § 9 + journal — 20.5 à 🟡 3/8-12).
- **Mockups produits** : `mockup-fiche-question-pasteque.html` (569 lignes, palette D, panel Drucker/Toyota Way/Edmondson/DORA-SPACE) · `mockup-fiche-question-pilotage-hebdo.html` (391 lignes, palette D, panel Scrum Guide/Kanban niveaux de vol/DORA-SPACE).
- **Avancement chantier 20.5** : 3/8-12 fiches MVP livrées. Panels encore à 0 : Coach exécutif (Drucker MBO), Gestion du changement (Kotter, ADKAR), Terrain des autres cadres (BSC, OKR Doerr, ITIL). Coach Lean = 1/2-3, Coach Agile = 1/2-3.
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges (4 identifiés à étoffer) · 20.3 volumétrie panels · 20.4 responsive double réponse (couplée à D4) · 20.6 trio alternatif v2.
...
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→G` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **rédiger une 4<sup>e</sup> fiche-question** sur un panel encore à 0 — Coach exécutif Drucker (*« Comment arbitrer entre les initiatives ? »*), Gestion du changement (*« Comment aligner mes équipes vers un objectif commun ? »*), ou Terrain des autres cadres (BSC ou OKR Doerr) ; (b) **mettre en mockup la 1<sup>re</sup> fiche-question** d'amélioration continue (rédigée le 05/05 mais sans mockup) ; (c) **traiter une des fiches déclaratives manquantes** ; (d) **rédiger la fiche cadre patrimoniale triangulation** ; (e) **traiter la dette filets niveau** (§ 2).
...
| 20.5 | **Rédaction des 8 à 12 questions MVP** | 🟡 en cours — **3 fiches livrées** : (1) *Par où démarrer une démarche d'amélioration continue ?* (panel Coach Lean, variante standard, 05/05/2026, mise en mockup à venir) ; (2) *Mes indicateurs reflètent-ils la réalité du terrain ?* (panel Drucker / Toyota Way / Edmondson / DORA-SPACE, variante méta § 4.6, 07/05/2026, commit `f5fb2df`) ; (3) *Quoi mettre dans mon pilotage hebdomadaire ?* (panel Coach Agile, variante à reformulation § 4.3, 07/05/2026, commit `ffba1ae`). **Reste 5 à 9 fiches MVP**. Panels encore à 0 : Coach exécutif (Drucker MBO), Gestion du changement (Kotter, ADKAR), Terrain des autres cadres (BSC, OKR Doerr, ITIL) |
