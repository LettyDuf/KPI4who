...
Chantier **7.2a-code.2 livré** le 21/04/2026 nuit (orchestrateur générique des parcours multi-étapes extrait, portes problème et cadre migrées). Le module `CM.Stepper` est désormais le seul responsable de l'état, de la navigation, du rendu de la barre d'étapes et du scroll ; les deux portes existantes ne conservent que leur taxonomie propre et leur rédactionnel. Iso-comportement strict respecté commit après commit — aucun changement visible côté utilisateur. La primitive `remplacerChoix(indexEtape, valeur)` a été ajoutée au stepper pour couvrir proprement le cas du pivot à étage constant (bloc *Cadres voisins*), documentée dans le compagnon. Le champ résiduel `domaine: null` a été retiré du ctx de la porte problème (audit a confirmé qu'il s'agissait d'une redondance, pas d'une dette). Commits : `be776d6` (squelette), `6ad4809` (bouton Recommencer délégué), `a2eba34` (migration porte problème), `6509fd0` (primitive `remplacerChoix`), `b36f393` (migration porte cadre), `8346d46` (doc compagnon mise à jour), `eaa514c` (nettoyage `domaine`).

**Prochain sous-chantier : 7.2a-code.3** — construction de la nouvelle porte niveau (`CM.VuePorteNiveau`), consommatrice de `CM.Roles` (livré en .1) et de `CM.Stepper` (livré en .2). Étape 1 en accordéon à 4 cartouches avec le double-libellé `Opérationnel / Tactique / Portefeuille / Exécutif` décidé en N1. Reste à câbler la porte dans la navigation principale et à lui donner sa taxonomie de première étape (accordéon rôles), sans retoucher stepper ni données rôles.
