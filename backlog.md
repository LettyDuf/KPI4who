...
| 7.2a-code.2 | **Extraction `CM.Stepper` générique + refacto portes problème et cadre** | 🟡 | Extraire l'orchestrateur de parcours multi-étapes de `CM.VuePorteProbleme` et `CM.VuePorteCadre` en un module `CM.Stepper` neutre (navigation, état `{ étape, choix }`, scroll, retour accueil, chips cliquables). Contrat d'API fixé dans `doc-contrats-stepper-roles.md`. Refacto à **iso-comportement visible utilisateur** — les deux portes existantes doivent continuer de marcher après chaque commit. Invariant structurant : nombre d'étapes arbitraire (pas de 3 en dur). | ⏸ |
| 7.2a-code.3 | **Nouvelle porte niveau `CM.VuePorteNiveau` (accordéon 4 cartouches)** | 🟡 | Construire la vue qui consomme `CM.Roles` (.1) et `CM.Stepper` (.2). Accordéon à 4 cartouches (Opérationnel / Tactique / Portefeuille / Exécutif), toutes fermées à l'arrivée ; clic sur un rôle valide l'étape 1 avec `{ niveau, role }`. Parcours ensuite : niveau → problème → cadre → résultats. Mockup-preview obligatoire avant tranche UX finale. Prérequis : 7.2a-code.1 livré, 7.2a-code.2 livré. | ⏸ |
...
Chantier **7.2a-code.1 livré** le 21/04/2026 soir (socle de données pour la porte niveau). Les 53 fiches de rôles sont désormais dotées d'ids stables, consommables par un module `CM.Roles` neutre et tolérant aux défauts éditoriaux. Un générateur Node.js (`tools/generer-roles.js`, zéro-dépendance) tient à jour la zone injectée dans `cadre-indicateurs.html`. Un document compagnon (`doc-contrats-stepper-roles.md`) fixe les contrats d'API pour éviter la dérive entre les trois sous-chantiers. Commits : `0c0a7fe` (ids), `f1612cd` (module + générateur), `c8a17a1` (doc compagnon).

**Prochain sous-chantier : 7.2a-code.2** — extraction de `CM.Stepper` à partir des portes problème et cadre, à iso-comportement. Contrat d'API déjà fixé.
...
1. **7.2a-code.2 — Extraction `CM.Stepper` + refacto portes problème et cadre** (🟡). **Prérequis 7.2a-code.1 satisfait (clos le 21/04/2026 soir avec `c8a17a1`).** Chantier architectural d'extraction à iso-comportement : les deux portes existantes doivent continuer de marcher après chaque commit intermédiaire. Contrat d'API déjà fixé dans `doc-contrats-stepper-roles.md`. Pas de big-bang : refacto progressive, chaque commit garde le parcours utilisateur fonctionnel.
2. **AUDIT-F-1 — Conseil pédagogique dans la porte cadre** (🟡). Micro-fix opportuniste ressorti de l'audit du 21/04/2026. Session courte, effet éditorial direct. Intermède possible entre 7.2a-code.1 et 7.2a-code.2, ou avant 7.2a-code.2 si besoin d'un sas plus léger.
