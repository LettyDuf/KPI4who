...
- **Séance 24/05/2026 (soir) — test utilisateur n°1 dépouillé, document compagnon créé** : Lætitia rapporte le premier test utilisateur de l'outil. 26 retours recueillis, triés en 8 familles (valeur/parcours, pédagogie des concepts, vocabulaire/glossaire, neutralité francophone, navigation/UX, cohérence éditoriale, coquilles, fonctionnalité) et consignés dans le nouveau document compagnon `test-utilisateur-1.md` (verbatim des 26 retours, qualification, table de ventilation proposée avec recoupements backlog cartographiés, 6 points d'arbitrage). Décision de cadrage de Lætitia : option 1B, ventiler ces retours dans le backlog existant (enrichir les lignes recoupées, créer les manquantes) plutôt qu'un chantier dédié. Aucune édition du backlog ni du code cette séance hormis cette clôture : la table de ventilation reste à valider. Le chantier 21 (jalon B.1 livré) reste l'ouvrage en cours, non perturbé.
- **Événement majeur de la séance 24/05/2026 (2 commits `7e40e73` `15e5216`), chantier 21 ouvert et jalon A livré** : ouverture du **chantier 21, vue Lexique** (`7e40e73`, section 21 du backlog réécrite avec plan de jalons A-G) puis livraison du **document compagnon `doc-contrats-lexique.md` v0.1** (`15e5216`, 327 lignes), référence du chantier. **Ambition retenue : exhaustive** (référentiel pédagogique, pas un simple index), parmi trois options proposées à Lætitia (miroir des fiches / lexique curé / référentiel pédagogique). **Deux arbitrages verrouillés** : (i) *Source de vérité, « Source + garde-fou »* : le Lexique devient la référence éditoriale, les fiches gardent leur attribut `data-def`, une sentinelle signale les divergences pour revue sans les empêcher (raison : une partie des 24 divergences term-def est volontaire, une source unique réinjectée les écraserait) ; (ii) *Taxonomie, 4 catégories en structure plate* : liste plate alphabétique comme ossature, catégorie en simple attribut (cadre / indicateur / concept / anti-pattern), recherche plein texte et filtre par catégorie, les auteurs ne sont pas des entrées. **Reconnaissance d'architecture menée** (agent général) : patron hexagonal du projet confirmé, 5 couches posées au doc compagnon (matière `lexique-source.md` vers générateur `outils/generer-lexique.js` vers zone balisée `CM.LEXIQUE-DATA` vers module `CM.Lexique` et `CM.VueLexique` vers sentinelle `outils/verifier-invariants-lexique.js`). **État des lieux** : environ 190 balises term-def, 142 libellés distincts dont 24 à définitions divergentes (Effet Goodhart 9 variantes, Gemba 6, Deming 4) et quelques non-termes, donc extraction brute exclue, audit éditorial requis. **Découpage en 7 jalons** (A cadrage livré, B audit et source éditoriale, C enrichissement, D générateur, E module et vue, F incarnation visuelle étape B³, G sentinelle et tests), effort estimé 25-35 h sur 7-10 séances. Mémoire `project_chantier_21_ouverture.md` posée.
- **Séance 24/05/2026 (suite), jalon B.0 du chantier 21 livré (3 commits `72cb6f0` `b1df813` plus clôture)** : ouverture du jalon B (audit éditorial du Lexique) par son sous-jalon B.0. Outil `outils/extraire-term-def.js` créé (Node zéro-dépendance, extrait et dédoublonne les balises term-def, codes de sortie 0/1/2) ; il génère `inventaire-term-def-brut.md` (190 balises, 142 libellés distincts, 24 divergents). Inventaire de travail `inventaire-lexique.md` produit : 142 libellés classés en 4 catégories (20 cadres, 20 indicateurs, 43 concepts, 49 anti-patterns) plus 10 non-termes à écarter, fusions et divergences repérées, environ 124 entrées visées. **3 constats collatéraux** : C1 (7 libellés term-def avec backslash parasite), C2 (data-def avec backslash rendus littéralement dans les infobulles, ~159 balises), C3 (titre fautif « Sales Accélération Formula »), tous traités dans le flux du jalon B. **3 arbitrages en attente de Lætitia** : A1 (sort des 10 non-termes face à l'invariant I1, reco retirer leur balise term-def), A2 (validation de la classification), A3 (famille outcome en entrées propres ou fondue). Prochain pas : validation puis jalons B.1 à B.5.
- **Séance 24/05/2026 (suite), décision « une fiche par auteur » (2 commits `5394e32` plus clôture)** : sur question de Lætitia portant sur le contenu réel des infobulles d'auteurs (Deming, Flyvbjerg, Lencioni...), constat qu'elles portent une leçon pédagogique et non une notice biographique. **Décision de Lætitia : donner une fiche à chaque auteur.** La taxonomie du Lexique passe de 4 à 5 catégories, ajout de « Auteurs et figures » (8 entrées : Deming, Dixon, Drucker, Flyvbjerg, Humble, Lencioni, Locke et Latham, Standish Group). Les fragments « Knowledge et Ability » et « Reinforcement » ne sont pas des auteurs, ils fusionnent dans ADKAR. Effet positif : l'invariant I1 de la sentinelle tient sans exclusion, plus de bulle orpheline. **Documents de référence actualisés** : `doc-contrats-lexique.md` passe en v0.2, `inventaire-lexique.md` gagne sa section 5 « Auteurs et figures », arbitrage A1 résolu. Restent A2 (validation de la classification) et A3 (famille outcome). Référentiel du Lexique réestimé à ~134 entrées.
- **Séance 24/05/2026 (suite), jalon B.1 amorcé (3 commits `6309b84` `a34ec74` plus clôture)** : ouverture de la rédaction de `lexique-source.md`, fichier source du Lexique. **Format d'une entrée arrêté** : titre de niveau deux portant l'`id`, champs en marqueurs gras (`**terme**`, `**categorie**`, `**definition**`, `**exemple**`, `**renvois**`), définition multi-paragraphes. **Premier lot du rayon anti-patterns rédigé** : 12 entrées sur 49 (de `acharnement thérapeutique` à `cynisme stratégique`), chacune avec définition canonique, exemple concret et renvois, selon la doctrine « l'entrée explique, elle n'affirme pas ». **Correction qualité en cours de séance** : le doc compagnon `doc-contrats-lexique.md` et le premier jet de `lexique-source.md` avaient été rédigés en français sans accents (lapsus de rédaction repéré et corrigé) ; `6309b84` ré-accentue le doc compagnon (densité passée de 3 à 426 caractères accentués), le lot B.1 a été livré directement accentué. Prochain pas : poursuivre le rayon anti-patterns (37 entrées restantes), par lots, après validation de la voix éditoriale par Lætitia.
- **Séance 24/05/2026 (suite), jalon B.1 livré, rayon anti-patterns complet (commit `babad10` plus clôture)** : voix éditoriale validée par Lætitia sur le premier lot. Les 37 entrées restantes du rayon anti-patterns rédigées dans `lexique-source.md` : le rayon est complet, 49 entrées, chacune avec définition canonique, exemple concret et renvois. Les 3 divergences du rayon résolues en définition unique (KPI individuel, optimisation locale, phantom delivery). Vérifications : 49 entrées, 0 caractère IA-typé, 0 identifiant dupliqué, 703 caractères accentués. Prochain pas : jalon B.2, rayon cadres et méthodes (20 entrées).
...
- **Séance 2026-05-20 (séance 2) — jalon C.3 LIVRÉ ET CLOS + câblage partiel jalon D (`a766b76` → clôture)** : migration de la fiche `resistance-transformation` (variante § 4.4 standard, trio par niveau, panel gestion du changement Kotter / ADKAR / Bridges / Lewin). **Cadrage** : 3 axes stables mental / comportement / durabilité (cartographie ADKAR Awareness+Desire / Knowledge+Ability / Reinforcement) ; cartographie du référentiel via agent Explore ; matrice 9 cellules dont 3 sur de vraies fiches (stratégique `s8·s6·s7`) et **6 en `CADRES_A_VENIR` à nom provisoire** (trios équipe et programme — la gestion du changement n'est cataloguée qu'au niveau stratégique). **Arbitrage Lætitia** : étendre le rendu pour qu'une cellule sans fiche porte un nom provisoire plutôt qu'un encart muet ; doctrine D3 nuancée (doc compagnon § 3.5 point 5, `nom` admis sur les seules cellules `CADRES_A_VENIR`). **Portage** : `a766b76` (rendu — `_composerStandard` propage `card.nom`, `_htmlCard` rend une card nommée « En cours de constitution », CSS), `03487e6` (données — bloc `resistance-transformation`, 9 intentions + 3 recommandations validées par Lætitia, doctrine v0.6), `15dc67f` (doc compagnon v0.8), `c31be03` (doc finalisé après smoke test). **41 assertions node vertes**, non-régression § 4.4 et § 4.6 confirmée. Smoke test interactif Lætitia validé, **tag `mvp-chantier-c3-livre`** posé. **Friction post-smoke traitée** (`3a28ddd`) : les cellules Comportement et Durabilité du trio programme reposaient sur les fiches qualité `p6`/`p7` détournées, dont le nom ne disait pas le changement et divergeait du trio équipe ; remplacées par 2 indicateurs pensés pour l'échelle programme (« Part des équipes ayant adopté la nouvelle pratique », « Part des équipes retombées à l'ancienne pratique ») en `CADRES_A_VENIR` à nom provisoire ; 19 assertions node vertes ; doc compagnon corrigé (`7abec99`). **Deux travaux connexes même séance** : (i) **`8522f24` — câblage partiel du jalon D** : le panneau « questions terrain » de l'accueil pointait vers les anciens mockups autonomes ; les 4 fiches déjà migrées (pastèque, pilotage-hebdo, amélioration-continue, résistance-transformation) sont reliées à leur version intégrée via la route `#fiche-q=` ; cascade-objectifs reste sur son mockup jusqu'à C.4. (ii) **`c514943` — fix titre pilotage-hebdo** : divergence entre l'index d'accueil et le champ `question` de la fiche depuis C.1, exposée par le câblage ; arbitrage Lætitia : le libellé d'accueil (« Quoi mettre dans mon pilotage hebdomadaire ? ») fait référence, le champ `question` s'y aligne. **Dossier de travail nettoyé en ouverture de séance** : 30 fichiers obsolètes (`.todelete` + 2 mockups de traçabilité) et 302 artefacts `.git/*.lock*` (dont 2 verrous orphelins) supprimés.
- **Séance 2026-05-23 — jalon C.4 LIVRÉ ET CLOS, jalon § 4.4 terminé (4 commits `fcaaa96` → `b3604d6` + clôture)** : migration de `cascade-objectifs`, dernière des 5 fiches-questions (variante § 4.4 standard, famille Drucker). **Cadrage** : reprise du mockup du 07/05, axes reformulés avec Lætitia en **Alignement / Précision / Accompagnement** — « Précision » absorbe deux défauts ajoutés par Lætitia (objectif flou, capteur inadapté), « Accompagnement » élargit l'ancien « Dialogue » à la mise en place. **Q1 passe de 5 à 7 maillons** : ajout de *objectifs trop nombreux* (→ Alignement, concentration de Drucker), *flous* et *capteur inadapté* (→ Précision), *sans accompagnement* (→ Accompagnement) ; répartition 3/2/2. **Constat de cartographie** : la discipline de cascade MBO n'est cataloguée à aucun niveau du référentiel — les **9 cellules sont en `CADRES_A_VENIR` à nom provisoire** (parti validé, cohérent avec la doctrine D3 et la friction C.3). **Portage sans changement de code** (moteur § 4.4 générique depuis C.2) : `fcaaa96` (données — bloc catalogue, 9 intentions + 3 recommandations, panel Drucker MBO), `0d903bb` (câblage — la tuile d'accueil pointe vers `#fiche-q=cascade-objectifs`), `08976e8` (doc compagnon v0.9). **Friction de smoke test traitée** (`b3604d6`) : les 9 noms provisoires se ressemblaient trop (3 concepts transposés sur 3 échelles) ; réécrits avec 9 mots de tête distincts, les 9 intentions inchangées. 64 assertions node vertes. Smoke test interactif Lætitia validé. **Constat collatéral `26.c4.coll.1` ouvert** : créer au référentiel les 9 indicateurs de discipline de cascade MBO + la fiche-cadre Management par objectifs. **Tag `mvp-chantier-c4-livre`.**
- **Séance 2026-05-23 (suite) — mini-chantier gestion du changement LIVRÉ (volet du constat collatéral n°10), 6 commits `7e88780` → `345fa15`** : création du cadre `change` (Gestion du changement) et de la famille `conduite-du-changement` (aucune des 9 familles existantes n'y convenait ; précédent assumé des familles à cadre unique), retag de s6/s7/s8 `generique` → `change` (décision Lætitia). **6 fiches d'indicateurs `cg-1` à `cg-6`** créées au référentiel au standard éditorial 2026-04, panel Kotter / ADKAR / Bridges / Lewin, nommées par les cellules `CADRES_A_VENIR` de `resistance-transformation` (trios équipe et programme). Tags thématiques régénérés via `outils/generer-tags-thematiques.js` (6 lignes à la matrice §6). **`resistance-transformation` entièrement adossée au référentiel** : ses 6 cellules `CADRES_A_VENIR` échangent leur nom provisoire contre le `ficheRef` réel `cg-*` (doctrine D3 point 5) ; le champ `nom` provisoire est retiré. Référentiel porté de 89 à 95 fiches, invariants `journal-invariants-23g.md` réactualisés (I1 → 95, I2 → 12). Vérifications vertes : cohérence du vocabulaire (13 cadres alignés), 63/63 requête-métriques, parse-check des harnais. Smoke test interactif Lætitia validé. **Point laissé de côté** : les 3 liens sortants de `resistance-transformation` indiquent encore « en cours de constitution » ; brancher un lien sortant de fiche-question vers une vue d'ensemble de cadre est une décision transverse (vaut aussi pour `cascade-objectifs` → MBO), à traiter à part. **Tag `mvp-cadre-changement-livre`.**
- **Séance 2026-05-23 (suite) — volet MBO du mini-chantier LIVRÉ (constat `26.c4.coll.1`), 5 commits `2ade758` → `0884f52`** : promotion du cadre `mbo` (« Management par objectifs ») de `CADRES_A_VENIR` vers les cadres actifs, famille `objectifs` inchangée. **9 fiches d'indicateurs `mbo-1` à `mbo-9`** créées au référentiel au standard éditorial 2026-04, panel Drucker / Locke & Latham / Humble, nommées par les cellules `CADRES_A_VENIR` de `cascade-objectifs` (trios équipe, programme, organisation). Tags thématiques régénérés. **`cascade-objectifs` entièrement adossée au référentiel** : ses 9 cellules échangent leur nom provisoire contre le `ficheRef` réel `mbo-*` (doctrine D3 point 5). Référentiel porté de 95 à 104 fiches, invariants réactualisés (I1 → 104, I2 → 15). Vérifications vertes (14 cadres alignés, 63/63 requête-métriques, parse-check des harnais). Smoke test interactif Lætitia validé. **Le mini-chantier éditorial du constat n°10 est entièrement bouclé, ses deux volets livrés.** **Tag `mvp-cadre-mbo-livre`.**
- **Séance 2026-05-23 (suite) — jalons D et E livrés, CHANTIER 26 CLOS (6 commits `74ece40` → `30e025c`)** : **jalon D** retire les 5 mockups autonomes de fiches-questions (`74ece40`) et archive la doctrine « autonome = source unique » (`0fde478`) ; `mockup-format-fiche-question.html` conservé (gabarit de format encore cité dans deux docs compagnons, décision Lætitia). **Jalon E** livre la sentinelle `outils/verifier-invariants-fiche-question.js` (`dadb638` — invariants I1-I5 du catalogue `CM.FicheQuestion`, patron `verifier-coherence-vocabulaire.js`, remplace les scripts node ad hoc des séances C.x) et actualise `scenario-non-regression.md` (`30e025c` — 104 fiches, §3 réécrit, nouveau §12 fiches-questions). Suite de vérification automatique verte (14 cadres alignés, 5 fiches d'invariants, 63/63 requête-métriques, parse-check des harnais d'accueil). Smoke test global Lætitia validé. **Chantier 26 entièrement clos** : les 5 fiches-questions intégrées au catalogue, adossées au référentiel (104 fiches), testées ; jalons A à E livrés, mini-chantier du constat n°10 bouclé. **Tag `mvp-chantier-26-livre`.**
- **Prochaine action recommandée** : deux pistes ouvertes, Lætitia tranchera l'ordre à la reprise. **Piste 1 — chantier 21 (Vue Lexique)** : jalon B.1 livré (rayon anti-patterns complet, 49 entrées dans `lexique-source.md`), prochain pas jalon B.2 (rayon cadres et méthodes, 20 entrées, même format), puis indicateurs, concepts et auteurs (jalons B.3 à B.5). Arbitrages A2 (classification) et A3 (famille outcome) portés avec recommandation, corrigeables à la relecture. Référence : `doc-contrats-lexique.md`. **Piste 2 — retours du test utilisateur n°1** : valider la table de ventilation de `test-utilisateur-1.md`, puis ventiler les 26 retours dans le backlog (option 1B). Autres pistes en attente : 24, 13.bis, 25. Tag de point de retour courant : `mvp-chantier-26-livre`.
- **Blocages / questions ouvertes** : (1) table de ventilation des 26 retours du test utilisateur n°1 à valider, voir `test-utilisateur-1.md` ; (2) ordre d'implémentation à trancher entre finir le chantier 21, prioriser la proposition de valeur de l'accueil, ou une passe de corrections rapides.
...
- **`change-management`** (ADKAR Prosci / Kotter) — ✅ **ADRESSÉ le 23/05/2026** : cadre `change` (Gestion du changement) créé, famille `conduite-du-changement`, 6 fiches `cg-1` à `cg-6`, retag de s6/s7/s8. Voir la séance du 23/05 en tête de backlog.
...
**Constats collatéraux ouverts à statuer** : n°8 (pauvreté du corpus Scrum dans le référentiel), n°10 (4 pistes cadres restantes : sales, pmi-evm, grc-compliance, hr-analytics — piste change-mgmt adressée le 23/05/2026). *[n°1, n°4 et n°9 clos le 01/05/2026 ; n°6 clos le 01/05/2026 ; n°2, n°3, n°11, n°12 clos antérieurement]*
...
- *Taxonomie, 5 catégories en structure plate.* Liste plate alphabétique comme ossature, catégorie en simple attribut (cadre / indicateur / concept / anti-pattern / auteur), pas d'arborescence. Recherche plein texte et filtre par catégorie. La 5e catégorie « Auteurs et figures » a été actée le 24/05/2026 après le jalon B.0 : les infobulles posées sur des noms d'auteurs portent une leçon pédagogique, elles méritent une entrée.
...
| B | Audit et source éditoriale : résoudre les 24 divergences, écarter les non-termes, classer en 5 catégories, rédiger `lexique-source.md`. Découpé B.0 à B.5. Le jalon le plus lourd. | 🟡 en cours, B.0 et B.1 livrés (rayon anti-patterns, 49 entrées) |
...
**Prochaine action recommandée.** Jalon B.1 livré : `lexique-source.md` contient le rayon anti-patterns complet (49 entrées). Prochain pas : jalon B.2, rayon cadres et méthodes (20 entrées), puis indicateurs, concepts et auteurs (jalons B.3 à B.5).
...
### 26.c4.coll Constats collatéraux du jalon C.4 *(ouverts le 23/05/2026)*

Items levés pendant le jalon C.4 (migration de `cascade-objectifs`). Hors scope du jalon en cours.

| Item | Description | Quand traiter |
|---|---|---|
| **26.c4.coll.1** *(dette de référentiel)* — ✅ **ADRESSÉ le 23/05/2026** (cadre `mbo` promu, 9 fiches `mbo-1` à `mbo-9`, `cascade-objectifs` câblée, tag `mvp-cadre-mbo-livre`) | **Les 9 indicateurs de discipline de cascade MBO étaient absents du référentiel, ainsi que la fiche-cadre Management par objectifs.** Origine : au cadrage C.4, la cartographie du référentiel a montré qu'aucun indicateur de discipline de cascade (objectif relié à la mission, objectif clair et mesurable, cadence de revue) n'existe, à aucun niveau ; le cadre MBO est en `CADRES_A_VENIR` depuis le chantier 1.ter. Les 9 cellules de `cascade-objectifs` portent donc un nom provisoire. À créer au référentiel : trio équipe (*Part des objectifs de l'équipe rattachés à la mission*, *Clarté et mesurabilité des objectifs de l'équipe*, *Fréquence des points objectifs manager-collaborateur*), trio programme (*Cohérence des objectifs d'équipe avec les objectifs du programme*, *Lisibilité partagée des capteurs entre équipes*, *Cadence des revues d'alignement inter-équipes*), trio organisation (*Concentration des objectifs stratégiques sur la mission*, *Couverture des objectifs stratégiques par un résultat clé mesurable*, *Régularité du réexamen stratégique des objectifs*), plus la fiche-cadre Management par objectifs (Drucker, Humble). Même logique que le constat collatéral n°10 (gestion du changement). | ✅ Livré le 23/05/2026 |
| **26.c4.coll.2** *(éditorial — lien entre fiches-questions)* | **Le `crossLink` `cascade-objectifs` ↔ `pasteque` relie deux fiches de sens opposés sans le dire.** Observation de Lætitia (23/05/2026) : `cascade-objectifs` traite le sens **descendant** (faire descendre les objectifs de la mission jusqu'au terrain, garantir l'alignement top-down) ; `pasteque` traite le sens **montant** (vérifier que l'information qui remonte du terrain est fidèle, bottom-up). Les deux forment la boucle verticale complète du pilotage — cascader sans vérifier la remontée produit justement l'effet pastèque. Le `crossLink` actuel n'affiche que le titre de l'autre question, il est muet sur cette complémentarité. À examiner : enrichir le libellé (des deux côtés, le lien est réciproque) pour nommer les deux sens du même dialogue vertical. | À statuer avec Lætitia |
