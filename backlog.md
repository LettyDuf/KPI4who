...
Dernière mise à jour : **24 avril 2026 — fin de journée** (ouverture étape (b) du chantier 14, tâche 1 tranchée : Option B seuils/paliers retenue. Reste à traiter : tags thématiques + signature `executer(filtre)`).
...
- **Chantier actif** : **chantier 14 — refactor hexagonal**. Étape (a) close. Étape (b) en cours : tâche 1 tranchée (Option B seuils/paliers actée dans `doc-contrats-chantier-14.md` §10.1, commit `21c76b2` mockup + commit documentaire à suivre). Tâches restantes : **(2) arrêter le vocabulaire fermé des tags thématiques** (10-14 tags parmi les 14 candidats) ; **(3) poser la signature `executer(filtre)`** dans le document compagnon. Puis extraction du module `CM.RequeteMetriques` + tests unitaires via patron générateur Node.
...
- **Dernier SHA sur `main`** : `21c76b2` (mockup seuils/paliers étape (b)) (annotations chantier 10). Précédents de la session : `bffc987` (chantier 16), `715673d` (chantier 15). Précédent de la session clôture étape (a) : `1ba5112`. **Tag git** `baseline-avant-hexagonal` = `5655b03`, point de retour absolu. Prochain tag à poser : `mvp-etape-a-schema-inventorie` sur `1ba5112` (reste à faire au prochain passage en étape b).
