...
- **Prochaine action recommandée** : choisir le prochain chantier parmi les **chantiers ouverts cumulés** restants : **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **22 (harmonisation tutoiement)**, **24 (URLs partageables des filtres)**. Trois **points reportés** restent en attente : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher), (2) mission de l'accueil unifié à insérer au-dessus du h2 épuré, (3) CSS mort `.porte-statut` (~15 lignes mortes à retirer dans un commit cosmétique léger). Aucun blocage actif sur l'outil. Suggestion d'ordre pour une prochaine séance : (a) commit cosmétique CSS mort `.porte-statut` (~10 min, gain technique propre), (b) mission de l'accueil unifié (chantier court, refait l'en-tête de la page), puis attaquer un chantier majeur restant.
