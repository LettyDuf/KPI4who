...
- **Événements majeurs de la séance (06/05/2026 journée)** : (1) **décision 20.1 close en deux temps** — palette **D bleu de Prusse `#1d3a5f`** retenue après audit visuel cross-niveaux (B vert profond confondait avec 2 niveaux, D avec 1 seul). Doctrine § 6.5 et § 4.5 actualisées. La doctrine du filet de sécurité a joué à la lettre (un seul commit token). (2) **Dette de référentiel close** — 4 fiches métriques manquantes créées sur patron éditorial enrichi (modèle `p6`/`s10`) : `lss-1` First-pass yield (équipe), `p9` Lead time (programme), `s11` COPQ (stratégique), `s12` Écart de vérité (stratégique). Nouveau préfixe `lss-*` introduit pour les indicateurs Lean Six Sigma orthodoxes (cohérent avec `ti-*` / `af-*`). Lien Premier passage conforme réactivé dans la fiche-question d'origine.
- **20 commits sur la journée** : matin palette B (10 commits) — `72a7451` … `f53d4b5` ; après-midi audit + bascule D (6 commits) — `b3d99fb` `cb5a353` `575a64a` `c64187c` `e89db68` `3c6632a` `a32054b` ; soirée référentiel (4 commits) — `9ba4f42` (lss-1) · `f8bf26d` (p9) · `d9d70c0` (s11) · `2afa2ac` (s12) · `bef3ae7` (lien réactivé fiche-question).
- **Sentinelles relancées** : `verifier-coherence-vocabulaire.js` — tous les vocabulaires alignés (12 cadres, 7 tags, 4 niveaux…) ; `lancer-tests-requete-metriques.js` — 63 tests verts. Aucune régression.
- **Mémoires touchées cette séance** : `project_palette_fiche_question_actee.md` mise à jour (D retenue, B en réserve, règle permanente ajoutée : produire systématiquement un mockup cross-niveaux avant tout arbitrage de palette d'accent secondaire).
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges · 20.3 volumétrie panels · 20.4 responsive double réponse · 20.5 rédaction MVP (**1/8-12 fiches livrées** — la première est désormais complète, ses 3 cards pointent toutes vers une fiche existante) · 20.6 trio alternatif v2 · 20.7 fiche-question méta véracité/pastèque (alimentée par `s12` Écart de vérité créée aujourd'hui) · 20.8 cross-link.
- **Dette détectée pendant la séance** : filets de niveau **équipe `#6fa89e` (2.70:1)** et **tactique `#c79373` (2.68:1)** sous-passent le seuil 3:1 WCAG SC 1.4.11 sur fond blanc/crème comme composants graphiques porteurs d'information. Programme `#7d9670` limite. Ouvert dans § 2 *UX à vérifier*.
- **Décisions du chantier 10** : ✅ D1, D2 (V3), D5 fermées ; ⏳ D3 (mode fin), D4 (responsive), D6 (deep-linking).
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→W` accumulées en attente côté ordi.
- **Prochaine action recommandée** : (a) **rédiger une seconde fiche-question** d'un autre panel (20.5 — alimente le MVP, le référentiel d'indicateurs est désormais consolidé pour cet exercice) ; (b) **rédiger la fiche-question méta véracité/pastèque (20.7)** — l'élément `s12` Écart de vérité est désormais disponible comme fiche cible ; (c) **traiter la dette filets niveau** détectée par l'audit AA (item § 2). Trois pistes : foncer d'un cran, accepter, doubler par redondance textuelle.
