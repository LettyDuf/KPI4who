...
- **Événements majeurs de la séance (07/05/2026 soir)** : (1) **4<sup>e</sup> fiche-question MVP** *« Par où démarrer une démarche d'amélioration continue ? »* (panel Coach Lean) autonomisée depuis le mockup format — doctrine *autonome = source unique, mockup-format pointe* actée. (2) **Mockup-format scène 1 → miroir didactique** + **cross-link pasteque ↔ amélio continue** réciproque effectif. (3) **5<sup>e</sup> fiche-question MVP** *« Pourquoi ma transformation résiste ? »* (panel Gestion du changement — Kotter / ADKAR / Bridges / Lewin) livrée puis itérée 2 fois (v2 « équipe pilote » → « en transformation », v3 sigles ADKAR + anglicismes nettoyés). **Premier test § 4.4 hors couloir Lean-Agile-Drucker concluant.** ADKAR utilisé comme grille pédagogique de Q1, mots français dans les cards. (4) **6<sup>e</sup> fiche-question MVP** *« Comment cascader des objectifs jusqu'au terrain ? »* (panel Coach exécutif Drucker MBO orthodoxe — Drucker / Locke & Latham / Humble) livrée en prolongation. **Famille Drucker fermée** : Drucker méta (pasteque) côté observation du déclaratif + Drucker MBO (cette fiche) côté mécanique de la cascade. Trio cartographique Alignement / Capteurs / Dialogue. Validée par Lætitia sans retouche.
- **8 commits sur la séance soir** : `9eaf704` (4e fiche autonome) · `613f46f` (mockup-format → miroir) · `f5cf068` (pasteque réciproque actée) · `6b39cad` (5e fiche v1) · `07aa693` (v2 équipe en transformation) · `6ad97bf` (v3 français orthodoxe) · `01796ee` (clôture intermédiaire après les 5 premières fiches) · `2f4f179` (6e fiche cascade objectifs MBO, livrée en prolongation).
- **Mockups produits cette séance** : `mockup-fiche-question-amelioration-continue.html` (457 lignes, palette D, § 4.4 trio par niveau, panel Coach Lean) · `mockup-fiche-question-resistance-transformation.html` (489 lignes v3, palette D, § 4.4 trio par niveau, panel Gestion du changement, anatomie enrichie d'un sur-libellé `.indicateur-phase`) · `mockup-fiche-question-cascade-objectifs.html` (482 lignes, palette D, § 4.4 trio par niveau, panel Coach exécutif Drucker MBO, sans champ `traduction` car MBO sans signature anglaise patrimoniale).
- **Avancement chantier 20.5** : **MVP CLOS à 6/8-12 fiches livrées — option A retenue (07/05/2026 séance nuit)**. Sous-cible doctrinale assumée (cible basse 8). Panels couverts : Coach Lean (2 — amélio continue + scène 1 originelle), Drucker méta (1 — pasteque), Coach Agile reformulation (1 — pilotage hebdo), Gestion du changement (1 — résistance), Coach exécutif Drucker MBO (1 — cascade objectifs). **Famille Drucker fermée.** Panel à 0 *Terrain des autres cadres* — assumé. **Raison documentée** : matière éditoriale explorée (panel ITIL+DORA+Goldratt élargi à Chaos Engineering), coûts cachés révélés à l'examen — (i) cible utilisateur DSI/SRE minoritaire pour le lectorat de l'outil, (ii) dérive doctrinale par ajout silencieux de Chaos Engineering au panel canonique *Terrain des autres cadres* (DORA/SPACE, BSC, OKR Doerr, ITIL/ITSM, EFQM), (iii) dette de référentiel passant de 11 à ~17 fiches manquantes pour gagner 1 à 2 fiches-questions. Garde-fou 16.2 *anti-encyclopédie — le manque utilisateur nommé* a tranché. Place gardée dans le panel pour un angle qui remontera nommément depuis le terrain.
- **Mémoires actualisées cette séance** : `feedback_q2_resemantiser_par_panel.md` (libellés Q2 à resemantiser au domaine du panel — pasteque + résistance, 2 occurrences) · `feedback_sigles_cadres_pas_dans_cards.md` (enseigner ADKAR/MBO/OKR dans Q1, parler français dans les cards) · `feedback_champ_traduction_reserve_signatures.md` (le `<span class="traduction">` est patrimonial, pas un libre-service à anglicismes).
- **Décisions ouvertes restantes du chantier 20** : 20.2 termes-pièges (5 identifiés à étoffer) · 20.3 volumétrie panels · 20.4 responsive · 20.6 trio alternatif v2.
- **Dette de référentiel cumulée** : (issue pastèque) 2 fiches déclaratives + 1 fiche cadre triangulation ; (issue résistance) 3 fiches métriques absentes (Score conscience-désir, Taux d'adoption, Fréquence régressions) + fiche cadre *Gestion du changement* (Kotter / ADKAR / Prosci / Bridges / Lewin) — constat collatéral n°10 du chantier 1.ter ; (issue cascade objectifs) 3 fiches métriques absentes (Taux d'objectifs reliés à la mission, Taux d'objectifs assortis d'un capteur, Cadence de revue manager-collaborateur) + fiche cadre *Management By Objectives* (Drucker / Humble) — actuellement en `CADRES_A_VENIR` (constat collatéral n°7 du chantier 1.ter), réintégration motivée par cette livraison.
- **Dette UX antérieure (§ 2)** : filets niveau **équipe `#6fa89e` (2.70:1)** et **tactique `#c79373` (2.68:1)** sous-passent 3:1 WCAG SC 1.4.11. Statut inchangé.
- **Vision MVP global v1 actée (07/05/2026 séance nuit)** : page d'accueil 3 portes + lien vers les 6 fiches-questions + bandeau du haut fonctionnel avec tableau de bord + impression. Effort 10-13h sur 3-4 séances. **Ordre 2 retenu** : (1) accueil 3 portes (~1h, squelette `#accueil-portes` existe déjà avec 3 portes Disponibles + 1 Bientôt, basculer l'accueil par défaut de pyramide → portes), (2) dette T1 chantier 9 (câbler voies d'ajout au panier — moteur `CM.Panier.ajouter` existe, aucun appelant UI sur les fiches, ~4-6h), (3) **option β retenue** — 5e porte *Par ma question* intégrée au HTML principal sur le modèle de *Par mon problème*, qui liste les 6 fiches-questions cliquables avec rendu en iframe ou redirection propre vers les mockups (~4h), (4) tests de bout en bout + finitions impression (~2h). La pyramide hiérarchique existante reste vivante en accueil alternatif accessible par lien de pied. Le chantier 10 (refonte navigation à 8 onglets) reste en attente — cette vision MVP est un sous-ensemble minimal qui ne refait pas la navigation.
- **Décisions du chantier 10** : ✅ D1, D2 (V3), D5 ; ⏳ D3 (mode fin — partiellement adressé par option β du MVP global v1), D4 (responsive), D6 (deep-linking).
- **Workflow git sandbox** : pattern `mv .git/index.lock` (+ `HEAD.lock`) toujours actif. Paires `.git/*.lock.tmp.A→X` accumulées en attente côté ordi.
- **Prochaine action recommandée** : suivre le plan MVP global v1 dans l'ordre 2. **Étape 1 — Activer l'accueil 3 portes par défaut** (~1h) : (a) basculer la fonction d'affichage de l'accueil dans `cadre-indicateurs.html` pour montrer `#accueil-portes` au lieu de la pyramide à l'ouverture, (b) ajouter un lien de pied dans l'accueil portes vers la pyramide (lien réciproque : la pyramide a déjà son lien vers les portes), (c) tester continuité de navigation — chaque porte ouvre sa vue, retour accueil revient sur les portes, refresh F5 atterrit sur les portes. Décision parallèle à prendre au passage : la 4e porte *Par ma maturité* en *Bientôt* lisible (mon avis) ou masquée pour le MVP. **Étape 2** — câbler dette T1 chantier 9. **Étape 3** — porte option β *Par ma question*. **Étape 4** — tests bout en bout. Travaux *en parallèle* possibles si énergie disponible : résorber la dette de référentiel (11 fiches manquantes, voir État courant ligne dette) — ces fiches manquantes deviennent visibles dès que les 6 fiches-questions sont exposées (étape 3), donc l'urgence montera après l'étape 3 ; traiter la dette filets niveau (§ 2) ; engager 20.4 responsive ; ouvrir 20.6 trio alternatif v2.
