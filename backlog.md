...
Dernière mise à jour : **30 avril 2026 — chantier 1.ter 4 cadres clos** (`safe`, `lean`, `itil`, `dora`) + extension taxonomie (3 nouveaux cadres `securite`/`cd`/`space`, nouvelle famille `risque-securite`). Plus tôt : chantier 1.ter 3 cadres clos (`safe`, `lean`, `itil`). Plus tôt : chantier 1.ter cadres `safe` et `lean` clos (5+14 conservées, 6+15 retirées). Plus tôt : chantier 1.ter cadre `safe` clos (5 conservées, 6 retirées, t4 enrichi). Plus tôt : chantier 1.ter ouvert (audit légitimité tags cadres, 10 cadres × 85 fiches × 144 occurrences). Plus tôt : axe Stratégique de 1.bis clos (1 phrase corrigée sur `s9`, term-def sur `COSO ERM` + `ISO 31000`, commit `ba690cb`). Méthode d'audit corrigée (vérification d'inclusion dans un span term-def) à reprendre pour les axes Tactique / Programme / Op TI / Op Affaires. Plus tôt : dette `tests-porte-niveau.html` éteinte (refonte hors-iframe via générateur Node, 9 suites au vert). Tag `mvp-dette-tests-porte-niveau-eteinte` sur `bc3042d`. Plus tôt dans la journée : chantier 9.F clos (libellé bouton « 🖨 Imprimer / PDF », tag `mvp-9f-libelle-pdf` sur `e8649e9`).
...
- **Événements majeurs de la séance (4)** : **(1) axe Stratégique de 1.bis clos** — 1 phrase corrigée sur `s9` (term-def `COSO ERM` + `ISO 31000`, commit `ba690cb`) ; le diagnostic initial surévaluait la dette d'un facteur ~4. Méthode d'audit corrigée actée. **(2) Chantier 1.ter ouvert** — audit légitimité des tags `cadres` sur tout le référentiel, après remontée de Lætitia : sur 11 fiches taggées `safe`, 4 légitimes, 3 ambiguës, 4 abusives. Périmètre : 10 cadres définis, 85 fiches mappées (sur 137), 144 occurrences. Méthode d'audit standardisée écrite, ordre d'attaque safe → lean → itil → dora → scrum → kanban → mbo → bsc → okr → generique. **(3) Chantier 1.ter — 2 cadres clos** : (a) Cadre `safe` — sur les 11 fiches taggées, 5 conservées (t6, t7, p1, p5 légitimes par mention LPM/PI Planning ; t4 enrichi avec mention Flow Metrics SAFe v6), 6 retirées (t1, t2, t5, p2, p3, p4 sans appui doctrinal). 2 commits code (`a3f9871` retraits META, `e81b179` enrichissement t4) + 1 commit doc de clôture (`84e0695`). (b) Cadre `lean` — sur les 29 fiches, 14 conservées (toutes signature canonique : Lead Time, Throughput, Cycle Time, WIP, OEE, DPMO, Flow Efficiency, etc.), 15 retirées (DevOps/HR/CES sans appui doctrinal + ambiguës où Lean n'était que garde-fou transversal). 1 commit refactor (`c082a4c`) + 1 commit doc de clôture (`4cd000d`). (c) Cadre `itil` — sur les 16 fiches, 10 conservées (toutes signature ITIL Service Op / Availability / Problem Mgmt), 6 retirées (p3 PMI ; af-sc1/sc2/sc3 CX Reichheld/Dixon ; ti-s1/ti-s3 sécurité hors corpus ITIL). 1 commit refactor (`3bcbe6c`) + 1 commit doc. **Constat collatéral n°2 ajouté** : taxonomie sans cadre sécurité (NIST CSF/ISO 27035) alors que le référentiel a ≥5 fiches sécurité. (d) Cadre `dora` — sur les 13 fiches, 4 conservées (les 4 keys), 9 retirées (sujets adjacents DevOps/CI/SRE). **Extension taxonomie** : 3 cadres ajoutés (`securite`, `cd`, `space`) + famille `risque-securite`. 13 fiches retaggées avec cadres signature stricte. **Constats collatéraux n°2 et n°3 ADRESSÉS** dans la même session. 3 commits : `662682a` ajout cadres + famille, `d1491a7` retraits dora + re-attributions, doc backlog ci-après. Audit méthodique mené avec une méthode corrigée (vérifier la couverture par `term-def` au premier usage hors `data-def`, et pas seulement compter les occurrences brutes). Résultat : **1 phrase à corriger** sur la fiche `s9` champ `objectif` — ajout de term-def sur `COSO ERM` et `ISO 31000` (définitions reprises mot-à-mot de `s10` pour cohérence cross-fiches). 1 commit code (`ba690cb`). Constat de méthode : le diagnostic initial du backlog 1.bis surévaluait la dette d'un facteur ~4 sur cet axe — la même méthode d'audit corrigée doit être appliquée aux 4 axes restants.
