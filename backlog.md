...
- **Avancement chantier 20.5** : 6/8-12 fiches MVP livrées. Panels couverts : Coach Lean (2 — amélio continue + scène 1 originelle), Drucker méta (1 — pasteque), Coach Agile reformulation (1 — pilotage hebdo), Gestion du changement (1 — résistance), Coach exécutif Drucker MBO (1 — cascade objectifs). **Famille Drucker fermée.** Panel encore à 0 : Terrain des autres cadres (BSC, OKR Doerr, ITIL — 1 à 3 fiches selon arbitrage).
...
- **Décisions du chantier 10** : ✅ D1, D2 (V3), D5 ; ⏳ D3 (mode fin), D4 (responsive), D6 (deep-linking).
...
- **Prochaine action recommandée** : (a) **clore le MVP** sur le dernier panel à 0 — Terrain des autres cadres (BSC / OKR Doerr / ITIL). Arbitrage à faire : 1 fiche transversale ou 3 fiches (une par cadre) ? ; (b) **résorber la dette de référentiel** (8 fiches à créer dont 3 fiches cadre — *Gestion du changement*, *MBO*, *triangulation déclaratif/objectif/humain*) ; (c) **traiter la dette filets niveau** (§ 2) ; (d) **engager la décision 20.4 responsive** maintenant que 6 fiches MVP existent comme matière de test ; (e) **ouvrir la décision 20.6 trio alternatif v2**.
...
| 20.5 | **Rédaction des 8 à 12 questions MVP** | 🟡 en cours — **3 fiches livrées** : (1) *Par où démarrer une démarche d'amélioration continue ?* (panel Coach Lean, variante standard, 05/05/2026, mise en mockup à venir) ; (2) *Mes indicateurs reflètent-ils la réalité du terrain ?* (panel Drucker / Toyota Way / Edmondson / DORA-SPACE, variante méta § 4.6, 07/05/2026, commit `f5fb2df`) ; (3) *Comment mesurer l'occupation hebdomadaire de mon équipe ?* (panel Coach Agile, variante à reformulation § 4.3, 07/05/2026, commit `ffba1ae` v1 puis `528d08f` v2 après reformulation — la question initiale *« Quoi mettre dans mon pilotage hebdomadaire ? »* était ouverte, incompatible avec § 4.3 ; reformulée pour exposer le terme-piège *occupation*). **Reste 5 à 9 fiches MVP**. Panels encore à 0 : Coach exécutif (Drucker MBO), Gestion du changement (Kotter, ADKAR), Terrain des autres cadres (BSC, OKR Doerr, ITIL). **Question candidate ajoutée** : *« Quoi mettre dans mon pilotage hebdomadaire ? »* à reverser comme fiche standard (§ 4.4) avec trio par niveau si le besoin se confirme — déversée du chantier 20.5 lors de la reformulation 07/05 |
