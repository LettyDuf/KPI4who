...
- **État de l'outil après reprise 09/05/2026 (`58a5dca`)** : trois changements depuis `ed52a23` — (a) retrait CSS mort `.porte-statut` (orphelin, sans usage, `35bbc61`), (b) ajout de la mission « manifeste deux étages » au-dessus du h2 de l'accueil (`378634d`, +41 lignes HTML+CSS), (c) doctrine § 6.1 strate 2 mise à jour (`58a5dca`, +15 lignes doc-cadre-visuel.md). Smoke test interactif à reprendre côté Lætitia : ouvrir l'accueil et vérifier le rendu de la mission deux étages. Le reste des invariants I1-I6 du `journal-invariants-23g.md` reste valide (aucune modification du référentiel ni du moteur de filtrage).
...
- **Prochaine action recommandée** : ordre validé par Lætitia en reprise 09/05/2026 — (a) **commit cosmétique CSS mort `.porte-statut`** ✅ livré (`35bbc61`, 15 lignes), (b) **mission de l'accueil unifié** ✅ livré (`378634d` code + `58a5dca` doc, composition Beta « manifeste deux étages »), (c) **chantier majeur 22 — harmonisation tutoiement** (priorité car la voix de l'outil est la plus visible juste après l'accueil unifié ; toute fiche écrite entre-temps fige le mauvais ton) — à attaquer en prochaine séance. Un seul **point reporté** restant : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher). Chantiers ouverts cumulés restants après 22 : **21 (vue Lexique)**, **13.bis (chips cumulables maturité — en attente du retour de la maturité au périmètre actif)**, **24 (URLs partageables des filtres — en attente trigger)**. Aucun blocage actif sur l'outil. Mockup d'arbitrage `mockup-mission-accueil.html` v6 conservé à la racine (référence pour la doctrine de la mission).
