...
- **Prochaine action recommandée** : séance 13-15/05/2026 close à `5760306` après 4 commits cumulés. Chantiers ouverts en attente d'arbitrage utilisateur, à reprendre en priorité : **(II) Option A pour le tableau de bord** — basculer `_htmlCartePanier()` vers le format unifié `htmlCarte()` (avec X retirer à la place des boutons panier ✓ / 💡), engagement pris le 09/05 mais non livré. Effort ~30-45 min. **Point reporté restant** : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher). **Autres chantiers ouverts cumulés** : **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables des filtres)**, **25 (audit zones inactives)**. ~~(I) Audit typographique du référentiel~~ ✅ **clos le 15/05** (I.a accents + I.b em dash, 4 commits cumulés, voir § 1.quater du backlog). Aucun blocage actif sur l'outil.
