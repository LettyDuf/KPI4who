...
| 7.2a | **Stepper générique + porte niveau en accordéon à 4 cartouches (architecture)** | 🟡 | *Scindé de l'ex-7.2 le 21/04/2026 après-midi.* Volet **architecture**. Extraire un `CM.Stepper` générique qui accepte un tableau ordonné d'étapes (nombre variable, 3/4/5+ — exigence d'extensibilité validée le 20/04/2026 pour accueillir 7.3 et 7.4 sans réécriture). Porte *niveau* rejoint le parcours guidé avec ordre **niveau → problème → cadre → résultats** ; étape 1 sous forme d'accordéon à 4 cartouches (Opérationnel / Tactique / Portefeuille / Exécutif — libellés de surface, `META.niveau` interne inchangé). Chaque cartouche liste 3 à 4 rôles canoniques cliquables (Scrum Master, PO, RTE, DG…). Clic sur le rôle = validation étape 1, état `{ niveau, role }` capturé. Décisions N1 (double-libellé) / R2 (rôles répétés si à cheval) / C2 (clic sur rôle) actées ; grille D des descriptifs (humaine/projet vs quatre axes Mintzberg) à arbitrer sur mockup-preview avant phase code. Voir `project_porte_niveau_roles.md`. | ⏸ |
| 7.2b | **Porte niveau — passe rédactionnelle complète sur les rôles (éditorial)** | 🟡 | *Scindé de l'ex-7.2 le 21/04/2026 après-midi.* Volet **éditorial**. Inventaire exhaustif des rôles par niveau, descriptifs travaillés selon la grille D retenue en 7.2a, arbitrages R2 sur les rôles à cheval (PO junior vs senior, Architecte d'équipe vs d'entreprise, Directeur de pôle vs de direction). Panel mobilisé : Elliot Jaques (horizon temporel / strates), Henry Mintzberg (axes de responsabilité), Marty Cagan (rôles produit), SAFe (rôles Agile scaled). Prérequis : 7.2a livré, mockup D arbitré. | ⏸ |
