...
- **Événements majeurs de la séance (08/05/2026 nuit, 11 commits)** : la séance bascule l'outil sur la **voie hybride** (fond bleu nuit pour la page, gris-bleu très clair pour la nav, blanc franc pour la matière) avec possibilité de bascule vers la **voie neutre clair** par `CM.Theme.appliquer('neutre')`. Décision majeure de doctrine : le crème (`doc-cadre-visuel.md` § 6.0/§ 6.1, posé le 05/05/2026) est définitivement écarté (« je n'aime vraiment pas »). § 6.0/§ 6.1 à amender selon la voie qui sortira du test terrain.
- **2 bugs de la séance précédente clos** : Bug B (masquage centralisé des vues de premier rang via `CM.App.masquerToutesVuesPremierRang()`, `3bdd2c7`) ; Bug A double-bandeau partiellement résolu (refonte plus large du bandeau via décision « tout passe par le même bandeau »).
- **Refonte architecturale en cours** — fusion *portes + axes* (scénario 1 acté ce soir comme amendement de la doctrine 23/04/2026 sur la bifurcation 4 axes / question). Mockup `mockup-fond-comparatif.html` produit pour trancher le fond. Architecture future : module `CM.PageLentilles` à modes plug-and-play (open/closed sur l'extension), document compagnon `doc-contrats-page-lentilles.md` à écrire.
- **Système de thèmes posé** (`d030c03`) — 4 nouveaux tokens INDÉPENDANTS de `--fond`/`--fond-2` : `--page-bg`, `--surface-nav`, `--texte-sur-page`, `--texte-2-sur-page`. Surcharge `:root[data-theme="neutre"]`. Module `CM.Theme` exposé (appliquer / lire / basculer / init). Persistance via `CM.Preferences` clé `'theme'`.
- **Vue Par ma question corrigée** (`32ff60a`) — cadre blanc intermédiaire + boutons en gris-bleu clair, lisibilité retrouvée. **Pattern à généraliser** : toute vue de premier rang qui contient du texte flottant doit poser un cadre blanc intermédiaire en hybride (sauf accueils qui assument le texte clair sur fond bleu).
- **Cascade + Maturité activées dans le bandeau** (`4601c45`) — fonction générique `CM.App.ouvrirVueDuBandeau(idOnglet)` posée. 6 occurrences de chaque stub remplacées. Libellé « La maturité ? » → « Maturité ».
- **Lexique** : option (A) — stub conservé, **chantier 21 ouvert dans le backlog** (`f5223fe`). Pas de dépendance bloquante.
- **11 commits sur la séance** : `3bdd2c7` (Bug B) · `b02de92` (mockup-comparatif) · `641e670` (matière blanc franc) · `3cb0027` (4e colonne hybride) · `b71e7e5` (retrait crème) · `d030c03` (système thèmes) · `32ff60a` (cadre blanc Par ma question) · `4601c45` (Cascade + Maturité bandeau) · `f5223fe` (chantier 21 Lexique).
- **Frictions visuelles attendues à régler dans la prochaine séance** : (i) double bandeau dans le tableau de bord (`.onglets` interne redondant avec le bandeau du haut — bug A originel à terminer) ; (ii) autres vues à auditer pour cadre blanc intermédiaire (#app cascade, #app maturité, vues détail des 3 portes 4-axes) ; (iii) accueil portes — vérifier que les cartes-portes restent lisibles sur fond bleu nuit après la bascule de fond.
- **Vision MVP global v1** : étape 4 (validation E2E par Lætitia) en pause le temps de la refonte hybride. Reprendra une fois les frictions résiduelles closes.
...
- **Mémoires actualisées cette séance** : aucune (les décisions de doctrine sont consignées dans le backlog et dans les commits).
- **Prochaine action recommandée** : Lætitia teste la voie hybride en navigant dans l'outil, remonte les frictions visuelles. Vague de petites remédiations attendue (cadre blanc intermédiaire à généraliser, bandeau interne du tableau de bord à supprimer, autres vues à corriger). En parallèle : option de basculer en `CM.Theme.appliquer('neutre')` pour comparaison terrain.
- **Blocages / questions ouvertes** : aucun ; tous les arbitrages stratégiques de la séance ont été tranchés.
