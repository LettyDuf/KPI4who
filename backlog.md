...
Dernière mise à jour : **24 avril 2026 — clôture de séance** (étape (b) du chantier 14 : tâche 1 close — Option B seuils/paliers ; tâche 2 Temps 1 close — vocabulaire tagsThematiques figé à 14 candidats avec arbitrages `transversalité` + `confiance`. 2 sous-questions en attente d'arbitrage pour lancer le Temps 2. Reste : Temps 2 tagging + Temps 3 analyse + tâche 3 signature `executer(filtre)`).
...
- **Chantier actif** : **chantier 14 — refactor hexagonal**. Étape (a) close. Étape (b) en cours : tâche 1 close (Option B seuils/paliers, `doc-contrats-chantier-14.md` §10.1). **Tâche 2 Temps 1 close** (vocabulaire arrêté à 14 tags dans `inventaire-tags-thematiques.md` ; arbitrages du jour : `communication` → `transversalité`, `sécurité psychologique` → `confiance`). Reste en tâche 2 : **Temps 2 tagging exhaustif des 84 fiches** (matrice fiches × tags) + **Temps 3 analyse quantitative** (taux d'usage, co-occurrences, orphelines) → tranche le vocabulaire final (10-12 tags probablement). Puis **tâche 3 : signature `executer(filtre)`** dans le document compagnon. Puis extraction `CM.RequeteMetriques` + tests unitaires via patron générateur Node.
...
- **Prochaine action recommandée** : étape (b) du chantier 14, tâche 2 **Temps 2 — tagging exhaustif des 84 fiches** (matrice `fiches × tags` dans `inventaire-tags-thematiques.md` §6). Puis Temps 3 (analyse quantitative → vocabulaire final 10-12 tags), puis tâche 3 (signature `executer(filtre)`).
- **2 sous-questions à trancher avant d'attaquer le Temps 2** (posées en clôture de session le 24/04, en attente d'arbitrage) :
    1. *Format de la matrice* : tableau 84×14 avec `x` (recommandé — lecture des co-occurrences immédiate en Temps 3) **ou** une ligne par fiche avec colonne `tagsThematiques` en toutes lettres (plus lisible en prose, moins bien pour l'analyse). → À trancher à l'ouverture.
    2. *Séquence de livraison* : un commit unique avec la matrice complète (recommandé — cohérence d'ensemble, relecture unique) **ou** trois commits par vague (équipe / programme / portefeuille+entreprise — plus de points d'arrêt mais plus long). → À trancher à l'ouverture.
...
- **Les chantiers 15 et 16 sont ouverts mais gelés** tant que 14 n'est pas livré (discipline 16.4 : ne pas cumuler les chantiers actifs).
