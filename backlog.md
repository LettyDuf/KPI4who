...
- **Événement majeur de la séance** : **audit interactif → 6 commits docs au-dessus de `2a61336`** (code intact, scenario corrigé, journal d'invariants posé, item 6.9 du backlog enrichi). Lætitia a testé `cadre-indicateurs.html` en Safari et remonté 7 points. Audit conjoint : aucune régression introduite par le chantier 14. Les 7 points se répartissent en (a) 5 comportements actuels conformes mais éloignés de ses attentes (chantiers 7.2a-code.3, 9, 10 gelés derrière 14), (b) 1 vraie dette de câblage UI du panier (T1 — `CM.Panier` extrait/testé mais aucun appelant en UI), (c) 1 creux éditorial structurel du référentiel sur les niveaux Programme / Tactique / Stratégique (1-3 fiches par couple, 3 couples à 0 fiche).
- **Chantier actif** : **chantier 14 — refactor hexagonal**. Étape (a) close (`mvp-etape-a-schema-inventorie`). Étape (b) close (`mvp-etape-b-coeur-extrait` = `2a61336`). Prochaine étape : **étape (c) — migration des 3 portes livrées vers `CM.RequeteMetriques`**. Discipline : porte par porte, journal d'invariants vert à chaque bascule, jamais en parallèle. Tag de clôture cible : `mvp-etape-c-portes-migrees`.
- **Document central pour la reprise** : **`journal-invariants-pre-c.md`** (commit `9525f8e`). Quatre invariants figés sur HEAD `2a61336` lors du test interactif du 25/04/2026. Sentinelles d'iso-comportement à reproduire à l'identique après chaque bascule. Toute divergence, même +1/-1, déclenche rollback à `5655b03`.
- **Test interactif côté Lætitia** : **partiellement complété**. I1 ✓ (Programme/Projet × Délais = 1 fiche `Taux d'avancement des jalons` + 6 délégation), I2 ✓ (DORA × Opérationnel = 13 fiches, première = `Fréquence de déploiement`), I3 ✓ partiel (ordre fiabilité OK sur les portions vues), I5 nombre noté (33 fiches Indicateur générique × Opérationnel) mais **titre de la première fiche d'I5 manquant — à compléter au prochain passage**. I4 et I6 en skip documenté (non atteignables depuis l'UI aujourd'hui).
- **Découverte structurelle (item 6.9 enrichi en `c849f19`)** : croisement automatique `niveau × META.tags` sur les 137 fiches du référentiel. Sur les 28 cellules de la matrice canonique 4×7, **3 cellules à 0 fiche** (Stratégique × Délais, Stratégique × Flux, Tactique × Qualité), **5 cellules à 1-2 fiches** (toutes aux niveaux supérieurs). Disproportion : 98 occurrences de tags au niveau Opérationnel vs 50 sur les 3 niveaux supérieurs cumulés. Action 6.9 reformulée en 3 étapes ordonnées : (a) densifier le tagging existant, (b) trancher sur les 3 couples à 0, (c) seulement après, envisager un 8e tag *Visibilité & transparence*. Toujours gelé derrière 14.
- **Chantier 9 (panier)** : encart de statut posé en `462bae9`. Moteur `CM.Panier` extrait + `tests-panier.html` vert, mais **aucun appelant UI** n'invoque `Panier.ajouter`. Le chantier 9 est en cadrage clos / livraison non démarrée. Câblage UI à reprendre comme tranche dédiée — gelée derrière 14.
- **Dernier SHA de contenu** : `c849f19` (cadrage 6.9 enrichi du diagnostic chiffré). Précédents de la séance : `9525f8e` (journal d'invariants), `b4e823b` (trous bloc État courant comblés), `462bae9` (encart §9 dette T1), `730e709` (item 6.9 ouvert), `be2a26b` (scenario reformulé). **Tags** : `baseline-avant-hexagonal` = `5655b03`, `mvp-etape-a-schema-inventorie` = `1ba5112`, `mvp-etape-b-coeur-extrait` = `2a61336` (inchangé — les 6 commits de séance sont du contenu, pas une bascule de jalon). Prochain tag cible : `mvp-etape-c1-porte-probleme-migree` après l'étape (c.1).
- **Fiches mémoire ajoutées cette séance** : `feedback_titre_suffit_pas_url` (test interactif Lætitia → titre lisible, pas id technique via barre d'adresse Safari), `feedback_creux_editorial_avant_bug` (remontée « le filtre ne marche pas » → croiser données avant code).
- **Prochaine action recommandée pour la prochaine séance** : trois pistes au choix selon l'énergie de Lætitia. (1) **démarrer (c.1) — porte *Par mon problème*** en consommatrice de `CM.RequeteMetriques.executer({niveau, probleme, domaine?})` ; sentinelle = I1 du journal ; tag `mvp-etape-c1-porte-probleme-migree`. (2) **compléter le journal** : titre de la première fiche d'I5 et liste complète des 13 fiches d'I2 pour I3 exhaustif. (3) **rouvrir 6.9** seulement après livraison 14, jamais avant (discipline 16.4).
- **Blocages / questions ouvertes** : aucun côté contenu. Côté workflow git sandbox : ⚠ `.lock` orphelins continuent de s'accumuler (cf. `feedback_git_workflow_sandbox`) — Lætitia, un nettoyage côté ordi (`rm .git/*.lock.kill*` dans le dossier projet) ramassera la cinquantaine accumulés cette séance.
