...
- **Prochaine action recommandée** : ouvrir le sous-chantier **23.g-4 — tests automatisés `CM.AccueilUnifie`** (différé en clôture séance 09/05, jalon B ; **préalable C3 résolu en `371a671`**, le libellé canonique vide est désormais figé). Pattern de référence : `tests-requete-metriques.html` (suite de tests JS pure, ouverture navigateur, assertions visibles). Effort estimé ~2-3 h. Couverture cible : les 6 invariants du journal 23.g-3 (I1-I6) reproduits en assertions automatiques + cas adjacents (changement de chips successifs, état restauré au montage, libellé d'axes actifs). À discuter en ouverture : faut-il fusionner avec un fichier de tests existant ou créer `tests-accueil-unifie.html` dédié ?
