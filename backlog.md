...
- **Prochaine action recommandée** : **Étape 2 du plan MVP global v1 — résorber la dette T1 du chantier 9** (~4-6h). Câbler les voies d'ajout au panier sur les fiches du référentiel : `CM.Panier.ajouter(ficheId)` existe et fonctionne, mais aucun appelant UI ne le déclenche depuis les fiches indicateur. Sans cette étape, le bouton « Mon tableau de bord » du bandeau livré ce soir affiche toujours « Votre tableau de bord est vide » — le geste utilisateur de constitution du panier est manquant. Approche probable : ajouter un bouton « Ajouter à mon tableau de bord » dans chaque fiche indicateur (tiroir de fiche `CM.Composants.ouvrirTiroir`), avec retour visuel et persistance via le moteur Panier. Travaux *en parallèle* possibles : nettoyer les `.todelete` et `.git/*.lock.tmp*` cumulés côté ordi ; ouvrir le câblage propre du Lexique (création d'un tiroir) pour transformer le stub du bandeau en lien fonctionnel.
