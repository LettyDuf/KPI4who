...
- **Prochaine action recommandée** : jalon C.1 livré bout en bout (5 commits `c36f2ca` → `05298e9`). **Prochaine étape immédiate : C.1.7 — smoke test interactif Lætitia** sur `cadre-indicateurs.html#fiche-q=pilotage-hebdo` dans Safari, captures éventuelles, corrections post-smoke en commits atomiques, tag de clôture `mvp-chantier-c1-livre` quand stable. **Après C.1.7, prochaine étape : C.2 — migration de la fiche `amelioration-continue` (variante § 4.4 standard) vers `CM.FicheQuestion`**. Effort estimé ~3h. À l'occasion de C.2, la cible droite de `pilotage-hebdo` (`ficheQCible: 'amelioration-continue'`) basculera automatiquement de mode dégradé à lien actif — démontre la robustesse du pattern option C. Puis C.3 resistance-transformation, C.4 cascade-objectifs (~3h chacun). **Pour C.2** : étendre `composer()` et `htmlFiche()` pour la variante § 4.4 (Q1 réordonne le trio, Q2 change le contenu de chaque card). Patron : `_composerMeta` de §4.6 (mécanique très voisine). **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord (engagement 09/05 non livré, ~30-45 min), **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**. **Friction beforeunload** : réglée doctrinalement (MVP éphémère). Aucun blocage actif sur l'outil. Tag de point de retour courant : `mvp-chantier-26h-livre` sur `48c35e5` (livré 16/05). Tag à venir : `mvp-chantier-c1-livre` au C.1.7.
