...
- **Mockups produits cette séance prolongée** : aucun. **Harnais de tests** : (i) `tests-accueil-unifie.html` (1457 l., 21 tests unitaires sur stubs en 6 sous-sections A.0-A.6) + `outils/construire-tests-accueil-unifie.js` (170 l.) — 21/21 verts. (ii) `tests-accueil-unifie-sentinelles.html` (3070 l. dont 2640 injectées : 5 zones inlinées depuis cadre-indicateurs.html — CM.Referentiel + CM.IndicateursMeta + 15 blocs de données + CM.RequeteMetriques + CM.AccueilUnifie) + `outils/construire-tests-accueil-unifie-sentinelles.js` (~210 l.) — 9 sentinelles : I1-I6 du journal + A1-A3 cas adjacents façade publique, 9/9 verts.
...
- **Prochaine action recommandée** : enchaîner sur **23.g-4.4 — outil compagnon + clôture chantier 23.g**. Trois livrables : (1) `outils/lancer-tests-accueil-unifie.js` — script Node compagnon qui régénère les deux harnais + lance un parse-check syntaxique sur le bloc <script> concaténé. Pattern de référence : `outils/lancer-tests-requete-metriques.js`. Effort ~20 min. (2) Mise à jour `scenario-non-regression.md` — §11 invariants enrichi avec une mention des deux harnais (fichier dédié vs section A) et des commandes de smoke-test (ouverture Safari). Effort ~15 min. (3) Clôture journal-invariants-23g.md — entrée datée acant 23.g-4.4 avec les SHAs cumulés de la séance. Effort ~10 min. Total estimé ~45 min. À l'issue, le chantier 23.g sera officiellement clos et un tag `mvp-chantier-23g-livre` pourra être posé sur le commit final.
