...
Dernière mise à jour : **18 avril 2026** (ajout section 6 « Évolutions porte Par mon problème en file d'attente », plafond de 5 recommandations retiré, révision af-c2 et décomposition `rendreFicheHtml` marquées livrées).
...
**Fiches déjà revues au standard 2026-04** : o1 (Fréquence de déploiement), o2 (Lead Time), o3 (Change Failure Rate), o5 (MTTR), p5 (Débit d'initiatives livrées), **af-c2 (Durée du cycle de vente, 18/04/2026)**, et les fiches du chantier 3 (s6 à s10, t6, t7, p6, p7) nativement rédigées à ce standard.
...
**Audit actuel (18/04/2026, après commits `006d12b` + `e4d7041`)** : périmètre plus large qu'annoncé dans la mémoire initiale (4 modules ajoutés non prévus dans la cible d'origine). `rendreFicheHtml` désormais décomposé en 11 sous-fonctions, plafond arbitraire levé sur les 2 portes. Nouvelle estimation : **7,5/10 clean code** (IIFE/namespace propres, fonctions courtes), **6/10 hexagonal-light** (Vues séparées des Composants, `rendreFicheHtml` décomposé, mais `CM.Html` reste à créer et pas encore de ViewModel). Cible à 5 itérations : **8,5/10 et 7,5/10**.
...
| 6.3 | Chips « cadres de travail » (DORA, Lean, Scrum…) visibles sur les cartes | 🟡 | La donnée META existe déjà sur 60+ fiches mais n'est jamais rendue — l'utilisateur ne voit pas à quel cadre méthodologique rattacher une métrique | ⏸ |
...
| 6.3 Chips cadres sur cartes | `CM.Composants.htmlCarte` (~ligne 3552) + lookup `CM.IndicateursMeta.META` (lignes 1953-2100) + vocabulaire `CADRES` déjà défini | Ajouter ligne de rendu chips après la pastille fiabilité — le dictionnaire `CADRES` contient déjà `{label, icone}` pour dora/lean/okr/mbo/scrum/kanban/safe/itil/bsc/generique |
...
La décomposition de `rendreFicheHtml` ayant été livrée en `006d12b`, la greffe opportuniste sur t4 pourra porter sur la création de `CM.FicheViewModel` (étape suivante du jalon 3.2).

**Note méthode** : la Règle 1 (`CM.Html.escape`) reste différée tant qu'aucune donnée externe n'entre dans le pipeline. Elle deviendra prioritaire dès qu'on reprendra le chantier 6 (panier + import JSON = premières entrées utilisateur).
