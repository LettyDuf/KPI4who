...
- **Prochaine action recommandée** : **chantier 26 CLOS** — les 5 fiches-questions sont intégrées, adossées au référentiel (104 fiches) et testées ; jalons A à E livrés, mini-chantier du constat n°10 bouclé. **Plus de chantier majeur en cours.** Pistes ouvertes, au choix : **21** (vue Lexique, stub à implémenter), **24** (URLs partageables des filtres), **13.bis** (chips de maturité cumulables), **25** (audit des zones de code inactives) ; finitions transverses : brancher les liens sortants des fiches-questions vers une vue d'ensemble de cadre, **26.c4.coll.2** (crossLink cascade ↔ pasteque) ; dette : **26.c1.coll.1** (audit NBSP module fiche-question), constat n°10 (4 pistes de cadres restantes : commercial, PMI/EVM, GRC-conformité, RH analytique), constat n°8 (corpus Scrum) ; post-MVP : **26.c2.coll.1** (fiche-question candidate), Option A tableau de bord, chantier 10 (Cascade stratégique, page Maturité). **Demande différée de Lætitia** : rendre cliquables les noms d'indicateurs dans les pieds de colonne § 4.3. Aucun blocage actif. Tag de point de retour courant : `mvp-chantier-26-livre`.
