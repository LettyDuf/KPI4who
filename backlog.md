...
Généralisation à l'ensemble du référentiel en cours, par lots éditoriaux. **Lot A livré** (`422b0a3`, 21/04/2026), **Lot B livré** (`9846600`, 21/04/2026), **Lot C livré** (`31019e3`, 21/04/2026). Lot D (Sécurité / Incident, ~2 liens + arbitrage MTTR DORA vs MTTR Sécurité) encore à faire — voir détail plus bas.
...
1. **Lot D — Sécurité / Incident** (🟡). Deux liens mais un arbitrage sur `s9.alt → MTTR` : pointe-t-on vers o4 (DORA générique) ou ti-s2 (MTTR Sécurité) ? À décider par lecture contextuelle du champ.
