...
- **Prochaine action recommandée** : séance 16/05/2026 close à `ddb6b74` après 2 commits doc (ouverture chantier 26 + doc compagnon v0.1). **Chantier 26 jalon A bouclé** — reste validation Lætitia du `doc-contrats-fiche-question.md` (3 points d'arbitrage : format source JS figé vs JSON, pas de fuite vers CM.Panier, ordre de migration pastèque d'abord). **Prochaine séance : jalon B — PoC sur la fiche pastèque** (variante § 4.6 la plus exigeante, matrice 3×3 axes × niveaux = 9 cellules). À faire avant le premier commit de code : poser le tag `baseline-avant-chantier-26`. **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord (engagement 09/05 non livré, ~30-45 min), **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**. **Point reporté restant** : (1) friction beforeunload — réglé doctrinalement (MVP éphémère, statu quo). Friction utilisateur résiduelle absorbée par le chantier 26 (les fiches deviendront des routes internes, plus de navigation externe). Aucun blocage actif sur l'outil.
