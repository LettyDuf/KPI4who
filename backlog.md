...
- **Prochaine action recommandée** : **chantier 21, jalon B.1 livré (rayon anti-patterns du Lexique complet, 49 entrées dans `lexique-source.md`).** Prochain pas : **jalon B.2, rayon cadres et méthodes** (20 entrées), même format. Puis indicateurs, concepts et auteurs (jalons B.3 à B.5). Arbitrages A2 (classification) et A3 (famille outcome) portés avec recommandation, corrigeables à la relecture. Référence : `doc-contrats-lexique.md`. Autres pistes en attente : 24, 13.bis, 25. Tag de point de retour courant : `mvp-chantier-26-livre`.
