...
**Constat collatéral n°2** *(remonté lors de l'audit cadre `itil` le 30/04/2026)*. La taxonomie `CADRES_META` ne contient **aucun cadre sécurité** (NIST CSF, ISO 27035, OWASP, MITRE ATT&CK, FAIR…), alors que le référentiel contient au moins 5 fiches sécurité (s9, ti-s1, ti-s2, ti-s3, ti-s4 + déjà détaggées de `lean` et `itil`). Conséquence : ces fiches sont mal couvertes par les portes (notamment « Par mon cadre »). À statuer dans un item dédié — créer un cadre `securite` (label « Sécurité (NIST CSF / ISO 27035) ») ou élargir la couverture autrement.
