...
- **Prochaine action recommandée** : **jalon C.3 CLOS** — smoke test validé sans friction, tag `mvp-chantier-c3-livre` posé. **Prochaine étape : C.4 — migration de `cascade-objectifs` (variante § 4.4 standard, famille Drucker), dernier jalon § 4.4.** **Décision Lætitia (20/05) : après C.4, intercaler le mini-chantier éditorial du constat collatéral n°10** — créer dans le référentiel les 6 fiches d'indicateurs de gestion du changement nommées par les cellules `CADRES_A_VENIR` de `resistance-transformation` (trios équipe et programme) plus la fiche-cadre Gestion du changement, pour adosser pleinement cette fiche-question au référentiel ; les jalons D et E suivent. Même patron technique que C.2/C.3, moteur en place (le rendu des noms provisoires des cellules `CADRES_A_VENIR` est disponible depuis `a766b76`). Puis **jalon D** (câblage complet : repointer la tuile cascade-objectifs après sa migration, retirer les 5 mockups autonomes orphelins) et **jalon E** (tests E2E + smoke test global). **Travail connexe ouvert (26.h.coll)** : appliquer des noms provisoires aux 3 cellules `CADRES_A_VENIR` de la fiche pastèque — le rendu le permet depuis `a766b76`, les données pastèque ne les portent pas encore. **Demande différée de Lætitia** : rendre cliquables les noms d'indicateurs dans les pieds de colonne § 4.3 (Throughput, Lead time, etc.) — à traiter après C.4. **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord, **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**, **26.c1.coll.1 (dette NBSP module fiche-question)**, **26.c2.coll.1 (fiche-question candidate post-MVP)**, **constat collatéral n°10 (fiche cadre Gestion du changement + indicateurs de gestion du changement absents — directement motivé par les 6 cellules `CADRES_A_VENIR` de C.3)**. **Friction beforeunload** : réglée doctrinalement. Aucun blocage actif. Tag de point de retour courant : `mvp-chantier-c3-livre`.
