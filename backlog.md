...
- **Prochaine action recommandée** : **jalon C.1 CLOS** — re-smoke test validé par Lætitia le 20/05/2026, tag `mvp-chantier-c1-livre` posé, mockups de travail archivés en `.todelete-c1`. **Prochaine étape : finaliser les pages de réponses** (formulation de Lætitia, séance 20/05). Périmètre exact à cadrer en ouverture de la prochaine séance : vraisemblablement C.2 / C.3 / C.4, c'est-à-dire la migration des trois fiches-questions § 4.4 restantes — `amelioration-continue`, `resistance-transformation`, `cascade-objectifs` — qui sont les pages délivrant un trio d'indicateurs en réponse. **Pour les fiches § 4.4** : étendre `composer()` et `htmlFiche()` (Q1 réordonne le trio, Q2 change le contenu de chaque card) ; patron `_composerMeta` de § 4.6 ; appliquer la doctrine éditoriale v0.6 (*la fiche explique le pourquoi, elle n'affirme pas*) et le rendu multi-paragraphes (`_normaliserParagraphes`). **Demande différée de Lætitia (séance 20/05)** : rendre cliquables les noms d'indicateurs dans les pieds de colonne § 4.3 (Throughput, Lead time, etc.) vers les fiches du référentiel — à traiter APRÈS la finalisation des pages de réponses. Trois options identifiées (statu quo / quelques noms canoniques / tous les noms en tableau d'objets). **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord (~30-45 min), **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**, **26.c1.coll.1 (dette NBSP module fiche-question)**. **Friction beforeunload** : réglée doctrinalement (MVP éphémère). Aucun blocage actif sur l'outil. Tag de point de retour courant : `mvp-chantier-c1-livre`.
