...
- **Prochaine action recommandée** : sous-chantier 26.h livré bout en bout. **Prochaine étape : C.1 — migration de la fiche pilotage-hebdo (§ 4.3 reformulation) vers `CM.FicheQuestion`** : 7 commits atomiques planifiés (données, domaine, vue, CSS, doc compagnon, État courant, clôture post-smoke). **Trois arbitrages validés en ouverture** : (1) mécanique « clic colonne » → option C, redirection vers fiche-question voisine avec pattern `CADRES_A_VENIR` pour les cibles non encore migrées ; (2) doctrine B rétrospective pour les em dashes du mockup d'origine (matière née avant le 16/05) ; (3) anglicismes patrimoniaux conservés avec `<span class="traduction">`. **À noter pour C.1** : le module et le rendu `CM.FicheQuestion` ne supportent à ce jour QUE la variante § 4.6 — il faudra étendre `composer()` et `htmlFiche()` pour la variante § 4.3 (et pour la § 4.4 à C.2). Effort estimé ~2h. Puis C.2 amelio-continue, C.3 resistance, C.4 cascade-objectifs (~3h chacun). **Constats collatéraux 26.h portés en § 26.h.coll** ci-dessous. **Autres chantiers ouverts cumulés** : (II) Option A tableau de bord (engagement 09/05 non livré, ~30-45 min), **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables)**, **25 (audit zones inactives)**. **Friction beforeunload** : réglée doctrinalement (MVP éphémère). Aucun blocage actif sur l'outil. Tag de point de retour : `mvp-chantier-26h-livre` sur `48c35e5` (à poser au commit suivant 5d5dae3+1).
