...
- **Points reportés cumulés (à reprendre en commits dédiés)** :
  ...
  4. **Double libellé pour combinaison vide** (constat C3 du journal 23.g-3) — compteur dit « Aucun indicateur ne combine ces lentilles. » et panneau dit « Aucune fiche ne correspond à cette combinaison d'axes. ». Deux registres lexicaux (indicateur/fiche, combine/correspond, lentilles/axes). À arbitrer : choix volontaire à confirmer ou harmonisation à effectuer.
...
- **Prochaine action recommandée** : ouvrir le sous-chantier **23.g-4 — tests automatisés `CM.AccueilUnifie`**. Pattern de référence : `tests-requete-metriques.html` (suite de tests JS pure, ouverture navigateur, assertions visibles). Effort estimé ~2-3 h. Couverture cible : les 6 invariants du journal 23.g-3 (I1-I6) reproduits en assertions automatiques + cas adjacents (changement de chips successifs, état restauré au montage, libellé d'axes actifs). À discuter en ouverture : faut-il fusionner avec un fichier de tests existant ou créer `tests-accueil-unifie.html` dédié ?
