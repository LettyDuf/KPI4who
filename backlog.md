...
- **Prochaine action recommandée** : séance 09/05/2026 close à `89ffeab` après 18 commits cumulés. **Chantiers ouverts en attente d'arbitrage utilisateur, à reprendre en priorité** : **(I) Audit typographique du référentiel — chantier scindé le 13/05/2026** en deux items distincts (voir § 1.quater). **(I.a) Audit accents** — dette d'accents systémique sur les fiches anciennes (*marche, capture, cles, gere, versionne, opportunites, recurrents, degradation, depassements*, etc.). Travail mécanique, 3 options à arbitrer : (A) agent autonome qui applique en bloc, (B) audit séquentiel par Claude avec table de remplacements validée mot par mot, (C) lots ciblés boy scout. **(I.b) Audit em dash** — 204 occurrences de `—` dans les chaînes des champs `definition`, `objectif`, `frequence`, `exemple_*`, `risque`, `alt`. Substitution mécanique vers `:` jugée inadaptée le 13/05/2026 (lecture du diff d'une session précédente non committée) : le `—` porte trois fonctions distinctes (incise encadrée, adversatif, explicitation) qu'un seul signe de remplacement ne couvre pas. Chantier à reprendre cas par cas, après livraison de (I.a). **Préalable du 13/05/2026** : revert du diff de session précédente (`mv` → `git checkout`) — état HTML revenu à `74ade4f`. **(II) Option A pour le tableau de bord** — basculer `_htmlCartePanier()` vers le format unifié `htmlCarte()` (avec X retirer à la place des boutons panier ✓ / 💡), engagement pris mais non livré. Effort ~30-45 min. **Point reporté restant** : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher). **Autres chantiers ouverts cumulés** : **21 (vue Lexique)**, **13.bis (chips cumulables maturité)**, **24 (URLs partageables des filtres)**, **25 (audit zones inactives, ouvert cette séance)**. Aucun blocage actif sur l'outil.
