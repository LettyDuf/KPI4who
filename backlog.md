...
- **Événement majeur de la séance jour (08/05/2026, 23 commits f5d5b66 → c50032c)** : **chantiers 23.a, 23.b et 23.c livrés** — la refonte radicale de l'accueil est passée de la doctrine au code interactif en une séance.
  - **23.a clos (7 commits)** : doctrine consignée dans `doc-cadre-visuel.md` (§ 6.1 réécrit, § 6.3 neutralisée, § 6.4 actualisé) et `doc-contrats-navigation.md` (§ 4.1 invariants 8→6 entrées, façades dépréciées au 23.f, nouveau § 6.6 contrats `CM.AccueilUnifie` en portée équilibrée).
  - **23.b clos (3 commits)** : bandeau du haut passe de 8 à 6 entrées partout (`7eb68b0`), squelette HTML de la page unifiée en bascule directe — option B retenue par Lætitia (`e586a97`), CSS scopé `.accueil-unifie` (`49fed62`).
  - **23.c clos (4 sous-commits + 1 mockup + 6 fixes)** : module `CM.AccueilUnifie` (`6f932ef`), accordéon des chips (`03bb645`), contenu des sélecteurs avec libellés français (`32430b3` après mockup d'arbitrage `8a144c0`), sélection effective + chip en mode actif (`dca9aa2`). Bugs remontés et corrigés en boucle : lisibilité voie hybride (`5b45070`), `<abbr>` littéral (`ab2e88c`), Problème en variante B compact (`526ffee`), entités UTF-8 + anglicisme CX (`41f3d61`), chip multi en compteur seul option A (`9f6505c`), emojis du référentiel (`c50032c` après mockup `9ea406c`).
- **Arbitrages tranchés cette séance (5 décisions clés)** :
  1. **Bandeau à 6 entrées** — *Par mes 4 axes* + *Par ma question* sortent du bandeau, absorbées dans l'accueil unifié.
  2. **Bascule directe (option B)** — l'ancien accueil 5 portes part immédiatement, l'outil reste partiellement cassé pendant 23.b → 23.e (cf. mémoire `project_chantier_23_strategie.md`).
  3. **Doctrine d'affichage par axe** — Niveau α (compact + sous-titre empilé) ; Problème, Cadre, Maturité B (compact sans sous-titres) ; emojis du référentiel sur les 4 axes ; libellés courts pour CX (*Expérience client*) et Sécurité.
  4. **Chip multi en compteur seul (option A)** — `✓ Cadre (3)` au lieu de concaténer les labels, taille fixe.
  5. **Vocabulaire technique vs novice** — *« Flux »* du lexique Lean signalé comme orientation cachée. Décision : **on laisse tel quel** pour le MVP global v1, à reprendre plus tard. Constat collatéral n°2 du chantier 23 dans le backlog.
- **Chantier 23 — état d'avancement** : 23.a ✅ · 23.b ✅ · 23.c ✅ · **23.d 🔴 prochain (cartouches cliquables avec compteur live)** · 23.e (liste filtrée 2 colonnes — branchement traducteur orthodoxe) · 23.f (suppression code orphelin) · 23.g (tests E2E + sentinelles).
- **Chantiers ouverts cumulés** : 21 (vue Lexique) · 13.bis (chips cumulables maturité) · 22 (harmonisation tutoiement) · **23 (refonte radicale accueil — sous-chantiers 23.d à 23.g restants)** · 24 (URLs partageables des filtres, en attente trigger) · *Constat collatéral n°2 latent — audit vocabulaire de l'accueil*.
- **État de l'outil aujourd'hui** : la page d'accueil unifiée est **visible et interactive** (chips ouvrent leur sélecteur, sélection persiste en localStorage, chip refermée affiche le mode actif). Mais **aucun filtre n'est encore appliqué** — la cartouche compteur reste inerte (23.d), la liste filtrée n'est pas peuplée (23.e). L'accueil 5 portes a disparu, la pyramide n'a plus de lien dans l'UI mais reste accessible via `#pyramide` jusqu'au 23.f.
- **Mémoires actualisées cette séance** : `project_chantier_23_strategie.md` (nouvelle, doctrine bascule directe).
- **Workflow git sandbox** : pattern `mv .git/index.lock` toujours actif. Paires `.git/*.lock.tmp.alpha → psi` accumulées (23 commits + 8 commits 23.a = ~30 paires), à nettoyer côté ordi.
