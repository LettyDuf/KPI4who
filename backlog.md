...
Dernière mise à jour : **21 avril 2026** (chantier 7.2a-inventaire — niveau Opérationnel récupéré et acté avec le SHA réel `cfa5b68`, niveau Tactique livré avec `6dd1d30` : 14 fiches rôles couvrant SAFe, ICAgile, Cagan, PMBOK, Lean Six Sigma, Urbanisation SI francophone, DAMA, fonction publique QC/CA. Deux niveaux restants à produire : Portefeuille, Exécutif).
...
| 7.2a-inventaire | **Inventaire éditorial des rôles par niveau (prérequis de la phase code)** | 🟡 | *Ouvert le 21/04/2026 après l'arbitrage D2 du mockup-preview. Niveau Opérationnel livré en calibration (`cfa5b68`, 13 rôles) ; niveau Tactique livré le 21/04/2026 (`6dd1d30`, 14 rôles) ; deux niveaux restants à produire : Portefeuille et Exécutif.* Construction d'un référentiel éditorial de ~12-15 rôles par niveau, couvrant Agile/Scaled, Lean Six Sigma, produit, PMBOK, fonction publique, management classique. Décisions actées : **D2** (quatre axes humaine / projet / méthodologique / stratégique, cumulables), **N1** (double-libellé), **R2** (rôle répété avec qualificatif si à cheval), **C2** (clic sur rôle), **α** (un seul champ *descriptif orthodoxe* par rôle — le champ *dérive du marché* est abandonné ; renverse la décision ρ1 prise plus tôt le 21/04/2026, voir `project_porte_niveau_doctrine_editoriale.md`). Règles éditoriales : pas de mot *senior* (jugement de valeur), PO reformulé par périmètre de produit et non par séniorité. Livrable : fichier `inventaire-roles-porte-niveau.md` (Opérationnel déjà en place), source éditoriale pour `CM.Roles` en phase code. Panel : SAFe®, Scrum Guide 2020, Lean Six Sigma (Yellow/Green/Black/Master Black Belt + Champion + Sponsor), Marty Cagan (rôles produit), PMBOK / PMI, PRINCE2, fonction publique québécoise et fédérale canadienne (sous-ministre, SMA), Mintzberg (cartographie axes), Jaques (horizon temporel pour trancher les cas limites). Voir `project_porte_niveau_roles.md`. | ⏸ |
