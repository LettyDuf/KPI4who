...
Dernière mise à jour : **25 avril 2026 — clôture de séance** (tâche 3 de l'étape (b) du chantier 14 avancée à 4 commits sur 5 : alignement §10.3, squelette + validation, injection tagsThematiques + générateur Node, sémantique union/intersection. Reste commit 3 = harnais HTML de tests automatisés)
...
- **Événement majeur de la session** : **tâche 3 de l'étape (b) avancée à 4 commits sur 5**. Vocabulaires de §10.3 alignés sur l'inventaire de l'étape (a) (`abd9315`) après audit montrant 4 divergences entre contrat et donnée réelle. Squelette `CM.RequeteMetriques` + validation stricte du filtre livré (`63e76f2`, 30 tests fumants verts). Préalable `tagsThematiques` traité par générateur Node + injection des 84 entrées (`4c7e1b8`, patron éditorial `project_outillage_generation_donnees` respecté). Logique union/intersection sur les 9 axes filtrants livrée (`2c6aa61`, 23 tests verts contre les 84 fiches réelles). Iso-comportement utilisateur strict — aucun consommateur n'appelle encore `executer`.
- **Chantier actif** : **chantier 14 — refactor hexagonal, étape (b)**. Tâche 1 close. Tâche 2 close. **Tâche 3 — 4 commits sur 5 livrés**. Reste : **commit 3** = tests automatisés via patron générateur Node + harnais HTML, couvrant les comportements limites du tableau §10.3 + un panel de filtres représentatifs croisant 2-4 clauses. Une fois commit 3 livré, poser le tag `mvp-etape-b-coeur-extrait` sur le HEAD final de l'étape (b).
- **Test interactif côté Lætitia en attente** : ouvrir `cadre-indicateurs.html` dans Safari (`Cmd + Opt + C` pour la console après activation *Développement*), jouer `scenario-non-regression.md` (~30 clics, ~10 min), et coller la mini-batterie console (5 lignes) pour vérifier que `CM.RequeteMetriques.executer`, `CM.IndicateursMeta.tagsThematiquesDe` et `CM._tagsThematiquesData` (84 entrées) sont bien chargés. Si régression détectée : mot-clé `rollback à l'étape X` (X = `mvp-etape-a-schema-inventorie`).
...
- **Dernier SHA de contenu** : `2c6aa61` (commit 2b — sémantique union/intersection). Précédents de la séance : `4c7e1b8` (commit 2a, injection tagsThematiques + générateur), `63e76f2` (commit 1, squelette + validation), `abd9315` (alignement §10.3). **Tag git** `baseline-avant-hexagonal` = `5655b03`, point de retour absolu. Tag `mvp-etape-a-schema-inventorie` posé sur `1ba5112`. Prochain tag à poser après commit 3 : `mvp-etape-b-coeur-extrait`.
...
- **Fiches mémoire pertinentes pour la reprise commit 3** : `project_chantier_14_ouverture` (plan, doctrine rollback), `project_document_compagnon_contrats` (§10.3 fait foi), `project_outillage_generation_donnees` (patron générateur Node — utilisé pour `outils/generer-tags-thematiques.js`), `feedback_git_workflow_sandbox` (nouvelle — séparer git add du git commit dans le sandbox), `feedback_anti_invention_documents`, `feedback_verifier_avant_affirmer`.
- **Prochaine action recommandée** : **commit 3** = générer un harnais HTML `tests-requete-metriques.html` via `outils/construire-tests-requete-metriques.js` (patron identique à `construire-tests-panier.js`), couvrant tous les comportements limites du tableau §10.3 + filtres représentatifs croisant 2-4 clauses. Idéalement après confirmation par Lætitia que le scénario manuel passe sans régression.
