...
- **Chantiers ouverts cumulés** : 21 (vue Lexique) · 13.bis (chips cumulables maturité) · 22 (harmonisation tutoiement) · ~~23 ✅ livré (refonte radicale accueil — clos sur ed52a23, tag mvp-chantier-23g-livre)~~ · 24 (URLs partageables des filtres, en attente trigger) · *Constat collatéral n°1 latent — sort de l'attribut `axes:[…]` Mintzberg* · *Constat collatéral n°2 latent — audit vocabulaire de l'accueil*.
...
- **État de l'outil après reprise 09/05/2026 (`35bbc61`)** : seul changement = retrait CSS mort `.porte-statut` (orphelin, sans usage). Aucun changement de comportement utilisateur. Smoke test interactif soir 8/05 toujours valide (cf. journal-invariants-23g.md).
...
- **Prochaine action recommandée** : ordre validé par Lætitia en reprise 09/05/2026 — (a) **commit cosmétique CSS mort `.porte-statut`** ✅ livré (`35bbc61`, 15 lignes), (b) **mission de l'accueil unifié** (point reporté n°2, à insérer au-dessus du h2 épuré — chantier court, demande arbitrage éditorial), (c) **chantier majeur 22 — harmonisation tutoiement** (priorité car la voix de l'outil est la plus visible juste après l'accueil unifié ; toute fiche écrite entre-temps fige le mauvais ton). Deux **points reportés** restent en attente : (1) friction garde-fou beforeunload sur clic fiche-question (4 options identifiées, à trancher), (2) mission de l'accueil unifié à insérer au-dessus du h2 épuré (= étape b ci-dessus). Chantiers ouverts cumulés restants après 22 : **21 (vue Lexique)**, **13.bis (chips cumulables maturité — en attente du retour de la maturité au périmètre actif)**, **24 (URLs partageables des filtres — en attente trigger)**. Aucun blocage actif sur l'outil.
