...
Dernière mise à jour : **20 avril 2026** (livraison chantier 6.7 : porte « Par mon cadre » active — accordéon par école de pensée, 7 familles, 10 cadres actifs + 10 cadres « à venir » affichés en tuiles grisées. Deux commits : `44c20b4` chore référentiel FAMILLES + `f1b25bc` feat porte complète. Révision des principes en 6.b pour refléter le pivot de la grille à plat vers l'accordéon par famille, décidé après mockup-preview).
...
| `CM.DiagnosticCadre` | Logique métier « quels indicateurs pour ce cadre × ce niveau » — délègue les niveaux à `CM.DiagnosticProbleme` pour ne pas dupliquer le vocabulaire | ✅ créé (20/04/2026, commit `f1b25bc`) |
| `CM.VuePorteCadre` | Vue stepper 1→2→3 du parcours par cadre (accordéon par famille en étape 1) | ✅ créé (20/04/2026, commit `f1b25bc`) |
...
| ~~6.7 Porte « Par mon cadre »~~ | ~~Nouveau module `CM.VuePorteCadre` parallèle à `CM.VuePorteProbleme`~~ | ✅ Livré 20/04/2026 (commits `44c20b4` + `f1b25bc`). Deux modules créés : `CM.DiagnosticCadre` (logique métier pure, délègue les niveaux à `CM.DiagnosticProbleme`) et `CM.VuePorteCadre` (contrôleur de vue). Référentiel enrichi : champ `famille` sur `CADRES`, nouvel objet `FAMILLES` (7 écoles de pensée) + `ORDRE_FAMILLES` + `CADRES_A_VENIR` (10 cadres projetés). API étendue : `infoFamille`, `familleDe`, `famillesOrdonnees`, `cadresParFamille`, `cadresAVenirParFamille`. Stepper 3 étapes (cadre → niveau → indicateurs) avec accordéon `<details>`/`<summary>` natif en étape 1. Palette UX 7 familles validée avec l'expert UX (bordure gauche 4px via `::before` + fond ≥ 94% luminance, chevron SVG 1.75rem renforcé). |
...
Chantier 6.7 **livré** le 20/04/2026 (commits `44c20b4` chore référentiel + `f1b25bc` feat porte complète). Les 3 portes actives (niveau, problème, cadre) cohabitent désormais sur l'accueil 4 portes — seule « Par ma maturité » reste en `Bientôt`.

**Candidats recommandés pour le chantier suivant** (à trancher avec Lætitia en début de prochaine session) :

1. **6.7a — Cadres complémentaires par co-occurrence** (🟡). Prolongement naturel de 6.7 : ajouter un bloc latéral dans les résultats de la porte cadre qui suggère *« ce cadre gagne à être combiné avec X et Y »*, calculé par co-occurrence sur `META.cadres`. Petit chantier (~30 lignes), haute cohérence intellectuelle avec le chantier qui vient d'être livré.
2. **7.6 — Liens cliquables vers les métriques citées** (🟡). Rendre cliquables les noms de métriques dans les encarts *Alternative recommandée* et *Vigilance*. Haute valeur d'usage, faible coût, dépend légèrement de la règle 1 (`CM.Html.escape()`) si on veut sécuriser à la même passe.
3. **7.2 — Stepper 3 étapes symétrique** (🟡). Ajouter la 3e étape sur les portes *problème* et *niveau* (symétrie avec la porte *cadre* qui porte déjà 3 étapes). Chantier structurant, peut être fait en extrayant un `CM.Stepper` générique.
