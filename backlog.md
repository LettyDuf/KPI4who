...
Dernière mise à jour : **26 avril 2026 — chantier 9.B.4 livré** (recherche globale dans la vue panier : input texte + chips Type/Niveau peuplés dynamiquement, estompage des cartes non-correspondantes, surlignage `<mark>` du fragment matché). Le chantier 9.C avait été livré plus tôt dans la journée (voie rapide d'ajout au panier). Le chantier 14 (refactor hexagonal) est livré le matin même, tag `mvp-chantier-14-livre` sur `6ea868b`. La voie rapide câble les icônes coche + ampoule en cascade sur les 5 vues consommatrices via la primitive `CM.Composants.htmlCarte` (porte problème, porte cadre, porte niveau, *Choisir mes indicateurs*, *Maturité & Recommandations*). Trois commits 9.C : `72d9ba7` voie rapide complète (CSS + primitive + câblage), `92ed54b` renforcement N2 de la lisibilité de l'icône active (vert sémantique fiabilité + fond pastille 10 %), `fdd8cba` correction de la vue d'entrée par la pyramide (Choisir mes indicateurs au lieu de Mon tableau de bord).
...
- **Événement majeur de la séance** : **chantier 9.B.4 livré** (audit + clôture de 9.B). Audit de reprise mené le 26/04/2026 a révélé que la décision ferme « recherche globale + 1-2 filtres contextuels » du cadrage 22/04 n'avait jamais été codée — 9.B était considéré clos sans cette tranche. Tranche 9.B.4 ouverte, mockup-preview produit (`00fb711`), option B arbitrée (input + chips Type/Niveau peuplés dynamiquement), module pur `CM.PanierRecherche` extrait avec marqueurs BEGIN/END (`154374d`), câblage CSS + JS (`096643e`). Test interactif Lætitia validé après diagnostic d'un bug de cache Safari (rechargement forcé Cmd+Option+R). Plus tôt dans la journée, **chantier 9.C livré** — la voie rapide d'ajout au panier est câblée bout-en-bout sur les 5 vues consommatrices. Trois commits : `72d9ba7` voie rapide (icônes coche + ampoule dans le pied de chaque carte du référentiel, option α arbitrée sur preview `abfdc62` parmi α/β/γ), `92ed54b` renforcement N2 de la lisibilité de l'icône active (vert sémantique fiabilité + fond pastille 10 %, choisi sur preview `113c8d4` parmi N1/N2/N3), `fdd8cba` correction de la vue d'entrée par la pyramide (*Choisir mes indicateurs* au lieu de *Mon tableau de bord* — diagnostic : ce n'était pas un bug 9.C, comportement existant depuis le commit `9b86473` du 18 avril, devenu visible avec le test du flux complet).
- **Architecture du câblage** : `CM.Composants.rendreIconesPanier(ind)` rend les deux boutons SVG (data-fiche-id + data-statut) en lisant `CM.Panier.statut`. `CM.App.basculerStatutPanier(ficheId, statut)` consomme `ajouter` / `retirer` / `basculer` selon l'état courant. `CM.App._rafraichirIconesPanier(resultat)` muté ciblément le DOM des icônes via l'abonnement à `CM.Panier`. Vue *Mon tableau de bord* hors-scope — utilise sa propre primitive `_htmlCartePanier` avec un bouton × retirer déjà câblé.
- **Vue d'entrée par la pyramide** : nouveau helper `CM.App._activerOnglet(idVue)` qui centralise la mise à jour onglet visuel + bascule de vue. Sélectionne l'onglet par parsing simple de son onclick. `_afficherApp` et `retourAccueil` passent désormais par ce helper et atterrissent sur *Choisir mes indicateurs*. Mémoire consignée : la pyramide elle-même est en sursis et disparaîtra au chantier 10.
...
- **Prochaines actions ouvertes (chantier 9 — tranches restantes)** :
  - **9.B vue panier** : ✅ **clos** — 9.B.0 → 9.B.4 livrés. La tranche 9.B.4 (recherche globale) a été identifiée comme dette en cours de séance (audit 26/04) puis livrée le jour même : module pur `CM.PanierRecherche` (`154374d`) + câblage barre + chips dans la vue (`096643e`), arbitrage option B sur preview `00fb711`. Tag `mvp-9b4-recherche-globale` posé sur le HEAD.
