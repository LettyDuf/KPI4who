...
Dernière mise à jour : **30 avril 2026 — chantier 1.ter intégralement clos** (10/10 cadres audités, dette globale ≈ 56%, ~80 corrections). Tag `mvp-chantier-1ter-clos` posé sur `c5fe2a1`. 6 constats collatéraux ouverts à statuer (n°1, n°4, n°6, n°8, n°9, n°10). Taxonomie étendue (`securite`, `cd`, `space` + famille `risque-securite`), 2 cadres orphelins déplacés en `CADRES_A_VENIR` (`bsc`, `mbo`), doctrine d'exclusivité de `generique` actée.
...
- **Événement majeur de la séance** : **chantier 1.ter intégralement clos** (10/10 cadres audités). Bilan : `okr` 0% · `itil` 38% · `kanban` 40% · `lean` 52% · `safe` 55% · `dora` 69% · `scrum` 83% · `bsc` 100% · `mbo` 100% · `generique` Pop1 nettoyée + 1 retag s9. ~80 corrections, dette globale ≈ 56%. **Extensions de taxonomie** (commit `662682a` lors de l'audit dora) : `securite`, `cd`, `space` ajoutés à `CADRES{}` + nouvelle famille `risque-securite`. **2 cadres orphelins déplacés vers `CADRES_A_VENIR`** : `bsc` (commit `e4be33c`), `mbo` (commit `0af6545`). **Doctrine d'exclusivité de `generique` actée** dans le code (commit `7fc5460`). **Tag posé** : `mvp-chantier-1ter-clos` sur `c5fe2a1`. Plus tôt même séance : axe Stratégique de 1.bis clos (`ba690cb`).
- **Apprentissages du jour (2)** : **(1)** Diagnostic « par parsing brut » sans isoler les attributs HTML donne une dette gonflée — toujours tester l'inclusion dans un `<span class="term-def">`. **(2)** Tagger un cadre par contiguïté (« niveau Tactique → SAFe ») sans appui doctrinal produit du faux signal massif (50–100% de dette selon le cadre). **Règle permanente** : pas de tag de cadre sans marqueur lexical OU métrique signature documentée du corpus officiel.
- **Validations par Lætitia** : ordre d'audit OKR→BSC→MBO→Kanban→Scrum→generique validé · option 1 (déplacement vers `CADRES_A_VENIR`) sur BSC et MBO · af-c4 ❌ par cohérence stricte · x1 et x2 retaggées `generique` après retrait scrum · ti-s5 garde `generique` (compliance ≠ security) · doctrine `generique` exclusive · constats n°9 et n°10 consignés · pose du tag MVP validée.
- **Workflow git sandbox** : pattern `mv .git/index.lock` + `mv .git/HEAD.lock` appliqué à chaque commit. Warnings `unable to unlink` persistent sans bloquer. ~25 fichiers `.git/*.lock.tmp*` accumulés cette session.
- **Prochaines actions ouvertes** : (1) **6 constats collatéraux à statuer** — n°1 (51 fiches hors `CADRES_META`), n°4 (densification t3), n°6 (divergence VOCAB.cadres / CADRES{} — possible bug latent à investiguer), n°8 (pauvreté corpus Scrum), n°9 (cadre `cx` manquant), n°10 (5 pistes : change-mgmt, sales, pmi-evm, grc-compliance, hr-analytics). (2) **1.bis axes restants** — Tactique (LPM, DORA, ROI), Programme (CRM probable faux positif), Op TI, Op Affaires. (3) Chantier 10 (gelé à `5655b03`) peut reprendre. (4) Items 6.9, 18, 12-17 en sommeil.
- **Tag posé** : `mvp-chantier-1ter-clos` sur `c5fe2a1`.
- **Blocages / questions ouvertes** : aucun. Fichiers `.todelete` toujours en attente côté ordi (~25 nouveaux `.git/*.lock.tmp*` à nettoyer).
...
**Constat collatéral n°4** *(remonté lors de l'audit cadre `okr` le 30/04/2026, à statuer hors 1.ter)*. La fiche **t3 « Objectifs trimestriels de portefeuille »** est la plus épurée du trio OKR : pas d'occurrence du mot **« ambitieux »** ni de la cible stretch (≈ 0,7) dans les champs visibles, alors que `s4` et `o9` portent l'ambition stretch. Densifier la définition de t3 pour intégrer un trait d'ambition stretch (cohérence éditoriale du trio OKR). Pas du périmètre 1.ter — à reprendre comme item de polissage dans une session future.
...
**Constat collatéral n°11** *(remonté en clôturant le constat n°6 le 30/04/2026, à statuer)*. **Harnais de tests `CM.RequeteMetriques` désaligné.** Le fichier `tests-requete-metriques.html` (~ligne 208) duplique son propre `VOCAB` pour exercer le module en isolation. Au moment du commit `d6ecd5f` (constat n°6 fix), ce VOCAB local contient encore les 10 valeurs périmées de `cadres` (`'dora', 'lean', 'okr', 'mbo', 'scrum', 'kanban', 'safe', 'itil', 'bsc', 'generique'` — incluant `mbo` et `bsc`, omettant `cd, space, securite`). Tests verts par accident : aucune fiche synthétique du harnais n'est taggée avec ces valeurs, donc la divergence ne se voit pas. Le harnais a perdu son rôle de gardien de l'invariant. Action : aligner ce VOCAB local sur `VOCAB.cadres` réel (11 valeurs), idéalement via la même source — par exemple générer la zone VOCAB du harnais depuis le module ou depuis §10.3. Risque actuel : faux sentiment de sécurité.

**Constat collatéral n°12** *(remonté en clôturant le constat n°6 le 30/04/2026, à statuer — dette préventive)*. **Test d'invariant `VOCAB.cadres ↔ CADRES{}` manquant.** Le constat n°6 aurait été détecté en automatique si un test garantissait l'égalité ensembliste entre les ids de `CADRES{}` (~ligne 4061 de `cadre-indicateurs.html`) et les valeurs admises par `VOCAB.cadres` (~ligne 4538). Plus généralement, un invariant similaire pourrait couvrir la cohérence avec §10.3 du doc compagnon (parser le tableau de signature, comparer aux deux tableaux du code). À spécifier dans le harnais existant ou dans un test dédié. Bénéfice : la prochaine extension/retrait de cadre échouerait au test si un seul des trois endroits est oublié.
