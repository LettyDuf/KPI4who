...
- `--accent-fiche-q-trame : rgba(31, 89, 69, 0.10)` — fond du surlignage anti-pattern. *Réglage du 06/05/2026 — opacité abaissée de 0.20 à 0.10 sur retour Lætitia (« surlignage trop fort dans le cadre Recommandation »). Décision tranchée sur mini-preview à 3 versions (`mini-preview-recommandation-anti-pattern.html`) ; version B (vert atténué) retenue plutôt que la doctrinale C (indigo + ambré) — la palette monochrome vert reste préférée comme signature fiche-question, l'atténuation suffit à redonner de la respiration au bloc.*
