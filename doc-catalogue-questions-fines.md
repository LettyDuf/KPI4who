...
Pour les questions contenant un terme-piège (vélocité, ROI, NPS, *occupation* — voir § 5). Le visiteur arrive, voit deux réponses voisines en juxtaposition, choisit en cliquant la colonne, est orienté vers la porte préconfigurée correspondante.

Exemples rédigés en mockup-preview : *« Comment améliorer la vélocité de mon équipe ? »* (mockup format scène 2, panel Coach Agile) et *« Comment mesurer l'occupation hebdomadaire de mon équipe ? »* (mockup pilotage hebdo, panel Coach Agile).

- Section *Votre vrai sujet* : deux colonnes côte à côte.
  - Colonne gauche — *Ce que vous demandez* : le terme posé (vélocité, occupation…), ses indicateurs propres, ses bons usages limités.
  - Colonne droite — *Ce que vous cherchez peut-être* : le terme voisin (prévisibilité, pilotage par le flux…), ses indicateurs, son intérêt actionnable.
- **Colonne entière cliquable** *(évolution v4.2 du 07/05/2026)* — la colonne EST l'unité d'action. `cursor: pointer` + hover discret (épaississement de la border-top de 3 à 5 px). Pas de bouton interne, pas de bandeau séparé.
- **Libellé de destination en pied de chaque colonne** — `.col-rep-destination`, format *« → Engagement & humain »* (gauche) / *« → Flux & goulots »* ou *« → Délais & prévisibilité »* (droite). Discret au repos (gris), passe en accent au hover.

**Évolution v4.1 → v4.2** *(actée le 07/05/2026 sur retour Lætitia v3 du mockup pilotage hebdo : « pourquoi ce bandeau d'orientation alors qu'il suffirait de cliquer sur le rectangle ? »)*. Suppression simultanée des deux mécaniques redondantes — boutons internes des colonnes et bandeau d'orientation séparé. Les deux doublonnaient l'action de la colonne. La colonne porte les deux rôles : cliquer (action) et nommer la destination (libellé en pied). Mockups de référence : `mockup-format-fiche-question.html` v4.2 (commit `80160fb`) et `mockup-fiche-question-pilotage-hebdo.html` v3 (commit `010987a`).

**Histoire de la doctrine.** v4 (06/05/2026, palette B vert profond puis bascule sur D bleu de Prusse, commits `26f5ccc` puis `cb5a353`) — palette signature et bandeau encore présent. v4.1 (06/05, commit `17d62f3`) — fond `.fiche` basculé en blanc franc (matière en blanc, crème en marge). v4.2 (07/05, commits `80160fb` et `010987a`) — colonnes cliquables, suppression des boutons et du bandeau d'orientation. La doctrine a convergé en 24 heures vers une économie de moyens : *la colonne est l'unité d'action, le mot juste fait le travail*.
...
### 4.6 Variante méta — trio à 3 axes invariants *(actée le 07/05/2026 sur fiche pastèque)*

**Origine.** La rédaction de la deuxième fiche-question — *« Mes indicateurs reflètent-ils la réalité du terrain ? »* (chantier 20.7) — a fait apparaître qu'une famille de fiches-questions ne peut pas se contenter de la doctrine § 4.4 *Trio par niveau*. Pour les fiches **méta** (questions sur les indicateurs eux-mêmes : véracité, biais, cohérence, écart déclaratif/observable), le trio orthodoxe n'est pas un triplet d'indicateurs *comparables* ; c'est un triplet d'**angles de triangulation** dont la combinaison produit la lecture juste. La doctrine § 4.4 reste le standard ; § 4.6 décrit la mécanique-sœur réservée aux fiches méta.

**Mécanique.**

- **3 axes invariants.** Les trois cards sont *toujours* les trois mêmes axes — par exemple, pour la fiche pastèque : *Déclaratif* (ce qu'on dit), *Objectif* (ce qu'on observe), *Humain anonyme* (ce qu'on vit). C'est la définition même de la triangulation : trois angles distincts sur la même réalité, jamais un seul.
- **Q1 (signal d'alerte) → ordre des cards.** Le visiteur arrive avec un symptôme — *« Tout est vert pourtant ça craque »* → axe Humain anonyme en card 1 ; *« Les chiffres ne disent pas la même chose que le terrain »* → axe Objectif en card 1 ; *« Les équipes ne croient plus aux annonces »* → axe Déclaratif en card 1. La triangulation reste complète, mais l'ordre dit *par où commencer*.
- **Q2 (niveau) → contenu de chaque card.** L'indicateur cible varie avec l'échelle. Au niveau équipe, l'axe *Humain anonyme* pointe vers un *Indice de bien-être* (`x4`) ; au niveau organisation, vers l'*Écart de vérité stratégique* (`s12`). Les trois axes sont les mêmes ; leur outillage change.

**Pourquoi cette variante.** Pour une fiche méta, fournir un trio *différent par niveau* (doctrine § 4.4) trahirait le sujet : la fiche enseigne précisément que la triangulation est invariante quand on cherche à débusquer la pastèque. La mécanique § 4.4 dit *« la nature de la question change avec le niveau »* — vrai pour les questions opérationnelles. La mécanique § 4.6 dit *« la nature de la question est la même à toutes les échelles, c'est l'instrument qui change »* — vrai pour les questions méta.

**Mode opératoire éditorial.** Pour chaque fiche-question méta, rédiger une **matrice 3 axes × 3 niveaux** (9 cellules), puis afficher le croisement Q1 × Q2 sélectionné. Le mockup `mockup-fiche-question-pasteque.html` (commit `f5fb2df`) implémente le patron de référence.

**Articulation avec § 4.4.** La doctrine § 4.4 reste le standard pour les fiches-questions de variante standard portant sur un objectif opérationnel (où démarrer une démarche, comment arbitrer, etc.). La doctrine § 4.6 est réservée aux fiches méta. La distinction se prend à la rédaction : si la question vise à mesurer un état du système d'indicateurs lui-même, c'est § 4.6 ; sinon c'est § 4.4. Aucun mélange dans une même fiche.
...
| 4 | **Seuil de bascule responsive de la double réponse** | ⏸ couplée à D4 (chantier 10). À retraiter avec la nouvelle mécanique v4.2 (colonnes cliquables, plus de bouton à miniaturiser) — la bascule responsive est désormais purement layout (2 colonnes → empilées), pas comportement | Quand D4 sera arbitrée |
| 5 | **Rédaction des 8 à 12 questions MVP** | 🟡 en cours — **3 fiches livrées** : (1) *Par où démarrer une démarche d'amélioration continue ?* (panel Coach Lean, variante standard, 05/05/2026) — mise en mockup à venir ; (2) *Mes indicateurs reflètent-ils la réalité du terrain ?* (panel Drucker / Toyota Way / Edmondson / DORA-SPACE, variante méta § 4.6, 07/05/2026, commit `f5fb2df`) ; (3) *Quoi mettre dans mon pilotage hebdomadaire ?* (panel Coach Agile, variante à reformulation § 4.3, 07/05/2026, commit `ffba1ae`). Reste 5 à 9 fiches MVP | Séances suivantes, panel par panel |
...
| 7 | **Fiche-question méta — *Mes indicateurs reflètent-ils la réalité du terrain ?*** | ✅ close 07/05/2026 — fiche rédigée éditorialement et mockup livré (`mockup-fiche-question-pasteque.html`, commit `f5fb2df`). Variante méta inaugurée — doctrine § 4.6 *Trio à 3 axes invariants*. Q1 (signal d'alerte, 4 options) → ordre des cards ; Q2 (niveau, 3 options) → contenu des cards. Matrice 9 cards (3 axes × 3 niveaux), 7 cibles existantes (s4 / lss-1 / p9 / s11 / x4 / p4 / s12), 2 fiches en cours de constitution (déclaratifs équipe + programme — dette éloquente). Cross-link activé vers la fiche-question d'amélioration continue (réciproque à activer dans son mockup). Lien sortant patrimonial vers fiche cadre triangulation (en cours de constitution) | — |
| 8 | **Mécanisme de cross-link entre fiches-questions** | ✅ close 07/05/2026 — les deux fiches-questions liées se citent réciproquement. Pasteque → amélioration continue (commit `f5fb2df`) ; amélioration continue → pasteque (commit `b2798cf`). Format technique acté : `<div class="cross-link">Question liée — <a href="#question=...">…</a></div>`, italique discret sous Recommandation | — |
...
- **07/05/2026 (revue + corrections)** — séance de revue Lætitia sur les v1 des fiches pasteque et pilotage hebdo. Trois retours sur le pilotage hebdo (boutons doublons, fond beige, question/réponse incohérente) traités en v2 (commit `528d08f`) avec rétrofit symétrique du fond blanc sur la pasteque (commit `8367a9c`). Quatrième retour sur le bandeau d'orientation (« pourquoi alors qu'il suffirait de cliquer sur le rectangle ? ») traité en v3 du pilotage hebdo (commit `010987a`) et en v4.2 du mockup format (commit `80160fb`). **Doctrine § 4.3 actualisée** : suppression des boutons internes ET du bandeau d'orientation, colonnes cliquables avec libellé de destination en pied. La colonne est l'unité d'action. Trois retours supplémentaires sur la pastèque (Q1 redondance des options, *équipe pilote*, *annonces internes* trop vague) traités en v2 et v3 (commits `798c4be` et `85c9f5f`) — Q1 ramené à 3 options orthogonales (1 par axe), formulation Déclaratif arrêtée à *« Les priorités nommées ne s'appliquent pas dans les arbitrages réels »* après itération à 3 mains. **Doctrine § 4.6 enrichie** : autant d'options Q1 que d'axes invariants, jamais plus.
- **07/05/2026 (suite)** — troisième fiche-question MVP rédigée éditorialement et mise en mockup : *« Quoi mettre dans mon pilotage hebdomadaire ? »* (chantier 20.5), **première matérialisation autonome de la variante § 4.3 (à reformulation)** hors mockup format. Couple acté : *Reporting d'occupation* ↔ *Pilotage par le flux* — choisi pour éviter la collision avec le mockup format scène 2 (vélocité ↔ prévisibilité) et pour rester orthodoxe Kanban. Indicateurs gauche : capacité, vélocité, points consommés, # tickets fermés, taux de remplissage. Indicateurs droite : débit, lead time, taux d'engagements tenus, WIP, change failure rate. Bandeau de raccourci : *Par mon problème — Flux & goulots* (basculer) ou *Engagement & humain* (garder). Bloc *Recommandation* positif avec la loi de Little comme pivot pédagogique pour articuler les deux anti-patterns nommés (*rapport d'occupation des heures* et *augmentation du WIP pour donner l'impression d'avancer*). Pied écoles : Scrum Guide / Kanban niveaux de vol / DORA-SPACE. Cross-link activé vers la fiche pastèque (le reporting d'occupation est une forme de déclaratif détaché de l'observable). Palette D, filet niveau équipe. Décision 20.5 passe à 🟡 en cours (3/8-12). Mockup : `mockup-fiche-question-pilotage-hebdo.html` (commit `ffba1ae`).
- **07/05/2026** — deuxième fiche-question rédigée éditorialement et mise en mockup : *« Mes indicateurs reflètent-ils la réalité du terrain ? »* (chantier 20.7), variante méta. Doctrine § 4.6 *Trio à 3 axes invariants* inaugurée comme mécanique-sœur de § 4.4 — pour les fiches méta, le trio orthodoxe est invariant (Déclaratif / Objectif / Humain anonyme), seul l'outillage varie selon l'échelle. Q1 (4 options de signal d'alerte) → ordre des cards ; Q2 (3 niveaux) → contenu. Matrice 9 cards, 2 cases en option mixte (déclaratifs équipe + programme — dette éloquente : le référentiel n'a pas d'instrument frontal pour l'auto-déclaration locale, qui est pourtant le terrain où la pastèque pousse). Pied écoles : Drucker / Toyota Way / Edmondson / DORA-SPACE. Cross-link activé vers la fiche-question d'amélioration continue, réciproque à activer dans `mockup-format-fiche-question.html`. Décision 20.7 close, décision 20.8 partiellement matérialisée. Mockup : `mockup-fiche-question-pasteque.html` (commit `f5fb2df`).
