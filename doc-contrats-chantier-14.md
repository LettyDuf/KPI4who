...
filtre = {
  ...
  cadres?:           Array<CadreId>,         // 14 valeurs : 'dora', 'cd', 'space', 'lean', 'okr', 'scrum', 'kanban', 'safe', 'itil', 'securite', 'cx', 'change', 'mbo', 'generique' (mise à jour 23/05/2026 — cf. journal §9)
...
- **01/05/2026 — Constat collatéral n°9 clos — cadre `cx` ajouté.** Création du cadre `cx` (Customer Experience) avec un corpus officiel à trois sources : Reichheld (NPS, *The Ultimate Question*, 1996/2003 — étendu en eNPS pour les collaborateurs), Dixon (CES, *The Effortless Experience*, 2013, étude HBR sur 75 000 interactions de service client), Hayes (CSAT, formalisation académique de la satisfaction client). Métriques signature documentées (au sens de la doctrine d'audit légitimité du 30/04) : NPS, eNPS, CSAT, CES. Création concomitante de la famille `experience` dans `FAMILLES{}` côté code (label « Expérience client », teinte `#e65100`, fond `#fff3e0`, position dans `ORDRE_FAMILLES` entre `objectifs` et `itsm` — les deux familles « relation au client/utilisateur » côte à côte). La famille `objectifs` (OKR/MBO/BSC/Hoshin) a été écartée : ce sont des cadres de cascade stratégique, pas d'orientation client — un placement par défaut aurait été une dette éditoriale. Audit-flash sur les 85 fiches du référentiel : 5 fiches `generique` réintégrées en `cx` (s2 NPS stratégique, af-sc1 CSAT, af-sc2 NPS Service, af-sc3 CES, af-r1 eNPS) + 1 fiche `itil` enrichie en `itil + cx` (af-sc5 TTFR — la fiche revendique elle-même « Cadres mobilisés : Effortless Experience (Dixon), The Ultimate Question (Reichheld) »). 9 faux positifs écartés (matches sur le mot français « ces » par regex `\bCES\b`). Cadre `cx` ajouté au présent §10.3 (12 valeurs). Doctrine §10.3 = source unique respectée — ordre orthodoxe doc → code → sentinelles → backlog (4 commits atomiques). Sentinelles `verifier-coherence-vocabulaire.js` + `lancer-tests-requete-metriques.js` exécutées sans régression.

- **23/05/2026 — Cadre `change` (Gestion du changement) ajouté — mini-chantier du constat collatéral n°10.** Création du cadre `change`, corpus officiel : Lewin (modèle dégel / changement / regel, 1947), Kotter (*Leading Change*, 1995, modèle en 8 étapes), ADKAR / Prosci (modèle individuel du changement), Bridges (*Managing Transitions*, 1991). Création concomitante de la famille `conduite-du-changement` dans `FAMILLES{}` (label « Conduite du changement », teinte `#3949ab`, fond `#e8eaf6`, insérée dans `ORDRE_FAMILLES` entre `decision` et `universel`). Aucune des 9 familles existantes ne convient à la conduite du changement, lignée intellectuelle distincte ; précédent assumé des familles `experience` et `risque-securite` à cadre unique. Trois fiches stratégiques retaguées `generique` vers `change` : s6 (Capacité d'absorption du changement), s7 (Taux de rotation volontaire réfléchi), s8 (Indice de confiance organisationnelle), qui forment le trio stratégique de la fiche-question `resistance-transformation` (décision Lætitia 23/05). Cadre `change` ajouté au présent §10.3 (13 valeurs) et à `VOCAB.cadres`. Commit code `7e88780` ; les 6 fiches d'indicateurs `cg-*` du cadre sont portées dans un commit dédié. Ferme le volet gestion du changement du constat collatéral n°10.

- **23/05/2026 (suite) — Cadre `mbo` (Management par objectifs) promu — volet 2 du mini-chantier (constat `26.c4.coll.1`).** Le cadre `mbo`, en réserve dans `CADRES_A_VENIR` depuis le chantier 1.ter, est promu vers `CADRES{}` (famille `objectifs`, inchangée). Libellé retenu : « Management par objectifs » (cohérent avec « Gestion du changement » ; le sigle MBO et Drucker restent dans la description). `mbo` ajouté à `VOCAB.cadres` et au présent §10.3 (14 valeurs). Commit code `2ade758` ; les 9 fiches d'indicateurs `mbo-*` suivent dans un commit dédié. Ferme le volet cascade MBO du mini-chantier éditorial.
