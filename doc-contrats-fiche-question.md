...
- **2026-07-12 (suite) · Candidats supplémentaires repliés sous le trio (backlog 29.5).** Retour du smoke test du 12/07/2026 : la limite de trois cards est parfois trop juste, Lætitia veut pouvoir montrer la largeur du choix sans casser la doctrine anti-surcharge. Cadrage acté : source éditoriale (pas de génération automatique par tags, cohérent avec la doctrine des panels nommés) ; le trio reste la recommandation, les candidats sont un repli discret. Schéma : champ optionnel de premier niveau `candidatsParNiveau`, objet figé indexé par niveau (`operationnel` / `programme` / `strategique`), chaque valeur étant un tableau figé de `{ ficheRef, intention }` (doctrine D3 : pas de nom dupliqué, le nom vient de `CM.Referentiel`). À la composition, les deux variantes à trio (§ 4.4 et § 4.6) publient `etat.candidats` = les candidats du niveau actif **moins ceux déjà affichés dans le trio courant** (le filtre est dynamique : avec les surcharges par signal, un même `ficheRef` peut être card sous un signal et candidat sous un autre). Au rendu, un bloc repliable natif (`<details>`) sous les cards : « Voir d'autres candidats (n) », lignes compactes nom cliquable (tiroir) + intention d'une phrase. Articulation avec la décision du 02/07 (« trio à 3 cartes préservé, richesse par les champs du tiroir ») : complétée, pas renversée — le tiroir donne la profondeur d'une fiche, le repli donne la largeur du choix. Garde-fou : la sentinelle vérifie les `ficheRef` et les clés de niveau de `candidatsParNiveau`.

- **2026-07-12 (suite) · Quatre surcharges « malaise » supplémentaires (audit 29.3).** Audit de l'équilibre performance / humain des intentions de la pastèque, sur remontée de Lætitia au smoke test du 12/07. Constats : l'axe humain est sain ; le déséquilibre venait du signal « malaise », qui ne surchargeait que 4 cellules sur 9 et retombait ailleurs sur des cellules de base au langage de pilotage. Traitement : 4 surcharges ajoutées (16 au total), dont 3 gardent le même `ficheRef` et reformulent l'intention à travers la lentille humaine (déclaratif × équipe `ti-d2`, déclaratif × programme `p10` avec l'image « vert dehors, rouge dedans » sans le mot banni, déclaratif × organisation `s4`) et 1 change de capteur (objectif × programme → `af-r5` micro-absentéisme, avec son garde-fou agrégé / jamais individuel). Le signal malaise est désormais servi sur 8 cellules sur 9 (humain × organisation reste sur sa surcharge `s16` existante ; la 9e, humain × programme, porte `p11` déjà pertinent). Enseignement de doctrine : `parSignal` sert autant à **resémantiser** un même indicateur selon le signal qu'à en changer.
