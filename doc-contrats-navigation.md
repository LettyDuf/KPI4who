...
- 08/05/2026 (chantier 23.a, ouverture refonte radicale accueil) — § 4.1 *Invariants de topologie* amendé (commit `30186ba`) : le bandeau du haut passe de **8 à 6 entrées** (Accueil · Mon tableau de bord · Cascade stratégique · La maturité ? · Lexique · À propos) ; les onglets *Par mes 4 axes* et *Par ma question* sortent du bandeau et sont absorbés dans l'accueil unifié (chips et cartouche compagnon). Nouvel invariant fondateur : *« l'accueil est le seul point d'entrée pour choisir un indicateur »* — remplace l'invariant fondateur du 23/04/2026 *« deux modes d'entrée parallèles »* qui posait deux onglets distincts. Forme historique consultable dans le commit `9aa2ec6`. § 6.1 / 6.2 / 6.3 enrichis d'encarts de dépréciation (commit `35f3bae`) : façades de portes mortes au 23.f, façades CM.App nav (`basculerAccueil`, `afficherAccueil`, `entrer`) supprimées au 23.f, hash `#pyramide`/`#portes` supprimés. **Nouveau § 6.6** *Contrats de l'accueil unifié — `CM.AccueilUnifie`* (commit `edb11d8`) : façade publique posée en portée équilibrée (pub/sub `surChangementFiltre`, persistance via `CM.Preferences`, branchement sur le traducteur orthodoxe survivant). Les URLs partageables des filtres (portée exhaustive) sont reportées au chantier 24.
