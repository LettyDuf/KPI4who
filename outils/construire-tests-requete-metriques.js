#!/usr/bin/env node
/**
 * outils/construire-tests-requete-metriques.js
 *
 * Patron : générateur Node.js zéro-dépendance, script strict.
 * Calque strict de outils/construire-tests-panier.js.
 *
 ...
 * pas charger cadre-indicateurs.html pour aller y piocher CM.RequeteMetriques.
 *
 * Mécanique : on extrait le bloc CM.RequeteMetriques de cadre-indicateurs.html
 * (entre les marqueurs BEGIN/END posés au commit 3 de l'étape b du
 * chantier 14) et on l'inline directement dans tests-requete-metriques.html
 * (entre les marqueurs CM.REQUETE-METRIQUES-INJECTION-BEGIN / END). Le
 * harnais teste alors le module dans sa propre page — même-origine
 * garantie, plus de blocage.
 *
 ...
 *     possible tant qu'on régénère après chaque modif de CM.RequeteMetriques.
 *   - Les ports CM.Referentiel et CM.IndicateursMeta sont montés en
 *     doubles de test (stubs in-memory) déclarés en dur dans le harnais
 *     (hors zone d'injection) — ils ne sont pas régénérés.
 *
 * Usage :
 *   node outils/construire-tests-requete-metriques.js
 *
 * Exit codes :
 *   0 — succès, tests-requete-metriques.html régénéré.
...
var CIBLE_HARNAIS = path.join(RACINE, 'tests-requete-metriques.html');

/* ─── Marqueurs (doivent apparaître exactement une fois, chacun) ─── */
var MARQUEUR_SOURCE_BEGIN = '/* ══ CM.RequeteMetriques — BEGIN';
var MARQUEUR_SOURCE_END   = '/* ══ CM.RequeteMetriques — END ══ */';
var MARQUEUR_CIBLE_BEGIN  = '<!-- CM.REQUETE-METRIQUES-INJECTION-BEGIN -->';
var MARQUEUR_CIBLE_END    = '<!-- CM.REQUETE-METRIQUES-INJECTION-END -->';
...
function main() {
  ...
  /* ─── 4. Composition HTML (wrapper <script> + prélude CM) ───────
     ...
     Le prélude `window.CM = window.CM || {}` est posé en amont de l'injection
     dans le harnais (avec les stubs des ports), il n'est donc pas répété ici. */
  ...
  try {
    resultat = injecterBloc(
      cible, MARQUEUR_CIBLE_BEGIN, MARQUEUR_CIBLE_END, injection, 'tests-requete-metriques.html'
    );
  } catch (e) {
    process.stderr.write(e.message + '\n');
    process.exit(1);
  ...
  process.stdout.write(
    'OK — bloc CM.RequeteMetriques injecté dans tests-requete-metriques.html ('
